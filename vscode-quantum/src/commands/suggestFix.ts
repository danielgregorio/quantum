...
import { quantumTags as schemaTags } from '../language/quantumSchema';
...
export class QuantumCodeActionProvider implements vscode.CodeActionProvider {
    ...
    private _getSimilarTags(unknownTag: string): string[] {
        // The tags the schema describes (generated from quantum-lsp for Core and AI);
        // this list was hand-kept and suggested tags that never existed (q:output, q:try).
        const quantumTags = schemaTags.map(tag => `q:${tag.name}`);
