...
def print_report(results: list[dict]):
    ...
    for r in results:
        ...
        if score < 5 and not r["qa_spec"]:
            print(f"        -> Suggestion: run /qa {r['name']} to generate qa_spec.yaml")

    print("\n" + "=" * 60)

    low = [r for r in results if r.get("score", 0) < 3]
    if low:
        print(f"WARNING: {len(low)} feature(s) with score < 3: {', '.join(r['name'] for r in low)}")
    else:
        print("All analysed features have score >= 3.")
    print("=" * 60 + "\n")
