...
class TestMemoryAdapter:
    ...
    def test_ack_message(self, broker):
        # It was `assert True  # Should not raise`: it passed even if ack()
        # were a `pass`. ack() only makes sense on a DELIVERED message — it
        # leaves the list pending acknowledgement. Acking a message nobody
        # consumed does nothing, and that is correct.
        received = []
    ...
    def test_nack_to_dlq(self, broker):
        ...
        # It was `assert len(dlq_messages) >= 0  # DLQ behavior depends on
        # implementation`. That passes if the DLQ never receives anything —
        # that is, the test called "nack to DLQ" asserted nothing about the
        # DLQ, and would pass with the message being dropped. That was
        # exactly what the SqliteAdapter did.
        assert len(dlq_messages) == 1, (
            f"the rejected message should be in the DLQ, but it has "
            f"{len(dlq_messages)}")
    ...
    def test_purge_queue(self, broker):
        ...
        # It was `assert count >= 0`: it passed if purge deleted nothing.
        assert count == 5, f"5 messages queued, purge returned {count}"
