...
from quantum.runtime.message_broker import Message, MessageBroker, QueueInfo
from quantum.runtime.adapters.memory_adapter import MemoryAdapter, InMemoryAdapter
...
class TestMemoryAdapter:
    ...
    def test_ack_message(self, broker):
        # Era `assert True  # Should not raise`: passava mesmo se ack() fosse
        # um `pass`. ack() so tem sentido sobre uma mensagem ENTREGUE — ela
        # sai da lista de pendentes de confirmacao. Acked sobre uma mensagem
        # que ninguem consumiu nao faz nada, e isso esta correto.
        received = []
        broker.declare_queue("ack-test")
        broker.consume("ack-test", lambda m: received.append(m), prefetch=1)

        broker.send("ack-test", Message(body="test"))
        time.sleep(0.2)

        assert len(received) == 1
        delivered = received[0]
        assert delivered.id in broker._pending_acks

        broker.ack(delivered)

        assert delivered.id not in broker._pending_acks
    ...
    def test_nack_to_dlq(self, broker):
        ...
        # Era `assert len(dlq_messages) >= 0  # DLQ behavior depends on
        # implementation`. Isso passa se a DLQ nunca receber nada — ou seja,
        # o teste chamado "nack para DLQ" nao afirmava nada sobre a DLQ, e
        # passaria com a mensagem sendo descartada. Era exatamente o que o
        # SqliteAdapter fazia.
        assert len(dlq_messages) == 1, (
            f"a mensagem rejeitada deveria estar na DLQ, mas ela tem "
            f"{len(dlq_messages)}")
        assert dlq_messages[0].body == "to-dlq"
    ...
    def test_purge_queue(self, broker):
        ...
        # Era `assert count >= 0`: passava se purge nao apagasse nada.
        assert count == 5, f"5 mensagens enfileiradas, purge devolveu {count}"
        assert broker.get_queue_info("purge-test").message_count == 0
