...
class QuantumQItem(pytest.Item):
    ...
    def runtest(self):
        ...
        # The examples that send mail write it to the log (MAIL-1: host: log);
        # before, q:mail "sent" everything to a print by default.
        runtime = ComponentRuntime(config={'mail': {'host': 'log', 'from': 'examples@quantum.dev'}})
    ...
    def _run_application(self, app, runtime):
        ...
        if app_type == 'ui':
            ...
            # UI-8: html is layout only; the mobile example is built for its
            # own (Laboratory) target, the one that runs its logic.
            alvo = 'mobile' if self.name == 'test-mobile.q' else 'html'
            output = UIBuilder().build(app, target=alvo)
    ...
    def _mock_datasource(self, runtime: "ComponentRuntime"):
        ...
        try:
            ...
            # In a temporary folder: the default is <repo>/test_data/quantum_test.db,
            # and no test writes into the repository.
            db = TestDatabase(str(Path(tempfile.mkdtemp(prefix='quantum-q-tests-')) / 'quantum_test.db'))
            db.setup()
