...
class TestAckIsPerThread:
    """Two active subscriptions must not acknowledge each other's message.

    `self._current_message` was ONE instance attribute, written at the top of
    every subscription's callback — and each subscription runs on its own
    polling thread. It was enough for the second thread to enter the callback
    while the first was still processing for handler A's `<q:ack>` to
    acknowledge B's message: A's went back to the queue (redelivered,
    reprocessed) and B's left the queue without ever being acknowledged by
    its own handler.
    """

    class _FakeBroker:
        """A broker that only keeps the callback and notes what was acknowledged."""
        ...
        def ack(self, msg):
            with self._lock:
                self.acknowledged.append(msg.id)

        def nack(self, msg, requeue=True):
            with self._lock:
                self.acknowledged.append(f'nack:{msg.id}')

    def test_two_subscriptions_do_not_acknowledge_each_other_s_message(self):
        service = MessageQueueService()
        broker = self._FakeBroker()
        service._broker = broker

        # The two threads only call ack() after BOTH have entered the
        # callback. That is exactly the window in which the shared attribute
        # held the value of the last thread to arrive.
        ...
        service.subscribe(name='a', queue='queue-a', handler=handler, ack='manual')
        service.subscribe(name='b', queue='queue-b', handler=handler, ack='manual')

        msg_a = Message(id='msg-a', queue='queue-a', body='a')
        msg_b = Message(id='msg-b', queue='queue-b', body='b')

        threads = [
            threading.Thread(target=broker.callbacks['queue-a'], args=(msg_a,)),
            threading.Thread(target=broker.callbacks['queue-b'], args=(msg_b,)),
        ]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=10)
            assert not t.is_alive(), 'the handler hung'

        # Each acknowledged once. Before: ['msg-b', 'msg-b'] — msg-a lost
        # and msg-b acknowledged twice.
        assert sorted(broker.acknowledged) == ['msg-a', 'msg-b']

    def test_nack_is_per_thread_too(self):
        service = MessageQueueService()
        broker = self._FakeBroker()
        service._broker = broker
        ...
        assert sorted(broker.acknowledged) == ['nack:m1', 'nack:m2']

    def test_ack_outside_a_handler_does_nothing(self):
        """With no message on this thread, ack() is a no-op — it must not
        take the message another thread is processing."""
        service = MessageQueueService()
        broker = self._FakeBroker()
        service._broker = broker
        ...
        # The main thread never entered a callback.
        service.ack()
        service.nack()
