...
from quantum.core.features.game_engine_2d.src.ast_nodes import (
    SceneNode, SpriteNode, PhysicsNode, CameraNode, PrefabNode, InstanceNode, GroupNode, HudNode,
    SoundNode, AnimationNode, TimerNode,
    TweenNode, ParticleNode, TilemapNode, TilemapLayerNode, EventNode, OnCollisionNode, RawCodeNode, HudTileNode, HudCounterNode, HudCollectionNode,
    HudSlotNode, HudOptionNode, SceneTransitionNode, PersistentNode, MapNodeDef, MapPathNode,
    EventActionNode, EnemyNode, EnemyAiNode, EnemyDefeatNode, EnemyOnHitNode, EnemyOnKillNode,
)
...
from quantum.runtime.godot_templates import (
    ...
    q2g, q2g_int, q2g_gravity, q2g_speed, q2g_jump,
...
class TestTscnBuilder:
    ...
    def test_sub_resource(self):
        tscn = TscnBuilder()
        tscn.add_sub_resource('RectangleShape2D', {
            'size': 'Vector2(32, 32)',
        })
        tscn.add_node('Root', 'Node2D')
        result = tscn.build()
...
class TestProjectGodot:
    ...
    def test_basic_project(self):
        ...
        # retro console native resolution
...
class TestPhysicsGeneration:
    ...
    def test_snes_gravity(self, codegen, output_dir):
        """Gravity uses retro console classic constant regardless of .q value."""
        scene = SceneNode('main')
        physics = PhysicsNode()
        ...
        # retro console classic default gravity: 750 px/s²
...
class TestPrefabGeneration:
    ...
    def test_enemy_prefab_with_ai(self, codegen, output_dir):
        ...
        prefab = PrefabNode('walker')
        prefab.entity_type = 'enemy'
        prefab.movement = 'patrol'
        prefab.move_speed = 0.5
        prefab.turn_on_edge = True
        prefab.facing = 'left'
        walker_sprite = SpriteNode('walker_sprite')
        walker_sprite.src = 'walker.png'
        walker_sprite.body = 'dynamic'
        prefab.add_child(walker_sprite)

        codegen.generate(scene, prefabs=[prefab], output_dir=output_dir)
        prefab_tscn = Path(output_dir, 'prefabs', 'walker.tscn').read_text(encoding='utf-8')
        assert 'CharacterBody2D' in prefab_tscn

        # Check AI script was generated
        assert Path(output_dir, 'scripts', 'prefab_walker.gd').exists()
        ai_script = Path(output_dir, 'scripts', 'prefab_walker.gd').read_text(encoding='utf-8')
...
class TestEventGeneration:
    ...
    def test_on_collision_generates_handler(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
        assert '_on_hero_collision' in script
...
class TestSceneScript:
    ...
    def test_registers_sprites_in_ready(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
        assert 'QuantumBridge.register_sprite("hero"' in script
...
class TestGroupGeneration:
    ...
    def test_group_with_children(self, codegen, output_dir):
        ...
        group.tag = 'enemy'
        enemy1 = SpriteNode('walker1')
        enemy1.src = 'walker.png'
        enemy1.x = 100
        enemy1.y = 200
...
class TestCompleteScene:
    ...
    def test_hero_like_scene(self, codegen, output_dir):
        """Test a scene similar to a Hero game."""
        scene = SceneNode('green_island_1')
        scene.width = 4096
        scene.height = 512
        ...
        # Tilemap
        tilemap = TilemapNode('world')
        tilemap.src = 'classic_tileset.png'
        tilemap.tile_width = 16
        tilemap.tile_height = 16
        ...
        result = codegen.generate(scene, output_dir=output_dir, project_name='HeroSMW')
        out = Path(result)
        ...
        assert 'config/name="HeroSMW"' in project
        assert 'move_left=' in project
        assert 'jump=' in project
...
class TestEdgeCases:
    ...
    def test_scene_with_only_physics(self, codegen, output_dir):
        scene = SceneNode('main')
        physics = PhysicsNode()
        physics.gravity_y = 2.0  # Quantum value — Godot uses retro console constant
        scene.add_child(physics)
        codegen.generate(scene, output_dir=output_dir)
        project = Path(output_dir, 'project.godot').read_text(encoding='utf-8')
        # Uses retro console classic gravity regardless of .q value
        assert '750.0' in project
...
class TestTileBasedHud:
    ...
    def _make_tile_hud_scene(self):
        ...
        hud.sprite_prefix = 'assets/classic/sprites/hud_'

        tile = HudTileNode()
        tile.sprite = 'hero_text.png'
        tile.x = 24
        tile.y = 8
        hud.add_child(tile)
    ...
    def test_tile_hud_collection_for_loop(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'hud_manager.gd').read_text(encoding='utf-8')
        assert 'for i in range(bonus_coins)' in script
    ...
    def test_tile_hud_sprite_prefix_resolution(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'hud_manager.gd').read_text(encoding='utf-8')
        # Sprite prefix should be applied: hero_text.png -> assets/classic/sprites/hud_hero_text.png
        assert 'assets/classic/sprites/hud_hero_text.png' in script
...
class TestMapSceneCodegen:
    ...
    def test_map_scene_generates_map_controller(self, output_dir, codegen):
        ...
        hero = SpriteNode('Hero')
        hero.src = 'hero_map.png'
        hero.x = 32
        hero.y = 180
        world_map.add_child(hero)
...
class TestPlayerControllerParity:
    ...
    def test_player_item_block_detection(self, codegen, output_dir):
        """Player controller should detect item_block and rotating_block collisions."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
        player = SpriteNode('hero')
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'player_controller.gd').read_text(encoding='utf-8')
        assert 'item_block-hit' in script
...
class TestEnemyPrefabParity:
    ...
    def test_prefab_uses_move_speed_directly(self, codegen, output_dir):
        ...
        sprite = SpriteNode('walker_body')
        sprite.src = 'walker.png'
        sprite.tag = 'enemy'
        sprite.body = 'dynamic'
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_walker.gd').read_text(encoding='utf-8')
    ...
    def test_prefab_die_emits_event(self, codegen, output_dir):
        ...
        prefab = PrefabNode('patroller')
        prefab.entity_type = 'enemy'
        prefab.movement = 'patrol'
        sprite = SpriteNode('g_body')
        sprite.src = 'patroller.png'
        sprite.tag = 'enemy'
        sprite.body = 'dynamic'
        prefab.add_child(sprite)
        ...
        scene = self._make_enemy_scene(prefab)
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_patroller.gd').read_text(encoding='utf-8')
...
class TestSceneEventsParity:
    ...
    def test_stomp_detection_generated(self, codegen, output_dir):
        ...
        assert 'walker_squished' in script
    ...
    def test_death_sequence_classic(self, codegen, output_dir):
        """death-sequence=classic should generate kill_player + death_complete."""
        scene = self._make_level_scene(death_sequence='classic', death_timer=3.5)
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
    ...
    def test_death_sequence_respawn(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
        assert 'QuantumBridge.respawn("hero"' in script
...
class TestGameOverScreen:
    ...
    def test_game_over_screen_generated_with_classic(self, codegen, output_dir):
        """death_sequence=classic should generate game_over_screen.gd."""
        scene = SceneNode('level-1')
        scene.width = 800
        scene.height = 600
        scene.death_sequence = 'classic'
        player = SpriteNode('hero')
        player.src = 'hero.png'
        player.body = 'dynamic'
        player.controls = 'arrows'
    ...
    def test_game_over_not_generated_without_classic(self, codegen, output_dir):
        """Without death_sequence=classic, no game_over_screen.gd."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
        player = SpriteNode('p')
...
class TestNamedCollisionLayers:
    ...
    def test_default_layer_names_without_physics(self, codegen, output_dir):
        """Without physics collision layers, use default classic names."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
        player = SpriteNode('p')
    ...
    def test_parser_named_collision_layer_string(self):
        ...
        src = '''<q:application id="g" type="game" engine="2d">
            <qg:scene name="main" width="800" height="600">
                <qg:sprite id="hero" src="m.png" x="0" y="0"
...
class TestBlockPrefabBehavior:
    """Test item_block (content + hits) code generation."""

    def test_item_block_generates_hit_method(self, codegen, output_dir):
        """ItemBlock with content and hits should generate hit() method."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        prefab = PrefabNode('item_block')
        prefab.entity_type = 'block'
        prefab.content = 'coin'
        prefab.hits = 1
        sprite = SpriteNode('item_block_body')
        sprite.src = 'item_block.png'
        sprite.tag = 'item_block'
        sprite.body = 'static'
        ...
        inst = InstanceNode('item_block')
        inst.x = 100
        inst.y = 200
        scene.add_child(inst)

        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_item_block.gd').read_text(encoding='utf-8')
        assert 'func hit()' in script
        assert '_hits_remaining' in script
        assert '_is_used' in script
        assert 'block-hit' in script

    def test_item_block_hits_countdown(self, codegen, output_dir):
        """ItemBlock should countdown hits and switch to used state."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
    ...
    def test_item_block_used_state_stops_animation(self, codegen, output_dir):
        """When item_block becomes used, animation should stop."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
    ...
    def test_item_block_scene_is_static_body(self, codegen, output_dir):
        """ItemBlock TSCN should use StaticBody2D with script."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
        ...
        codegen.generate(scene, output_dir=output_dir)
        tscn = Path(output_dir, 'prefabs', 'item_block.tscn').read_text(encoding='utf-8')
        assert 'StaticBody2D' in tscn
        assert 'prefab_item_block.gd' in tscn
...
class TestCollectibleBehavior:
    ...
    def test_bonus_coin_shimmer(self, codegen, output_dir):
        """Bonus coin (collectible + tag bonus_coin) should have shimmer modulate effect."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        prefab = PrefabNode('bonus_coin')
        prefab.entity_type = 'item'
        prefab.collectible = True
        prefab.reward = 'score:50'
        sprite = SpriteNode('yc')
        sprite.src = 'bonus_coin.png'
        sprite.tag = 'bonus_coin'
        sprite.sensor = True
        prefab.add_child(sprite)
        scene.add_child(prefab)

        inst = InstanceNode('bonus_coin')
        inst.x = 300
        inst.y = 200
        scene.add_child(inst)

        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_bonus_coin.gd').read_text(encoding='utf-8')
...
class TestCheckpointParsing:
    ...
    def test_parser_checkpoint_default_none(self):
        ...
        src = '''<q:application id="g" type="game" engine="2d">
            <qg:scene name="main" width="800" height="600">
                <qg:prefab name="walker" entity-type="enemy">
...
class TestTilemapDataSrcGeneration:
    ...
    def test_tilemap_data_src_loader_has_correct_json_path(self, codegen, output_dir):
        ...
        tilemap.data_src = 'assets/classic/tile_map.json'
        tilemap.tile_width = 16
        tilemap.tile_height = 16
...
class TestPlayerControllerHitCall:
    """Player controller should call collider.hit() on item_block/rotating_block."""

    def test_item_block_hit_call_before_emit(self):
        from quantum.runtime.godot_templates import PLAYER_CONTROLLER_GD
        # The template should call hit() before emit_event for item_block
        item_block_section = PLAYER_CONTROLLER_GD[PLAYER_CONTROLLER_GD.find('tag == "item_block"'):]
        assert 'collider.hit()' in item_block_section
...
# ==========================================================================
# Phase 9: Rotating Block Timing + Bonus Coin Polish Tests
# ==========================================================================
...
class TestBonusCoinPolish:
    """Bonus coin should have random phase init and speed * 5.0."""

    def test_bonus_coin_random_phase(self, codegen, output_dir):
        scene = SceneNode('main')
        prefab = PrefabNode('bonus_coin')
        prefab.entity_type = 'item'
        prefab.collectible = True
        prefab.reward = 'score:1000'
        # Add sprite child with bonus_coin tag
        sprite = SpriteNode('bonus_coin_sprite')
        sprite.src = 'bonus_coin.png'
        sprite.tag = 'bonus_coin'
        prefab.add_child(sprite)
...
class TestIntegrationEndToEnd:
    ...
    def test_complete_scene_generates_all_functions(self, codegen, output_dir):
        """A complete classic-style scene should generate all expected functions."""
        scene = SceneNode('main')
        scene.death_sequence = 'classic'
        scene.death_timer = 3.0
        ...
        # Prefabs
        walker = PrefabNode('walker')
        walker.defeated_by = 'stomp'
        walker.stomp_bounce = -200
        checkpoint = PrefabNode('checkpoint')
        checkpoint.tag = 'checkpoint'

        codegen.generate(scene, prefabs=[walker, checkpoint], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
...
class TestParityTwoStageStomp:
    ...
    def test_two_stage_squish(self, codegen, output_dir):
        scene, walker = self._scene_with_stomp()
        codegen.generate(scene, prefabs=[walker], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
...
class TestParityAreaEntered:
    ...
    def test_area_entered_handler_delegates(self, codegen, output_dir):
        ...
        codegen.generate(scene, output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
        assert '_on_hero_area_entered' in script
...
class TestDynamicSpriteRegistration:
    ...
    def test_static_registration_without_instances(self, codegen, output_dir):
        ...
        assert 'register_sprite("hero"' in script
...
class TestEnemyCodegen:
    ...
    def _make_enemy_scene(self):
        ...
        # Instances
        inst1 = InstanceNode('walker')
        inst1.instance_id = 'walker1'
        inst1.x = 416
        inst1.y = 370
        scene.add_child(inst1)

        return scene, walker, enemy

    def test_enemy_script_ai_data(self, codegen, output_dir):
        scene, walker, enemy = self._make_enemy_scene()
        codegen.generate(scene, prefabs=[walker], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_walker.gd').read_text(encoding='utf-8')
    ...
    def test_enemy_meta_key_per_prefab(self, codegen, output_dir):
        ...
        codegen.generate(scene, prefabs=[walker], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'scene_main.gd').read_text(encoding='utf-8')
        # Should use prefab-specific meta key, not hardcoded "walker_squished"
        assert 'walker_squished' in script
    ...
    def test_enemy_backward_compat(self, codegen, output_dir):
        ...
        walker.health = 2
        walker.stomp_bounce = -200
        walker.movement = 'patrol'
        walker.move_speed = 50.0
        sprite = SpriteNode('walker_body')
        sprite.src = 'walker.png'
        sprite.tag = 'enemy'
        sprite.body = 'dynamic'
        ...
        assert 'walker_squished' in scene_script
...
# ================================================================
# BATCH classic FEATURES TESTS
# ================================================================

class TestQblockTagDetection:
    """F1: ItemBlock prefab with tag='item_block' generates block script even without explicit content/hits."""

    def test_item_block_tag_generates_hit_method(self, codegen, output_dir):
        """A prefab with sprite tag='item_block' should generate hit() method via _write_block_script."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        item_block = PrefabNode('item_block')
        sprite = SpriteNode('item_block_body')
        sprite.src = 'item_block.png'
        ...
        anim.auto_play = True
        sprite.add_child(anim)
        item_block.add_child(sprite)

        codegen.generate(scene, prefabs=[item_block], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_item_block.gd').read_text(encoding='utf-8')
        assert 'func hit()' in script
        assert '_hits_remaining' in script
    ...
    def test_item_block_tag_defaults_coin_content_1_hit(self, codegen, output_dir):
        """ItemBlock without explicit content/hits defaults to coin/1."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
    ...
    def test_item_block_with_explicit_content_uses_it(self, codegen, output_dir):
        """ItemBlock with explicit content='mushroom' uses that instead of default."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        item_block = PrefabNode('item_block')
        item_block.content = 'mushroom'
        item_block.hits = 1
        sprite = SpriteNode('item_block_body')
        sprite.src = 'item_block.png'
        sprite.tag = 'item_block'
...
class TestShimmerEffect:
    """F3: Bonus coin collectible uses shimmer instead of cos() spin."""

    def test_bonus_coin_uses_shimmer_not_spin(self, codegen, output_dir):
        """Bonus coin collectible should use modulate shimmer, not scale.x = cos()."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        yc = PrefabNode('bonus_coin')
        yc.collectible = True
...
class TestChargeAI:
    """F4: Bullet Charger uses 'charge' AI type - horizontal flight without gravity."""
    ...
    def test_charge_ai_generates_no_gravity(self, codegen, output_dir):
        ...
        bb = PrefabNode('bullet_charger')
        sprite = SpriteNode('bb_body')
        sprite.src = 'bullet_charger.png'
        sprite.tag = 'enemy'
        sprite.body = 'dynamic'
        sprite.collision_layer = 2
        sprite.collision_mask = 1
        ...
        on_kill.effect = 'puff'
        defeat.on_kill = on_kill
        enemy = EnemyNode('bullet_charger')
        enemy.ai = ai
        enemy.defeat = defeat
        scene.add_child(enemy)

        codegen.generate(scene, prefabs=[bb], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_bullet_charger.gd').read_text(encoding='utf-8')
...
class TestFlyingBlock:
    """F6: Flying item_block generates oscillation + hit handler."""

    def test_flying_item_block_generates_oscillation(self, codegen, output_dir):
        """Prefab with flying=True generates sine wave movement."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600

        fqb = PrefabNode('flying_item_block')
        fqb.flying = True
        ...
        script = Path(output_dir, 'scripts', 'prefab_flying_item_block.gd').read_text(encoding='utf-8')
        assert 'sin(' in script
        assert 'func hit()' in script


class TestEmergeAI:
    """F7: Biting Plant uses 'emerge' AI type - timer-based emerge/retract."""
    ...
    def test_emerge_ai_generates_timer_cycle(self, codegen, output_dir):
        ...
        pp = PrefabNode('biting_plant')
        sprite = SpriteNode('pp_body')
        sprite.src = 'biting_plant.png'
        sprite.frame_width = 16
        sprite.frame_height = 16
        sprite.tag = 'hazard'
        sprite.body = 'static'
        ...
        defeat.by = 'fire'
        defeat.health = 1
        enemy = EnemyNode('biting_plant')
        enemy.ai = ai
        enemy.defeat = defeat
        scene.add_child(enemy)

        codegen.generate(scene, prefabs=[pp], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_biting_plant.gd').read_text(encoding='utf-8')
...
class TestMushroomPipeline:
    ...
    def test_item_block_mushroom_content_codegen(self, codegen, output_dir):
        """ItemBlock with content='mushroom' generates correct block script."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
...
class TestCollectibleDataDriven:
    ...
    def test_bonus_coin_escalating_score(self, codegen, output_dir):
        """Bonus coin collectible should emit event with escalating score support."""
        scene = SceneNode('main')
        scene.width = 800
        scene.height = 600
        ...
        codegen.generate(scene, prefabs=[yc], output_dir=output_dir)
        script = Path(output_dir, 'scripts', 'prefab_bonus_coin.gd').read_text(encoding='utf-8')
        assert 'bonus_coin-collected' in script
