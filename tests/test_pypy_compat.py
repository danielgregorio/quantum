...
class TestQuantumPyPyAdapter:
    ...
    def test_get_memory_usage_without_psutil(self, monkeypatch):
        # The fallback imported `resource` outside the try; `resource` only
        # exists on POSIX, so without psutil the function broke on Windows.
...
class TestCrossImplementation:
    ...
    def test_lambda_functions(self):
        ...
        add = lambda x, y: x + y  # noqa: E731 -- this test is about lambdas
        assert add(3, 4) == 7

        square = lambda x: x ** 2  # noqa: E731 -- this test is about lambdas
