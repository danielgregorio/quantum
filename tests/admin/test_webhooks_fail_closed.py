...
class TestGitHub:
    ...
    def test_unconfigured_refuses_even_a_well_formed_signature(self):
        ...
        assert service().verify_github_signature(
            payload, sign("whatever-the-attacker-wants", payload)) is False

    def test_configured_accepts_the_right_signature(self):
        payload = b'{"ref":"refs/heads/main"}'
        svc = service(github_secret="the-real-secret")
        assert svc.verify_github_signature(
            payload, sign("the-real-secret", payload)) is True

    def test_configured_rejects_a_wrong_signature(self):
        payload = b'{"ref":"refs/heads/main"}'
        svc = service(github_secret="the-real-secret")
        assert svc.verify_github_signature(
            payload, sign("a-wrong-secret", payload)) is False
    ...
    def test_the_payload_is_what_is_signed(self):
        svc = service(github_secret="the-real-secret")
        signature = sign("the-real-secret", b'{"ref":"main"}')
        assert svc.verify_github_signature(b'{"ref":"OTHER"}', signature) is False
...
class TestGitLab:
    ...
    def test_unconfigured_refuses_any_token(self):
        assert service().verify_gitlab_token("anything") is False
    ...
    def test_configured_rejects_a_wrong_token(self):
        assert service(gitlab_token="t0ken").verify_gitlab_token("other") is False
