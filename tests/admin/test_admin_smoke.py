...
def app(tmp_path_factory):
    ...
    from backend import job_service, main
    # The admin opens the app's ./quantum_jobs.db; here, one of its own —
    # the /jobs routes created it in the repository.
    job_service._job_service = job_service.JobAdminService(
        str(tmp_path_factory.mktemp("admin-jobs") / "quantum_jobs.db"))
    yield main
    if job_service._job_service._executor is not None:
        job_service._job_service._executor.shutdown()
    job_service._job_service = None


@pytest.fixture(scope="module")
def client(app):
    # `with` on purpose: Starlette's TestClient only fires the startup events
    # when used as a context manager — and startup is where the admin runs
    # `init_db()` (create_all) and `seed_db()`.
    #
    # Without it the test depended on a `quantum_admin.db` already on disk. On
    # a developer's machine it exists (left over from a real run), so it
    # passed; on a clean clone — CI, or anyone new — it does not, there are no
    # tables, and EIGHTY-FOUR routes answered 500. Green here, red there, by
    # the machine's state.
...
class TestTheApiIsClosedByDefault:
    ...
    def test_no_page_renders_data_to_an_anonymous_visitor(self, app, client, auth):
        ...
        marker = f"secret{uuid.uuid4().hex[:10]}"
...
class TestNenhumaEscritaSemToken:
    """The gate covered ONLY `/api`, and the admin's real API lives at the root.
    ...
    def test_no_write_route_answers_without_a_token(self, app, client):
        open_ = []
        for route in app.app.routes:
            path = getattr(route, "path", "")
            if path.startswith(("/docs", "/openapi", "/redoc", "/static",
                                "/frontend")):
                continue
            for method in sorted(set(getattr(route, "methods", None) or ())
                                 & set(self.WRITES)):
                if path in ("/auth/login", "/auth/logout"):
                    continue
                if path.startswith(("/webhooks/", "/health")):
                    continue
                url = re.sub(r"\{[^}]+\}", "1", path)
                if client.request(method, url).status_code != 401:
                    open_.append(f"{method} {path}")
        assert open_ == [], f"{len(open_)} writes without a token: {open_[:6]}"

    def test_login_stays_public(self, client):
        # A gate that blocks the login is a locked door without a key.
        assert client.post(
            "/auth/login",
            params={"username": "admin", "password": PASSWORD},
        ).status_code == 200

    def test_the_webhook_reaches_its_own_handler(self, client):
        # A webhook carries no JWT — GitHub calls it. It has its own guard by
        # signature, and the 401 here comes from IT (missing secret), not
        # from the gate.
        response = client.post("/webhooks/github", json={},
                               headers={"X-GitHub-Event": "push"})
        assert response.status_code in (401, 400)

    def test_with_a_token_the_write_goes_through(self, client, auth):
        created = client.post("/projects",
                             json={"name": "proj-portao", "description": ""},
                             headers=auth)
        assert created.status_code == 201, created.text
        client.delete(f"/projects/{created.json()['id']}", headers=auth)
...
class TestUserManagementDoesTheWork:
    ...
    def test_a_user_can_be_created_and_deleted(self, client, auth, cleanup):
        created = client.post(
            "/auth/users",
            json={"username": "maria", "password": "a-really-good-password",
                  "role": "user"},
            headers=auth,
        )
    ...
    def test_the_password_no_longer_has_to_travel_in_the_url(self, client, auth, cleanup):
        ...
        r = client.post("/auth/users",
                        json={"username": "joao", "password": "another-good-password"},
                        headers=auth)
        assert r.status_code == 200
        cleanup.append("joao")

    def test_creating_without_a_password_is_a_400_not_a_crash(self, client, auth):
        r = client.post("/auth/users", json={"username": "no-password"},
                        headers=auth)
...
class TestDatasourcesCanBeEdited:
    """A datasource could be created and deleted, never corrected.

    There was create, delete, start/stop/restart, test, logs and setup — and no
    PUT. A wrong host or password meant deleting and creating again, losing
    the history. The DatasourceUpdate schema already existed; nobody had
    written the route.
    """

    @pytest.fixture
    def datasource(self, client, auth):
        """Creates a project + datasource, and cleans up even if the test fails.

        The first version used a fixed name and only cleaned up AFTER the
        yield: a test that failed halfway left the project behind, and the
        next run died in setup with "already exists". A suite that passes only
        once is not a suite — the same defect fixed in test-query-insert.q,
        made again here.

        A unique name per run, and cleanup in try/finally.
        """
        import uuid

        name = f"proj-ds-test-{uuid.uuid4().hex[:8]}"
        ...
        try:
            created = client.post(
                ...
                json={"name": "db1", "type": "postgres",
                      "connection_type": "direct", "host": "wrong",
                      "port": 5432, "database_name": "app",
                      "username": "u", "password": "old-password"},
                headers=auth)
            assert created.status_code == 201, created.text
            yield created.json()["id"]
        finally:
            if created is not None and created.status_code == 201:
                client.delete(f"/datasources/{created.json()['id']}", headers=auth)
            client.delete(f"/projects/{project_id}", headers=auth)
    ...
    def test_the_password_is_stored_encrypted(self, client, auth, datasource):
        ...
        client.put(f"/datasources/{datasource}",
                   json={"password": "new-password"}, headers=auth)

        stored = crud.get_datasource(SessionLocal(), datasource)
        assert stored.password_encrypted != "new-password", "a plain-text password"
        assert decrypt_or_legacy(stored.password_encrypted) == "new-password"
    ...
    def test_omitting_the_password_keeps_the_one_on_record(self, client, auth,
        ...
        client.put(f"/datasources/{datasource}", json={"host": "other"},
                   headers=auth)
        after = crud.get_datasource(SessionLocal(), datasource).password_encrypted
        assert after == before, "changing the host erased the password"
