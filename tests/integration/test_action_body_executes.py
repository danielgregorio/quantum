...
class TestElseIfBranchesUseTheRightShape:
    def test_the_elseif_branch_fires(self, app, db):
        src = (
            ...
            '<q:if condition="n == 1">'
            '<q:redirect url="/one" />'
            '<q:elseif condition="n == 2"><q:redirect url="/two" /></q:elseif>'
            '<q:else><q:redirect url="/other" /></q:else>'
            '</q:if>'
            '</q:action></q:component>'
        )
        rt = ComponentRuntime()
        action = _action_from(src)
        assert _run(app, rt, action, {"n": "2"})[0] == "/two"
        assert _run(app, rt, action, {"n": "1"})[0] == "/one"
        assert _run(app, rt, action, {"n": "7"})[0] == "/other"
