...
class TestTheFactory:
    ...
    def test_a_missing_config_path_falls_back_instead_of_crashing(self, wsgi):
        app = wsgi.create_app(config_path="/does/not/exist/quantum.config.yaml")
...
class TestTheDocumentedCommandImports:
    ...
    def test_the_target_resolves(self, target, tmp_path):
        import os
        import pathlib
        module_name, attribute = target.split(":")
        # From a temporary folder: importing quantum.runtime.wsgi builds the app
        # with ./quantum.config.yaml, and from the checkout that wrote the
        # repository's ./logs. PYTHONPATH keeps this checkout's quantum.
