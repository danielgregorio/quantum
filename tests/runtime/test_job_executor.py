...
class TestJobQueueService:
    ...
    def test_dispatch_with_priority(self, job_queue):
        job_queue.dispatch("low_priority", priority=1)
        job_queue.dispatch("high_priority", priority=10)
...
class TestComponentIntegration:
    ...
    def test_execute_job_dispatch(self, tmp_path):
        from quantum.core.parser import QuantumParser
        ...
        component = parser.parse(xml)
        # Its own job database (it wrote ./quantum_jobs.db), and a real stop:
        # the worker it starts used to run the job after the test had ended.
        runtime = ComponentRuntime(config={'job_db_path': str(tmp_path / 'jobs.db')})
        try:
