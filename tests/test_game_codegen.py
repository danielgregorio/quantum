...
class TestTilemapVisual:
    ...
    def test_tilemap_visual_sprites(self, codegen):
        from quantum.core.features.game_engine_2d.src.ast_nodes import TilemapNode, TilemapLayerNode
        scene = SceneNode('main')
        tm = TilemapNode('map1')
        tm.src = 'tileset.png'
        tm.tile_width = 32
...
class TestSemanticBehaviors:
    ...
    def test_enemy_patrol_generates_goomba_ai(self, codegen):
        """Patrol enemy should generate setGoombaAI call."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', movement='patrol', move_speed=2, facing='left'
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'game.setGoombaAI("test_entity"' in html
...
class TestBehaviorCollisionHandlers:
    ...
    examples/smw_polished.q wanted a patrol that turns at a wall AND at a
