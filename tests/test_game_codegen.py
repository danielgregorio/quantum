...
from quantum.core.features.game_engine_2d.src.ast_nodes import (
    ...
    SoundNode, InputNode, AnimationNode, TimerNode,
    TweenNode, ParticleNode,
)
...
from quantum.core.features.game_engine_2d.src.ast_nodes import RawCodeNode, TilemapLayerNode, SpawnNode, ClickableNode
...
class TestHudGeneration:
    def test_hud_html(self, codegen):
        from quantum.core.ast_nodes import HTMLNode, TextNode
        scene = SceneNode('main')
        hud = HudNode()
        ...
        src = '''<q:application id="test-game" type="game" engine="2d">
            <qg:behavior name="Collectible">
                ...
                <q:function name="onCollect">
                    <q:set name="score" value="{score + self.value}" />
                </q:function>
            </qg:behavior>
            <qg:prefab name="Coin">
...
class TestTilemapVisual:
    ...
    def test_tilemap_visual_sprites(self, codegen):
        from quantum.core.features.game_engine_2d.src.ast_nodes import TilemapNode
        scene = SceneNode('main')
        tm = TilemapNode('map1')
        tm.src = 'tileset.png'
        tm.tile_width = 32
    ...
    def test_tilemap_collision_still_works(self, codegen):
        ...
        scene.add_child(s)
        html = codegen.generate(scene)
        # This asserted 'isStatic: true' and a 'tilemap-collision' tag —
        # individual Matter.js static bodies, one per solid tile. That
        # implementation was deliberately replaced by an AABB grid ("avoids
        # edge-catching issues and supports slopes natively",
        # game_code_generator.py), and the test was left asserting the old
        # output. It has been red ever since, for a feature that works.
        #
        # The contract now: a collision layer becomes a grid the runtime
        # resolves against, carrying the tile size and the actual tile data.
        assert '_collisionGrids' in html
        assert '_resolveTilemapCollisions' in html
        assert '_collisionGrids.push({ grid: [[1,0],[0,1]], tw: 32, th: 32 })' in html
...
# Helper to build a prefab+instance scene for semantic tests
def _make_semantic_scene(entity_type=None, **sem_attrs):
    """Create a scene with a single prefab instance having semantic attributes."""
    scene = SceneNode('main')
    scene.width = 800
    scene.height = 600

    # Need a player sprite with body for collision system
    ...
    inst = InstanceNode('TestPrefab')
    inst.x = 200
    inst.y = 300
    inst.instance_id = 'test_entity'
    scene.add_child(inst)

    prefab = PrefabNode('TestPrefab')
    if entity_type:
        prefab.entity_type = entity_type
    for attr, val in sem_attrs.items():
        setattr(prefab, attr, val)
    ps = SpriteNode('entity_sprite')
    ps.src = 'entity.png'
    ps.body = 'dynamic'
    prefab.add_child(ps)
...
class TestSemanticBehaviors:
    """Test Phase 2: auto-codegen from prefab semantic attributes."""

    def test_enemy_patrol_generates_patroller_ai(self, codegen):
        """Patrol enemy should generate setPatrolAI call."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', movement='patrol', move_speed=2, facing='left'
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'game.setPatrolAI("test_entity"' in html
        assert '-2' in html  # facing left = negative speed

    def test_enemy_stomp_generates_collision_handler(self, codegen):
        """Stomp enemy should generate collision handler with events."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', defeated_by='stomp', health=1,
            movement='patrol', move_speed=1
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'enemy-stomped' in html
        assert 'enemy-killed' in html
        assert 'game.destroySprite' in html
        assert 'collisionHandlers.push' in html

    def test_enemy_health_2(self, codegen):
        """Enemy with health=2 should set _semHealth = 2."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', defeated_by='stomp', health=2,
            movement='patrol', move_speed=1
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert '_semHealth = 2' in html
        assert '_semHealth <= 0' in html

    def test_item_collectible(self, codegen):
        """Item should generate item-collected event and destroySprite."""
        scene, prefab = _make_semantic_scene(
            entity_type='item', effect='grow', collectible=True
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'item-collected' in html
        assert 'game.destroySprite' in html
        assert 'grow' in html

    def test_block_hit(self, codegen):
        """Block should generate block-hit event and _semHitsLeft."""
        scene, prefab = _make_semantic_scene(
            entity_type='block', hits=3, content='coin'
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'block-hit' in html
        assert '_semHitsLeft' in html
        assert 'coin' in html

    def test_no_semantics_no_extra_code(self, codegen):
        """Plain prefab without semantic attrs should not add semantic code."""
        scene, prefab = _make_semantic_scene()  # no entity_type
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'no semantic behaviors' in html

    def test_hazard_damage(self, codegen):
        """Hazard should emit player-damaged on contact."""
        scene, prefab = _make_semantic_scene(
            entity_type='hazard', contact_damage=2
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'player-damaged' in html
        assert 'damage: 2' in html

    def test_goal_level_complete(self, codegen):
        """Goal should emit level-complete on contact."""
        scene, prefab = _make_semantic_scene(entity_type='goal')
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'level-complete' in html

    def test_custom_defeat_event(self, codegen):
        """Custom on_defeat should use custom event name."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', defeated_by='stomp', health=1,
            on_defeat='boss-defeated', movement='patrol', move_speed=1
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert 'boss-defeated' in html
        # Should NOT have default enemy-killed
        assert 'enemy-killed' not in html

    def test_enemy_reward_kill(self, codegen):
        """Enemy with reward_kill should emit reward event on defeat."""
        scene, prefab = _make_semantic_scene(
            entity_type='enemy', defeated_by='stomp', health=1,
            reward_kill='score:200', movement='patrol', move_speed=1
        )
        html = codegen.generate(scene, prefabs=[prefab])
        assert "game.emit('reward'" in html or 'game.emit("reward"' in html
        assert 'score:200' in html


class TestBehaviorCollisionHandlers:
    """A behavior use can carry more than one collision handler.

    a platformer example wanted a patrol that turns at a wall AND at a
    ledge, and wrote on-collision= twice on one <qg:use>. That is a duplicate
    attribute: the whole file failed to parse, and two game tests had been
    red on it. Repeating the <qg:use> is not the same thing — it would
    instantiate the behavior twice — so the extra handlers are children:
    ...
    def _scene(self, use):
        scene = SceneNode('main')
        sprite = SpriteNode('enemy')
        sprite.src = 'e.png'
        sprite.add_child(use)
        scene.add_child(sprite)
        return scene

    def _patrol(self):
        beh = BehaviorNode('PatrolBehavior')
        beh.add_child(FunctionNode('onWallHit'))
        beh.add_child(FunctionNode('onEdgeDetected'))
        return beh

    def test_the_attribute_form_still_works(self, codegen):
        use = UseNode('PatrolBehavior')
        use.on_collision = 'onWallHit'
        use.collision_tag = 'solid'
        html = codegen.generate(self._scene(use), behaviors=[self._patrol()])
        assert 'b.onWallHit' in html
        assert "'solid'" in html or '"solid"' in html

    def test_two_handlers_both_reach_the_output(self, codegen):
        use = UseNode('PatrolBehavior')
        use.on_collision = 'onWallHit'
        use.collision_tag = 'solid'
        use.extra_collisions = [{'handler': 'onEdgeDetected', 'tag': 'edge'}]
        html = codegen.generate(self._scene(use), behaviors=[self._patrol()])
        assert 'b.onWallHit' in html
        assert 'b.onEdgeDetected' in html

    def test_each_handler_keeps_its_own_tag_filter(self, codegen):
        use = UseNode('PatrolBehavior')
        use.on_collision = 'onWallHit'
        use.collision_tag = 'solid'
        use.extra_collisions = [{'handler': 'onEdgeDetected', 'tag': 'edge'}]
        html = codegen.generate(self._scene(use), behaviors=[self._patrol()])
        edge_block = html.split('b.onEdgeDetected')[0].split('collisionHandlers.push')[-1]
        assert 'edge' in edge_block, "the edge handler would fire on a wall"

    def test_the_behavior_is_instantiated_once(self, codegen):
        use = UseNode('PatrolBehavior')
        use.on_collision = 'onWallHit'
        use.extra_collisions = [{'handler': 'onEdgeDetected', 'tag': 'edge'}]
        html = codegen.generate(self._scene(use), behaviors=[self._patrol()])
        assert html.count('new PatrolBehavior(') == 1

    def test_the_parser_reads_the_children(self):
        from quantum.core.parser import QuantumParser
        app = QuantumParser().parse("""<q:application id="G" type="game"
                xmlns:q="https://quantum.lang/ns"
                xmlns:qg="https://quantum.lang/game">
          <qg:scene name="main">
            <qg:sprite id="enemy" src="e.png" tag="enemy">
              <qg:use behavior="PatrolBehavior">
                <qg:on-collision handler="onWallHit" tag="solid" />
                <qg:on-collision handler="onEdgeDetected" tag="edge" />
              </qg:use>
        ...
        sprite = app.scenes[0].children[0]
        use = sprite.children[0]
        assert (use.on_collision, use.collision_tag) == ('onWallHit', 'solid')
        assert use.extra_collisions == [
            {'handler': 'onEdgeDetected', 'tag': 'edge'}]
