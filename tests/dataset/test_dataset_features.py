...
_FEATURES_DIR = Path(__file__).resolve().parent.parent.parent / "quantum" / "core" / "features"
...
def test_dataset_example(feature, polarity, example, json_path):
    ...
    if feature == "native_testing":
        _test_native_testing(polarity, example)       # a *.test.q, not a page
        return
...
def _test_native_testing(polarity: str, example: dict):
    """TEST-4: a positive example parses as a test file; a negative one is a parse error."""
    from quantum.core.features.native_testing.src import TestParseError, parse_test_source

    code = example["code"]
    if polarity == "positive":
        assert parse_test_source(code).tests
    else:
        with pytest.raises(TestParseError, match=example["error"]):
            parse_test_source(code)
