...
class TestHtmlAdapterDesktopMode:
    ...
    def test_input_twoway_binding(self, parser):
        ...
        Este teste afirmava `__quantumCall('__set_state'` — o nome com o bug.
        `def __set_state` dentro de uma classe vira `_QuantumAPI__set_state`
        pelo name mangling do Python, e o pywebview so expoe nomes publicos,
        entao a chamada caia num console.warn silencioso. O teste se chama
        "two-way binding" e fixava no lugar exatamente a chamada que nunca
        escreveu de volta no Python.
        """
        adapter = UIHtmlAdapter(desktop_mode=True)
        app = parse_ui(parser, '<ui:window><ui:input bind="username" /></ui:window>')
        html = adapter.generate(app.ui_windows, app.ui_children, 'Test')

        assert "__quantumCall('set_state'" in html
        assert "name:'username'" in html

    def test_the_bound_call_reaches_a_method_that_exists(self, parser):
        """O nome chamado pelo JS tem de existir no objeto que o pywebview expoe."""
        import re
        ...
        chamados = set(re.findall(r"__quantumCall\('([^']+)'", html))
        assert chamados, "nenhuma chamada para o Python foi gerada"

        code = UIDesktopAdapter().generate(app.ui_windows, app.ui_children, 'Test')
        # O pywebview so expoe atributos publicos do objeto api.
        definidos = set(re.findall(r"^    def ([a-zA-Z]\w*)\(", code, re.M))

        faltando = [n for n in chamados if n not in definidos]
        assert faltando == [], (
            f"o JS chama {faltando} e o QuantumAPI nao define: a chamada cai "
            f"num console.warn silencioso")
...
class TestDesktopAdapter:
    ...
    def test_generates_quantum_api_class(self, parser):
        ...
        # Publico de proposito: `__set_state` sofria name mangling e o
        # pywebview nunca o encontrava.
        assert 'def set_state(self, args):' in code
