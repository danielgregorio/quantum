...
class MockExecutionContext:
    ...
    def create_child_context(self) -> 'MockExecutionContext':
        """Create a child context (mirrors the real ExecutionContext).

        The real one shares session/application/request scopes with its
        parent and layers local scope on top; for executor tests, seeding a
        copy of the visible variables is equivalent.
        """
        child = MockExecutionContext(self._variables)
        child.parent = self
        return child
...
def if_executor():
    ...
    from quantum.runtime.executors.control_flow.if_executor import IfExecutor
    runtime = MockRuntime()
...
def loop_executor():
    ...
    from quantum.runtime.executors.control_flow.loop_executor import LoopExecutor
    runtime = MockRuntime()
