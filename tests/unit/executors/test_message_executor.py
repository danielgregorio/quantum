...
from quantum.runtime.executors.messaging.message_executor import MessageExecutor, SubscribeExecutor
...
class MockMessagingService:
    ...
    def subscribe(self, name: str, topic: str = None, topics: str = None,
                  queue: str = None, handler=None, ack: str = 'auto',
                  prefetch: int = 1) -> str:
        """Mock subscribe matching the real MessageQueueService signature:
        a single `handler` callable and `topics` as a comma-separated string."""
        self.last_subscribe = {
            'name': name,
            'topic': topic,
            'topics': topics,
...
class MockMessagingRuntime(MockRuntime):
    ...
    def __init__(self, variables: Dict[str, Any] = None):
        ...
        self._services.message_queue = self._messaging_service
...
class TestSubscribeTopic:
    ...
    def test_subscribe_multiple_topics(self):
        ...
        # subscribe() takes topics as a comma-separated string
...
class TestSubscribeHandlers:
    ...
    def test_handlers_passed(self):
        ...
        # subscribe() takes one handler callable, not a handlers dict
        handler = runtime._messaging_service.last_subscribe["handler"]
...
class TestSubscribeResultStorage:
    ...
    def test_stores_subscription_reference(self):
        ...
        # subscribe() returns the subscription id string
        assert runtime.execution_context.get_variable("mySub") == "sub-mySub"
