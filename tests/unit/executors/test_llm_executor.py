...
class MockLLMRuntime(MockRuntime):
    ...
    def __init__(self, variables: Dict[str, Any] = None):
        ...
        # IA-1: the real resolution, with llm.model configured
        from quantum.runtime.service_container import ServiceContainer
        self._services.llm_model = ServiceContainer({'llm': {'model': 'phi3'}}).llm_model
...
class TestCompletionMode:
    ...
    def test_default_model(self):
        # IA-1: no model= — the configured llm.model, not a literal
        runtime = MockLLMRuntime()
        executor = LLMExecutor(runtime)
...
class TestErrorHandling:
    ...
    def test_error_stores_failure_result(self):
        ...
        assert "LLM error" in error_result["error"]["message"]      # IA-5: same shape as q:invoke
