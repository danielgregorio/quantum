...
from quantum.runtime.executors.control_flow.set_executor import SetExecutor
...
class TestErrorHandling:
    ...
    def test_increment_non_numeric_raises_instead_of_inventing_a_number(self):
        """Incrementing a genuinely non-numeric value is an error.

        This test used to assert the opposite — that "hello" incremented by 5
        becomes 5 — with the comment "the executor catches the error and
        returns step value as fallback". That was describing a bug, not a
        feature: _execute_increment raised ExecutorError for a non-numeric
        value and then caught its own exception one line later, discarding the
        string and inventing a number.

        The same swallow corrupted the ordinary case, which is what made it
        worth fixing rather than documenting: a stock of "7" decremented by 5
        became -5, because a .q attribute is a string unless the author wrote
        type="number". Numeric-looking strings are coerced now, and only a
        genuinely non-numeric value raises.
        """
        runtime = MockRuntime({"text": "hello"})
        executor = SetExecutor(runtime)
        ...
        with pytest.raises(ExecutorError, match="non-numeric"):
            executor.execute(node, runtime.execution_context)

        # And the value is left alone rather than replaced with a guess.
        assert runtime.execution_context.get_variable("text") == "hello"
