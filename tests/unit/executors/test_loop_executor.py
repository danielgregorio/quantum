...
class TestQueryLoop:
    ...
    def test_query_loop_empty_results(self):
        """A query with no rows loops zero times (LOOP-4).

        The executor used `if not query_data:`, which is True for [], and
        raised "not found" for an empty result."""
        ...
        assert not executor.execute(node, runtime.execution_context)
