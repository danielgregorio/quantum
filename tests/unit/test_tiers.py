...
class TestClassification:
    ...
    def test_ai(self):
        assert tiers.tier_of("agent") == "ai"

    def test_files_and_mail_are_core(self):
        # 0.21: FILE-1/2 and MAIL-1/2 in the SPEC, proven by projects/helpdesk.
        assert tiers.tier_of("file") == tiers.tier_of("mail") == "core"

    def test_transaction_is_core(self):
        # DB-4, used by projects/blog.
        assert tiers.tier_of("transaction") == "core"

    def test_team_is_experimental(self):
        # No SPEC rule and no proving app yet.
        assert tiers.tier_of("team") == "experimental"
    ...
    def test_core_and_ai_do_not_overlap_experimental(self):
        assert not (tiers.SUPPORTED & tiers.EXPERIMENTAL)


class TestApplicationTypes:
    """D1/D2: the game engine stays in the repo as LABORATORY, not parked."""

    def test_game_is_laboratory(self):
        assert tiers.app_tier_of("game") == "laboratory"

    def test_alternative_targets_are_experimental(self):
        for app_type in ("terminal", "ui"):
    ...
    def test_declared_web_applications_were_removed(self):
        ...
        # APP-2 (0.22): type="testing", the qtest: engine, joined them.
        assert {"html", "api", "microservices", "testing"} == tiers.REMOVED_APP_TYPES
    ...
    def test_running_a_game_warns(self, tmp_path):
        ...
        assert "LABORATORY" in out.stdout + out.stderr
...
class TestWarning:
    ...
    def test_warns_once_per_tag(self, caplog):
        with caplog.at_level(logging.WARNING, logger="quantum.tiers"):
            for _ in range(1000):
                tiers.warn_if_unsupported("dump")
        assert len(caplog.records) == 1
