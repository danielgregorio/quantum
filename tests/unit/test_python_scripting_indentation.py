...
def run(source, **params):
    # python_scripting is now OFF by default. These tests run because this
    # repository's quantum.config.yaml turns it on explicitly, and
    # ComponentRuntime() without a config reads that file. If it ever stops
    # turning it on, these tests fail with the refusal message — which is the
    # right behaviour, not a broken test.
...
class TestTheBridgeLogs:
    ...
    def test_reading_a_variable_still_works(self):
        bridge = QuantumBridge(None, {"name": "quantum"})
        assert bridge.name == "quantum"
