...
class TestPolicyDefault:
    def test_this_repository_enables_it_in_its_own_config(self):
        # This test was called `test_enabled_by_default` and asserted that
        # the DEFAULT was on. After the inversion it would have kept passing —
        # not because of the default, but because this repository's
        # quantum.config.yaml turns it on explicitly (the examples use
        # q:python). A test that passes for the wrong reason is worse than one
        # that fails. The real default is in TestTheDefaultIsOff.
        runtime = ComponentRuntime()
        assert runtime.services.python_scripting_enabled is True, (
            "this repo's quantum.config.yaml should turn scripting on")
...
class TestTheDefaultIsOff:
    """The default was ON — the last fail-open in the configuration.

    Running a `.q` you did not write meant running its Python, with no
    sandbox, in your process. A default has to be safe for whoever has not
    read the documentation; whoever uses q:python has read it, because they
    ...
    def test_the_config_wins_over_the_environment(self, monkeypatch):
        # A versioned file that says `false` must not be overridden by an
        # environment variable forgotten in the shell.
        from quantum.runtime.service_container import ServiceContainer
        monkeypatch.setenv('QUANTUM_PYTHON_SCRIPTING', '1')
        services = ServiceContainer(config={'security': {'python_scripting': False}})
        assert services.python_scripting_enabled is False
