...
class TestTheGuardsCoveredOnlyHalfTheCases:
    """The cost guards only applied to integer ** integer.

    `_guard_pow` started with

        if not isinstance(left, int) or not isinstance(right, int):
            return

    so ANY mix with a float went straight through to the operator:
    `2.0 ** 100000000` raised OverflowError, which is not an ExpressionError
    and so went up through the request handler as an unexplained 500 —
    exactly what the guard exists to prevent.

    And nothing limited the DEPTH of the tree. `_eval` recurses, and Python's
    limit (~1000 frames) is shared with Flask, the executor and the renderer,
    so `{---...1}` with a thousand signs raised RecursionError: again not an
    ExpressionError, and again a 500 — with the interpreter scraping the
    stack limit on the way.
    """
    ...
    def test_a_power_with_a_float_is_refused_too(self, ev, expr):
        with pytest.raises(ExpressionError, match="too large"):
            ev.evaluate(expr, {})

    def test_a_float_that_overflows_the_exponent_becomes_ExpressionError(self):
    ...
    def test_cheap_powers_with_a_float_still_pass(
            self, ev, expr, expected):
        assert ev.evaluate(expr, {}) == expected

    def test_a_too_deep_expression_is_refused(self, ev):
        with pytest.raises(ExpressionError, match="nested more than"):
            ev.evaluate('-' * 500 + '1', {})

    def test_the_refusal_comes_before_any_recursion(self, ev):
        """It must not be RecursionError: it has to be ExpressionError."""
        try:
            ev.evaluate('-' * 500 + '1', {})
        except ExpressionError:
            pass
        except RecursionError:
            pytest.fail("RecursionError escaped as RecursionError")

    def test_a_normal_depth_still_passes(self, ev):
        assert ev.evaluate('-' * 20 + '1', {}) == 1     # even: back to positive
