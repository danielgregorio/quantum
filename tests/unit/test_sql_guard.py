...
class TestNoInjectionPathExists:
    ...
    def test_a_bound_parameter_is_data_not_syntax(self, tmp_path):
        ...
        con.execute("CREATE TABLE u (id INTEGER PRIMARY KEY, name TEXT)")
        ...
        r = svc.execute_query(
            "t", "SELECT name FROM u WHERE name = :n", {"n": "x' OR '1'='1"}
        )
        assert r.data == [], "the payload was parsed as syntax, not compared as data"

        # And the ordinary case still works.
        r2 = svc.execute_query("t", "SELECT name FROM u WHERE name = :n", {"n": "ana"})
        assert [row["name"] for row in r2.data] == ["ana"]
