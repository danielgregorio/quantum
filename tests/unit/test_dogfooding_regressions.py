...
class TestAssetUrlsAreRootRelative:
    ...
    def test_stylesheet_link_is_root_relative(self):
        import inspect
        from quantum.runtime import web_html     # the asset extraction (was in web_server)
        source = inspect.getsource(web_html)
...
class TestServerStartReportsFailure:
    ...
    def test_reloader_child_skips_the_port_check(self, server, monkeypatch):
        ...
        # As a reloader child it would write .quantum.pid in the cwd (the
        # repository) with os.getppid() -- the pytest process -- and, being the
        # child, never remove it. A later `quantum stop` in the repository
        # (test_cli_contract) then ran `taskkill /F /T` on pytest: the whole
        # run died with no report, in some test orders.
