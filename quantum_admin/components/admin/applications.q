<q:component name="AdminApplications" require_auth="true" require_role="admin" login_url="/admin/login">
  <!--
    Applications. The logic is in quantum_admin/services/projects.py
    (admin.projects.*); this screen only calls the services and shows the result.
    The data lives in the admin database — before, in settings/projects.yaml.
  -->
  ...
  <q:action name="createProject" method="POST">
    ...
    <q:invoke name="created" service="admin.projects.create" onerror="continue">
    ...
    <q:if condition="created_result.success">
      <q:redirect url="/admin/applications" flash="Application {created.name} created" />
    </q:if>
  ...
  <q:action name="deleteProject" method="POST">
    ...
    <q:invoke name="removed" service="admin.projects.delete">
  ...
  <q:action name="syncProjects" method="POST">
    <q:invoke name="synced" service="admin.projects.sync" />
    <q:redirect url="/admin/applications" flash="{len(synced.created)} new application(s) found on disk" />
  </q:action>

  <q:action name="importYaml" method="POST">
    <q:invoke name="imported" service="admin.import.run" />
    <q:redirect url="/admin/applications" flash="{len(imported.projects)} application(s) imported; database backup at {imported.backup}" />
  </q:action>

  <!-- query.search may not exist: the bare reference is '' (EXPR-3) -->
  <q:set name="search_term" value="{query.search}" />
  <q:invoke name="apps" service="admin.projects.list">
    <q:param name="search" value="{search_term}" />
  </q:invoke>
  <q:invoke name="totals" service="admin.projects.summary" />
  <q:invoke name="pending" service="admin.import.pending" />

  <AdminShell title="Applications" active="applications" flash="{flash}" flashType="{flashType}">

    <q:if condition="len(pending.projects) &gt; 0">
      <div class="qa-card qa-mb-6">
        <div class="qa-card-body">
          <p class="qa-text-sm">
            {len(pending.projects)} application(s) exist only in the old settings/projects.yaml:
            <span class="qa-font-mono">{join(pending.projects)}</span>.
    ...
    <div class="qa-grid qa-grid-4 qa-mb-6">
      <div class="qa-stat-card"><div class="qa-stat-label">Total Apps</div><div class="qa-stat-value">{totals.total}</div></div>
      <div class="qa-stat-card"><div class="qa-stat-label">Active</div><div class="qa-stat-value">{totals.active}</div></div>
      <div class="qa-stat-card"><div class="qa-stat-label">Running</div><div class="qa-stat-value">{totals.running}</div></div>
      <div class="qa-stat-card"><div class="qa-stat-label">Connectors</div><div class="qa-stat-value">{totals.connectors}</div></div>
    </div>

    <div style="display: flex; gap: 12px; margin-bottom: 24px; flex-wrap: wrap;">
      <form method="GET" action="/admin/applications" style="flex: 1; min-width: 200px;">
        <div class="qa-search-bar">
          <input type="text" name="search" placeholder="Search applications..." value="{search_term}" aria-label="Search applications" />
        </div>
      </form>
      <form method="POST" action="/admin/applications">
    ...
    <q:if condition="len(apps) == 0">
      <div class="qa-card qa-mb-6">
        <div class="qa-empty">
          <div class="qa-empty-title">{'No Results' if search_term else 'No Applications Found'}</div>
          <div class="qa-empty-text">{'No applications match your search.' if search_term else 'Use Sync from Disk to discover existing projects, or create one above.'}</div>
        </div>
      </div>
    </q:if>
