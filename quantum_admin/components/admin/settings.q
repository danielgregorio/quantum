<q:component name="AdminSettings" require_auth="true" require_role="admin" login_url="/admin/login">
  <!--
    The root's quantum.config.yaml (admin.settings.*) and the environment
    (admin.system.info). Saving changes only the values' lines and keeps the
    comments; the previous version rewrote the whole file with yaml.dump.
    The form comes filled in with what is in the file — before it came with
    debug off, log INFO and TTL 300, and saving changed those values without
  ...
  <q:action name="saveSettings" method="POST">
    ...
    <q:invoke name="saved" service="admin.settings.save_server_config" onerror="continue">
  ...
  <q:invoke name="system" service="admin.system.info" />

  <AdminShell title="Settings" active="settings" flash="{flash}" flashType="{flashType}">

    <q:if condition="not cfg.found">
      <div class="qa-flash qa-flash-error" role="alert">There is no quantum.config.yaml in {system.root}.</div>
    </q:if>
              ...
              <div>
                <div class="qa-form-group qa-mb-4">
                  ...
                  <select id="log_level" name="log_level" class="qa-input">
                    <q:loop type="array" var="level" items="{['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']}">
                      <q:if condition="upper(cfg.logging.level) == level">
                        <option value="{level}" selected="selected">{level}</option>
                      <q:else><option value="{level}">{level}</option></q:else>
                      </q:if>
                    </q:loop>
                  </select>
                </div>
    ...
    <div class="qa-card">
      ...
      <div class="qa-card-body">
        <div class="qa-grid qa-grid-2">
          <div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Quantum</span><span class="qa-font-mono">{system.quantum_version or 'not installed (running from a clone)'}</span></div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Python</span><span class="qa-font-mono">{system.python_version}</span></div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Platform</span><span class="qa-font-mono">{system.platform}</span></div>
          </div>
          <div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Project root</span><span class="qa-font-mono">{system.root}</span></div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Tags with a parser</span><span class="qa-font-mono">{system.parser_tags}</span></div>
            <div class="qa-flex qa-items-center qa-justify-between"><span class="qa-text-sm qa-text-muted">Executors</span><span class="qa-font-mono">{system.executors}</span></div>
          </div>
        </div>
      </div>
    </div>
