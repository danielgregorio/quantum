<q:component name="AdminAppDetail" require_auth="true" require_role="admin" login_url="/admin/login">
  <!--
    One application: /admin/app/<name>. Each action finds the project by the `name`
    of the URL (ROUTE-2) — the previous version trusted project_id and app_name
    coming from hidden form fields. The services are in
    quantum_admin/services/apps.py, projects.py and connectors.py.

    Removed from this screen, for never having worked: generating and running
    the project's component tests (the generator imported `core.parser`, which does
    not exist since the move from src/ to quantum/) and the "external changes"
    warning on the config. Component tests are on the Components screen.
  -->
  ...
  <q:action name="updateProject" method="POST">
    ...
    <q:invoke name="saved" service="admin.projects.update" onerror="continue">
    ...
    <q:if condition="saved_result.success">
      <q:redirect url="/admin/app/{saved.name}" flash="Application updated" />
    </q:if>
    <q:flash type="error" message="{saved_result.error.message}" />
  ...
  <q:action name="createProjectConnector" method="POST">
    ...
    <q:if condition="created_result.success">
      <q:redirect url="/admin/app/{name}?tab=connectors" flash="Connector {created.name} created" />
    </q:if>
  ...
  <q:action name="detachConnector" method="POST">
    <q:param name="connector_id" type="string" required="true" />
    <q:invoke name="detached" service="admin.connectors.detach" onerror="continue">
      <q:param name="connector_id" value="{connector_id}" />
    </q:invoke>
    <q:if condition="detached_result.success">
      <q:redirect url="/admin/app/{name}?tab=connectors" flash="Connector {detached.name} is now public" />
    </q:if>
    <q:flash type="error" message="{detached_result.error.message}" />
    <q:redirect url="/admin/app/{name}?tab=connectors" />
  </q:action>
  ...
  <q:action name="saveProjectConfig" method="POST">
    ...
    <q:invoke name="app" service="admin.projects.by_name"><q:param name="name" value="{name}" /></q:invoke>
    <q:invoke name="written" service="admin.projects.save_config" onerror="continue">
      <q:param name="project_id" value="{app.id}" type="integer" />
      <q:param name="config_yaml" value="{form.config_yaml}" />
    </q:invoke>
    <q:if condition="written_result.success">
      <q:redirect url="/admin/app/{name}?tab=config" flash="Configuration saved" />
    </q:if>
    <q:flash type="error" message="{written_result.error.message}" />
  ...
  <q:action name="createEnvironment" method="POST">
    ...
    <q:invoke name="app" service="admin.projects.by_name"><q:param name="name" value="{name}" /></q:invoke>
    <q:invoke name="environment" service="admin.environments.create" onerror="continue">
      <q:param name="project_id" value="{app.id}" type="integer" />
      <q:param name="name" value="{env_name}" />
    </q:invoke>
    <q:if condition="environment_result.success">
      <q:redirect url="/admin/app/{name}?tab=environments" flash="Environment {environment.name} created" />
    </q:if>
    <q:flash type="error" message="{environment_result.error.message}" />
    <q:redirect url="/admin/app/{name}?tab=environments" />
  </q:action>

  <q:action name="createDefaultEnvironments" method="POST">
    <q:invoke name="app" service="admin.projects.by_name"><q:param name="name" value="{name}" /></q:invoke>
    <q:invoke name="defaults" service="admin.environments.create_defaults">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:redirect url="/admin/app/{name}?tab=environments" flash="{len(defaults)} environment(s) created" />
  </q:action>

  <q:action name="updateEnvironment" method="POST">
    <q:param name="environment_id" type="integer" required="true" />
    <q:invoke name="environment" service="admin.environments.update" onerror="continue">
    ...
    <q:if condition="environment_result.success">
      <q:redirect url="/admin/app/{name}?tab=environments" flash="Environment {environment.name} saved" />
    </q:if>
    <q:flash type="error" message="{environment_result.error.message}" />
    <q:redirect url="/admin/app/{name}?tab=environments" />
  </q:action>
  ...
  <q:action name="deleteEnvironment" method="POST">
    <q:param name="environment_id" type="integer" required="true" />
    <q:invoke name="removed" service="admin.environments.delete" onerror="continue">
      <q:param name="environment_id" value="{environment_id}" type="integer" />
    </q:invoke>
  ...
  <q:action name="startServer" method="POST">
    <q:invoke name="app" service="admin.projects.by_name"><q:param name="name" value="{name}" /></q:invoke>
    <q:invoke name="started" service="admin.servers.start" onerror="continue">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:if condition="started_result.success">
      <q:redirect url="/admin/app/{name}?tab=runtime" flash="Server started" />
    </q:if>
    <q:flash type="error" message="{started_result.error.message}" />
    <q:redirect url="/admin/app/{name}?tab=runtime" />
  </q:action>

  <q:action name="stopServer" method="POST">
    <q:invoke name="app" service="admin.projects.by_name"><q:param name="name" value="{name}" /></q:invoke>
    <q:invoke name="stopped" service="admin.servers.stop" onerror="continue">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:if condition="stopped_result.success">
      <q:redirect url="/admin/app/{name}?tab=runtime" flash="Server stopped" />
    </q:if>
    <q:flash type="error" message="{stopped_result.error.message}" />
    <q:redirect url="/admin/app/{name}?tab=runtime" />
  </q:action>

  <!-- ===================================================== page data -->
  <q:set name="tab" value="{query.tab}" default="general" />
  <q:set name="tabs" type="array" value='[{"id": "general", "label": "General"}, {"id": "connectors", "label": "Connectors"}, {"id": "components", "label": "Components"}, {"id": "config", "label": "Config"}, {"id": "environments", "label": "Environments"}, {"id": "runtime", "label": "Runtime"}]' />
  ...
  <q:if condition="app_result.success">
    <q:invoke name="files" service="admin.projects.files">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:invoke name="connectors" service="admin.connectors.list">
      <q:param name="application_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:invoke name="providers" service="admin.connectors.providers" />
    <q:invoke name="config" service="admin.projects.config">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <q:invoke name="environments" service="admin.environments.list">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
    <!-- without quantum.config.yaml the status is an error: the Runtime tab says why -->
    <q:invoke name="server" service="admin.servers.status" onerror="continue">
      <q:param name="project_id" value="{app.id}" type="integer" />
    </q:invoke>
  ...
  <AdminShell title="{app.name if app_result.success else 'Application'}" active="applications" flash="{flash}" flashType="{flashType}">
    ...
    <q:if condition="app_result.success">
      <div class="qa-flex qa-items-center qa-mb-4" style="gap: 12px;">
        ...
        <span class="{'qa-process-running' if server_result.success and server.running else 'qa-process-stopped'}">{'Running' if server_result.success and server.running else 'Stopped'}</span>
      </div>

      <q:loop type="array" var="t" items="{tabs}">
        <q:if condition="t.id == tab">
      ...
      <div class="qa-tabs-content">
        ...
        <div class="qa-panel-general">
          <div class="qa-grid qa-grid-4 qa-mb-6">
            <div class="qa-stat-card"><div class="qa-stat-label">Components</div><div class="qa-stat-value">{len(files.components)}</div></div>
            <div class="qa-stat-card"><div class="qa-stat-label">Size on Disk</div><div class="qa-stat-value">{files.disk_size}</div></div>
            <div class="qa-stat-card"><div class="qa-stat-label">Environments</div><div class="qa-stat-value">{len(environments)}</div></div>
            <div class="qa-stat-card"><div class="qa-stat-label">Static Files</div><div class="qa-stat-value">{files.static_files}</div></div>
          </div>
                ...
                <div class="qa-form-row qa-mb-4">
                  ...
                  <div class="qa-form-group">
                    ...
                    <select id="status" name="status" class="qa-input qa-select">
                      <q:loop type="array" var="state" items="{['active', 'archived', 'error']}">
                        <q:if condition="state == app.status">
                          <option value="{state}" selected="selected">{state}</option>
                        <q:else><option value="{state}">{state}</option></q:else>
                        </q:if>
                      </q:loop>
                    </select>
                  </div>
        ...
        <div class="qa-panel-connectors">
          <div class="qa-card qa-mb-6">
            ...
            <div class="qa-card-body qa-p-0" style="overflow-x: auto;">
              <q:if condition="len(connectors) &gt; 0">
                ...
                <div class="qa-form-row qa-mb-4">
                  <div class="qa-form-group">
                    ...
                    <select id="provider" name="provider" class="qa-input qa-select" required="">
                      ...
                      <q:loop type="array" var="p" items="{providers}">
          ...
          <div class="qa-card">
            <div class="qa-card-header">
              <div>
                ...
                <div class="qa-card-subtitle">{len(files.components)} .q files in {app.source_path}/components</div>
              </div>
            </div>
            <div class="qa-card-body qa-p-0" style="overflow-x: auto;">
              <q:if condition="len(files.components) &gt; 0">
                <table class="qa-table">
                  <thead><tr><th>Route</th><th>Component</th><th>File</th><th>Lines</th><th>Tags</th></tr></thead>
                  <tbody>
                    <q:loop type="array" var="comp" items="{files.components}">
                      <tr>
        ...
        <div class="qa-panel-environments">
          <div class="qa-card qa-mb-6">
            ...
            <div class="qa-card-body">
              ...
              <q:if condition="len(environments) == 0">
        ...
        <div class="qa-panel-runtime">
          <div class="qa-card qa-mb-6">
            ...
            <div class="qa-card-body">
              <q:if condition="not server_result.success">
                <div class="qa-text-sm qa-text-muted">{server_result.error.message}</div>
              </q:if>
              <q:if condition="server_result.success">
                ...
                <q:if condition="server.running">
                  <div class="qa-flex qa-items-center qa-justify-between qa-mb-4"><span class="qa-text-sm qa-text-muted">PID</span><span class="qa-font-mono qa-text-sm">{join(server.pids, ', ')}</span></div>
