<q:component name="AdminJobs" require_auth="true" require_role="admin" login_url="/admin/login">
  <!-- The q:job queue in quantum_jobs.db, read-only (admin.jobs.list). -->
  <q:import component="AdminShell" from="admin/_layout" />

  <q:invoke name="queue" service="admin.jobs.list" />
  ...
  <AdminShell title="Jobs" active="jobs" flash="{flash}" flashType="{flashType}">
    ...
    <div class="qa-grid qa-grid-4 qa-mb-6">
      <div class="qa-stat-card">
        ...
        <div class="qa-stat-value" style="color: var(--q-warning);">{queue.counts.pending}</div>
        <div class="qa-stat-change"><span class="qa-text-muted">waiting in queue</span></div>
      </div>
      <div class="qa-stat-card">
        <div class="qa-stat-label">Running</div>
        <div class="qa-stat-value" style="color: var(--q-info);">{queue.counts.running}</div>
        <div class="qa-stat-change"><span class="qa-text-muted">currently executing</span></div>
      </div>
      <div class="qa-stat-card">
        <div class="qa-stat-label">Completed</div>
        <div class="qa-stat-value" style="color: var(--q-success);">{queue.counts.completed}</div>
        <div class="qa-stat-change"><span class="qa-text-muted">finished successfully</span></div>
      </div>
      <div class="qa-stat-card">
        <div class="qa-stat-label">Failed</div>
        <div class="qa-stat-value" style="color: var(--q-danger);">{queue.counts.failed}</div>
        <div class="qa-stat-change"><span class="qa-text-muted">out of attempts</span></div>
      </div>
    </div>

    <q:if condition="queue.error">
      <div class="qa-flash qa-flash-error" role="alert">quantum_jobs.db could not be read: {queue.error}</div>
    </q:if>

    <q:if condition="queue.found">
      <div class="qa-card">
        <div class="qa-card-header">
          <div>
            <div class="qa-card-title">Recent Jobs</div>
            <div class="qa-card-subtitle">{queue.total} jobs in total, newest {len(queue.jobs)} shown</div>
          </div>
        </div>
        <div class="qa-card-body qa-p-0" style="overflow-x: auto;">
          <table class="qa-table">
            ...
            <tbody>
              <q:loop type="array" var="job" items="{queue.jobs}">
                <tr>
    ...
    <q:if condition="not queue.found">
      <div class="qa-card">
        <div class="qa-empty">
