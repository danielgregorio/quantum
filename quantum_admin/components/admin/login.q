<q:component name="AdminLogin">
  <!--
    Admin login. Uses the same credentials as the FastAPI (admin.auth.login:
    ADMIN_PASSWORD, or the password generated and printed at startup), with a lock
    after repeated wrong attempts. Stores in the session what AUTH-1 requires.
  -->
  <q:action name="signIn" method="POST">
    ...
    <q:if condition="auth.ok">
      ...
      <q:set name="session.sessionExpiry" value="{dateAdd('h', auth.expires_in_hours)}" type="string" />
