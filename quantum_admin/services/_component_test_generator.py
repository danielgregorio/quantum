...
(Copied from components/admin/_test_generator.py, which generated imports from
the old src/ layout — `from core.parser import ...` — so every generated test
failed with ModuleNotFoundError. The imports now use the quantum package.)
...
class ComponentTestGenerator:
    ...
    def generate(self):
        ...
        lines.append(f'        from quantum.core.parser import QuantumParser')
        ...
        for qname in self.queries:
            ...
            lines.append(f'        from quantum.core.parser import QuantumParser')
            lines.append(f'        from quantum.core.ast_nodes import QueryNode')
            lines.append(f'        parser = QuantumParser()')
            lines.append(f'        with open(COMPONENT_PATH, "r", encoding="utf-8") as f:')
            lines.append(f'            source = f.read()')
            lines.append(f'        ast = parser.parse(source)')
