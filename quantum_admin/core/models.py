...
def _utcnow() -> datetime:
    """Naive UTC now, as the columns store it; datetime.utcnow() is deprecated."""
...
# No longer used: the cloud deploy integrations were removed with the admin's
# deploy feature. The table stays so an existing admin database keeps loading.
class CloudIntegration(Base):
    ...
    def to_dict(self, include_credentials: bool = False):
        ...
        if include_credentials:
            # The column is encrypted now (it was plain JSON under an
            # "_encrypted" name). decrypt_or_legacy also reads rows written
            # before that, so existing configuration is not lost.
            try:
                from .secret_manager import decrypt_or_legacy
            except ImportError:
                from secret_manager import decrypt_or_legacy
            result["credentials"] = (
                json.loads(decrypt_or_legacy(self.credentials_encrypted))
                if self.credentials_encrypted else {}
            )
        return result
