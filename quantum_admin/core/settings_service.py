...
class SecuritySettings:
    # There was a `secret_key: str = "change-me-in-production"` here. Nothing
    # in the admin ever read it — the JWT signing key comes from
    # JWT_SECRET_KEY in auth_service — but _create_default_global_settings
    # wrote it into the settings file on disk, where it looked like the place
    # to set a secret. A decoy credential is worse than none: someone sets it,
    # believes the admin is configured, and it signs nothing. Removed; set
    # JWT_SECRET_KEY instead.
    jwt_algorithm: str = "HS256"
    jwt_expiry_hours: int = 24
    # Loopback by default. "*" here plus allow_credentials let any website
    # call the admin API.
...
class GlobalSettings:
    ...
    # No `deployment` section: the admin's deploy feature was removed. A
    # global.yaml that still has one loads as before; nothing reads it.
