    ...
    def update_connector(self, connector_id: str, data: Dict[str, Any]) -> Optional[Connector]:
        ...
        for i, conn in enumerate(connectors):
            if conn.id == connector_id:
                ...
                conn.updated_at = datetime.now(timezone.utc).replace(tzinfo=None).isoformat()
    ...
    def _update_status(self, connector_id: str, status: ConnectorStatus):
        ...
        for conn in connectors:
            if conn.id == connector_id:
                ...
                conn.last_tested = datetime.now(timezone.utc).replace(tzinfo=None).isoformat()
                break
        self._save_connectors(connectors)
            ...
            for position, item in enumerate(data):
                try:
                    ...
                    raise ConnectorFileError(
                        f"{CONNECTORS_FILE}: entry {position} ({name!r}) could not be loaded: {e}. "
