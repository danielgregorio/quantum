...
async def _lifespan(_app):
    # @app.on_event is deprecated in FastAPI; the same two functions run here.
    startup_event()
    yield
    shutdown_event()
...
#
# This was allow_origins=["*"] with allow_credentials=True — a comment saying
# "in production, specify your frontend URL" and no way to do it. Any page on
# the internet could call the admin API from a visitor's browser.
#
# Default: the loopback origins the admin is actually served on. Override with
# QUANTUM_ADMIN_CORS_ORIGINS (comma-separated). "*" is still possible, but only
# by asking for it, and never in production mode.
_ADMIN_STRICT = os.environ.get("QUANTUM_ADMIN_ENV", "").lower() in (
    "production", "prod")
_cors_env = os.environ.get("QUANTUM_ADMIN_CORS_ORIGINS", "").strip()
if _cors_env:
...
if "*" in ALLOWED_ORIGINS and _ADMIN_STRICT:
    raise RuntimeError(
        "QUANTUM_ADMIN_CORS_ORIGINS='*' with QUANTUM_ADMIN_ENV=production: "
        "that lets any site call the admin API. List the real origins."
    )
...
# ============================================================================
# API AUTHENTICATION — DEFAULT DENY
# ============================================================================
#
# Authentication was per route, and most routes did not ask for it: /api/
# projects, /api/dashboard, /api/jobs-list, /api/docker/dashboard and the
# whole /api/resources/* family answered 200 to a request with no token,
# while /api/users/dashboard and /api/settings/dashboard answered 401. The
# admin's data was readable by anyone who could reach the port.
#
# Route-by-route auditing is what produced that state, and a new route added
# tomorrow would default to open again. So the gate lives here: everything
# under /api needs a token unless it is on this list.
#
# Set QUANTUM_ADMIN_ALLOW_ANONYMOUS=1 to go back to the old behaviour for a
# local session. It logs a warning every time it lets a request through.

PUBLIC_API_PREFIXES = (
    "/api/auth/",        # login itself cannot require a login
    "/api/health",
)
...
_ALLOW_ANONYMOUS = os.environ.get(
    "QUANTUM_ADMIN_ALLOW_ANONYMOUS", "").lower() in ("1", "true", "yes")

if _ALLOW_ANONYMOUS and _ADMIN_STRICT:
    raise RuntimeError(
        "QUANTUM_ADMIN_ALLOW_ANONYMOUS with QUANTUM_ADMIN_ENV=production: "
        "that serves the whole admin API without a login."
    )


# Routes that are public by nature, besides `/api/auth/*` and `/api/health`:
#
# - `/auth/login` is where the token is obtained; requiring it here is a door
#   locked without a key.
# - `/webhooks/*` cannot carry a JWT — the caller is GitHub/GitLab. They
#   have their own HMAC signature check, which refuses when the secret
#   is not configured.
# - `/health` e a sonda do orquestrador.
PUBLIC_WRITE_PATHS = frozenset({"/auth/login", "/auth/logout"})
PUBLIC_WRITE_PREFIXES = ("/webhooks/", "/health")
...
def _api_requires_auth(path: str, method: str = "GET") -> bool:
    """This gate covered ONLY `/api`, and the admin's real API lives at the root.

    37 WRITE routes were left out: /settings/connectors,
    /docker/containers/{id}/start, /projects/{id}/deploy... Measured with the
    server up, `POST /settings/connectors/{id}/test` answered 200 with no
    token at all.

    Now every write requires a token, on any path, with an explicit public
    list. Reads outside `/api` too: only the public GET list stays open
    (see PUBLIC_GET_PATHS).
    """
...
@app.middleware("http")
async def substitute_url_prefix(request: Request, call_next):
    """Replace the {URL_PREFIX} placeholder in HTML that forgot to.

    The codebase writes '{URL_PREFIX}/auth/login' into HTML and substitutes it
    at exactly one call site. Every other producer returned the placeholder
    literally — including the LOGIN PAGE, whose fetch therefore posted to
    '/%7BURL_PREFIX%7D/auth/login' (404) and whose stylesheet 404'd too. The
    admin could not be logged into at all, in the default configuration.

    Nine routes were affected. Doing it here fixes them together and keeps the
    next one from regressing; the substitution is idempotent, so the call site
    that already does it stays correct.
    """
    response = await call_next(request)

    ctype = response.headers.get("content-type", "")
    if not ctype.startswith("text/html") or response.headers.get("content-encoding"):
        return response
    ...
    if b"{URL_PREFIX}" in body:
        body = body.replace(b"{URL_PREFIX}", URL_PREFIX.encode())
...
async def require_api_auth(request: Request, call_next):
    """Refuse unauthenticated API calls, whatever the route forgot to declare."""
    path = request.url.path
    if ROOT_PATH and path.startswith(ROOT_PATH):
        path = path[len(ROOT_PATH):] or "/"

    if request.method != "OPTIONS" and _api_requires_auth(path, request.method):
        if _ALLOW_ANONYMOUS:
            logger.warning(
            ...
            if not token and reading:
                ...
                # so the cookie cannot be used to fake one (CSRF).
...
def startup_event():
    ...
    # Initialize Docker service: the configured DOCKER_HOST first, then local
    docker_host = os.environ.get("DOCKER_HOST", "unix:///var/run/docker.sock")
    try:
        docker_service = DockerService(docker_host=docker_host)
        print(f"[OK] Docker service initialized: {docker_host}")
  ...
  <div class="qa-login-card">
    ...
    <form id="login-form" onsubmit="doLogin(event)">
      ...
      <div class="qa-form-group">
        ...
        <!-- This shipped with value="admin": the login page filled in the
             default password for you. The default is gone; prefilling a
             password is not something a login form should do anyway. -->
        <input type="password" name="password" id="password" required class="qa-input" placeholder="Enter your password" autocomplete="current-password" />
      </div>
...
<head>
  ...
  <link rel="stylesheet" href="{URL_PREFIX}/static/quantum-admin.css">
  <script>
    // Every panel on this page is an htmx request, and htmx does not send the
    // bearer token the login flow stored — which is why the data routes had
    // no auth at all: adding it would have blanked the UI. One hook here
    // covers all of them, so the API can require a token.
    document.addEventListener('htmx:configRequest', function (evt) {{
      var t = null;
      try {{ t = localStorage.getItem('token'); }} catch (e) {{}}
      if (t) {{ evt.detail.headers['Authorization'] = 'Bearer ' + t; }}
    }});
    // A 401 means the session is gone. Say so, instead of leaving empty
    // panels that look like "there is no data".
    ...
    <aside class="qa-sidebar">
      ...
      <nav class="qa-sidebar-nav">
        ...
        <div class="qa-sidebar-section">
          ...
          <a href="https://danielgregorio.github.io/quantum/" target="_blank" rel="noopener noreferrer" class="qa-sidebar-link">
    ...
    <div class="qa-grid qa-grid-2 qa-mt-6">
      <div class="qa-card">
        <div class="qa-card-header">
          ...
          <button class="qa-btn qa-btn-ghost qa-btn-sm" hx-post="{URL_PREFIX}/docker/connect?host=ssh://user@localhost" hx-target="#docker-info" hx-swap="innerHTML">Reconnect</button>
        </div>
...
def get_docker_content():
    ...
    <div class="qa-card qa-mb-6">
      <div class="qa-card-body qa-flex qa-justify-between qa-items-center">
        ...
        <button class="qa-btn qa-btn-secondary" hx-post="{URL_PREFIX}/docker/connect?host=ssh://user@localhost" hx-swap="none" onclick="setTimeout(()=>location.reload(),1000)">Reconnect</button>
      </div>
    </div>
    <div class="qa-tabs">
...
def get_project_detail_content(project_id: int):
    ...
    <div id="tab-content">
      ...
      <div id="tab-environments" class="qa-tab-content qa-hidden">
        <div class="qa-card">
          <div class="qa-card-header qa-flex qa-justify-between qa-items-center">
            <h3 class="qa-card-title">Environments</h3>
            <div class="qa-flex qa-gap-2">
      ...
      <div class="qa-modal-content" style="max-width: 700px;">
        ...
        <div class="qa-modal-body">
          <form id="environment-form" onsubmit="saveEnvironment(event)">
            ...
            <!-- Health check -->
            <h4 class="qa-font-bold qa-mb-3">Health Check</h4>
            <div class="qa-form-group">
    ...
    function refreshLogs(dsId) {{
      ...
      .then(({{ ok, d }}) => {{
        ...
        if (!ok) {{
          logsEl.textContent = 'Error: ' + (d.detail || 'could not read the logs');
        }} else {{
          logsEl.textContent = d.logs || '(no log lines)';
...
def get_settings_content():
    ...
    <div id="settings-docker" class="qa-tab-panel">
      <div class="qa-card">
        ...
        <div class="qa-card-body">
          <div class="qa-form-group"><label class="qa-label">Docker Host</label><input type="text" class="qa-input" value="ssh://user@localhost" /><p class="qa-input-hint">SSH or TCP connection</p></div>
          <div class="qa-form-row">
        ...
        <div class="qa-card-footer qa-flex qa-justify-end qa-gap-2">
          <button class="qa-btn qa-btn-secondary" hx-post="{URL_PREFIX}/docker/connect?host=ssh://user@localhost" hx-swap="none">Test Connection</button>
...
def get_cicd_content():
    ...
    return '''
    <div class="qa-section-header">
      <p class="qa-text-secondary">Webhook events from your Git provider</p>
    </div>
...
def docker_connect(host: str = None):
    ...
    Args:
        host: Docker host URL. Examples:
            - ssh://user@docker-host.example.com (SSH tunnel)
...
def login(username: str, password: str, request: Request, response: Response):
    ...
    # fake a write from another site.
    response.set_cookie(
...
async def create_user(
    ...
    Accepts a JSON body as well as query parameters. It was query-only, which
    means the password travelled in the URL — into access logs, browser
    history and proxies. The query form still works so nothing that calls it
    today breaks.
    """
...
@app.delete("/auth/users/{username}", tags=["Auth"])
def delete_user(username: str, admin: User = Depends(require_admin)):
    """Delete a user (admin only).

    The Users screen had a Delete button that alerted "TODO" — AuthService
    has delete_user(), there was simply no route. This is it, with the two
    guards the service does not make: you cannot delete yourself (that locks
    you out mid-session) and you cannot delete the admin account (it is the
    one rebuilt from ADMIN_PASSWORD at boot).
    """
    ...
    if username == "admin":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="The admin account cannot be removed"
        )

    if not get_auth_service().delete_user(username):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No user named {username!r}"
        )
            ...
            return HTMLResponse(content="""
                <div class="qa-empty qa-p-4">
                    ...
                    <button class="qa-btn qa-btn-primary qa-btn-sm qa-mt-4"
                            hx-post="{URL_PREFIX}/docker/connect?host=ssh://user@localhost"
                            hx-swap="outerHTML">
                        Connect to Docker
                    </button>
                </div>
...
def update_datasource(
    datasource_id: int,
    changes: schemas.DatasourceUpdate,
    db: Session = Depends(get_db),
    user: User = Depends(require_auth),
):
    """Change a datasource.

    The DatasourceUpdate schema already existed; no route used it. You could
    create and delete a datasource, never fix it: a wrong host or wrong
    password meant deleting and recreating, losing the history. It was the
    most visible hole in the admin's core.

    The password is encrypted here, as on the creation path; sending an empty
    or absent `password` keeps the stored one, instead of erasing it.
    """
    datasource = crud.get_datasource(db, datasource_id)
    if not datasource:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if password:
        try:
            ...
            from secret_manager import encrypt_value
        fields["password_encrypted"] = encrypt_value(password)

    if not fields:
        # Nothing to do is not an error, but saying "updated" without updating
        # is exactly the pattern this session spent the day removing.
        return datasource

    updated = crud.update_datasource(db, datasource_id, **fields)
    logger.info(
        "datasource %s changed by %s: %s",
        datasource_id, user.username, ", ".join(sorted(fields)),
    )
    return updated
...
def get_datasource_container_logs(
    ...
    Failures are HTTP errors with a `detail`, not a 200 with an `error` key
    (which a client checking the status took for logs): 404 for an unknown
    datasource or a container that no longer exists, 409 for a datasource
    ...
    try:
        ...
        if type(e).__name__ == "NotFound":        # docker.errors.NotFound
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"Container {datasource.container_id} no longer exists")
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                            detail=f"Docker did not answer: {e}")
    try:
...
def test_datasource_connection(
    ...
    # The password is stored encrypted (line ~8582 encrypts it on save), and
    # this route handed the CIPHERTEXT to the driver as the password. "Test
    # connection" therefore failed on every datasource that has a password,
    # with an authentication error that blamed the credentials. The decrypt
    # helper was already used elsewhere in this file.
    try:
        from .secret_manager import decrypt_or_legacy
    except ImportError:
        from secret_manager import decrypt_or_legacy
    try:
        ds_password = (
            decrypt_or_legacy(datasource.password_encrypted)
            if datasource.password_encrypted else None
        )
    except ValueError as exc:
        # Encryption key changed/missing. Say so, instead of trying to
        # connect with the encrypted text and reporting "invalid password".
...
# (A second GET /datasources/{datasource_id}/logs was defined here. Starlette
#  always matched the one above, so it never ran; removed, which also ends
#  FastAPI's duplicate operation id warning.)
...
def run_component_tests(
    ...
    component.last_test_run_at = datetime.now(timezone.utc).replace(tzinfo=None)
    db.commit()
    ...
    def run_tests_background(test_run_id, component_id, test_files):
        ...
        try:
            ...
            if run:
                ...
                run.duration_seconds = int((datetime.now(timezone.utc).replace(tzinfo=None) - run.started_at).total_seconds())
...
def get_project_logs(
    ...
    if since:
        if since.endswith('h'):
            ...
            minutes = int(since[:-1])
            since_dt = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(minutes=minutes)
        else:
            try:
...
def get_project_logs_html(
    ...
    since_dt = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(minutes=minutes) if minutes else None
...
def generate_sample_logs(
    ...
    sources = ['app', 'database', 'access', 'api', 'security']
    ...
    for i in range(count):
        ...
        log_entry = ApplicationLog(
            project_id=project_id,
            timestamp=datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(minutes=random.randint(0, 60)),
            level=level,
            source=source,
            message=message,
...
def get_projects_list_html(db: Session = Depends(get_db)):
    ...
    <script>
        function showCreateProject() {
            // This alerted "Create project form - TODO" next to a working
            // creation flow: openCreateWizard() opens the template wizard
            // on the projects page, and POST /projects exists. Use it.
            if (typeof openCreateWizard === 'function') {
                openCreateWizard();
            } else {
                window.location.href = '{URL_PREFIX}/projects';
            }
        }
        ...
        function editProject(id) {
            // Editing lives on the project's own page (PUT /projects/{id}).
            window.location.href = '{URL_PREFIX}/projects/' + id;
        }
    </script>
    '''
...
def get_users_dashboard_html(
    ...
    # This was db.query(User).all(). `User` here is the auth dataclass, not a
    # SQLAlchemy model — there is no users table — so the screen raised
    # AttributeError and answered 500. Nobody saw it, because the route
    # required a token and every other route did not, so nothing sent one.
    #
    # The real user store is AuthService's, so read it from there.
    ...
    <div class="qa-users-dashboard">
        ...
        <div class="qa-alert qa-alert-warning qa-mb-6">
            Users live in memory, not in the database: anything created here
            is gone when the process restarts. The <code>admin</code> account
            is rebuilt at each start from <code>ADMIN_PASSWORD</code>.
        </div>
    ...
    for u in users:
        ...
        role_badge = 'qa-badge-primary' if role == 'admin' else 'qa-badge-gray'
        # The store keeps no creation date. Saying so beats printing a date
        ...
        html += f'''
                        <tr>
                            <td>#{u['id']}</td>
                            <td>{u['username']}</td>
                            <td><span class="qa-badge {role_badge}">{role}</span></td>
                            <td>{created}</td>
                            <td>
                                <button class="qa-btn qa-btn-sm qa-btn-danger"
                                        onclick="deleteUser('{u['username']}')"
                                        {'disabled title="the admin account cannot be removed"'
                                         if u['username'] == 'admin' else ''}>Delete</button>
                            </td>
                        </tr>
        '''

    # The Edit button alerted "Edit user N - TODO" and there is no endpoint
    # behind it — no role change, no rename. A button that cannot work is
    # worse than no button, so it is gone. Add and Delete stay, because
    # POST /auth/users and DELETE /auth/users/{username} do the work.
    html += '''
                    </tbody>
                </table>
            </div>
        ...
        <div class="qa-card qa-mt-6" id="create-user-card" style="display:none">
            <div class="qa-card-header"><h4>New user</h4></div>
            <div class="qa-card-body">
                <div class="qa-form-group">
                    <label class="qa-label">Username</label>
                    <input type="text" id="new-username" class="qa-input" autocomplete="off">
                </div>
                <div class="qa-form-group">
                    <label class="qa-label">Password</label>
                    <input type="password" id="new-password" class="qa-input" autocomplete="new-password">
                </div>
                <div class="qa-form-group">
                    <label class="qa-label">Role</label>
                    <select id="new-role" class="qa-input">
                        <option value="user">user</option>
                        <option value="admin">admin</option>
                    </select>
                </div>
                <button class="qa-btn qa-btn-primary" onclick="createUser()">Create</button>
                <span id="user-form-msg" class="qa-text-muted"></span>
            </div>
        </div>
    </div>
    <script>
        ...
        function showCreateUser() {
            var card = document.getElementById('create-user-card');
        ...
        async function createUser() {
            ...
            var res = await fetch('{URL_PREFIX}/auth/users', {
            ...
            if (res.ok) {
                ...
                msg.textContent = err.detail || ('Failed (HTTP ' + res.status + ')');
            }
        }
        async function deleteUser(username) {
            if (!confirm('Delete user ' + username + '?')) { return; }
            var res = await fetch('{URL_PREFIX}/auth/users/' + encodeURIComponent(username), {
...
def get_settings_dashboard_html(user: User = Depends(require_auth)):
    ...
    # This called settings_service.get_all(), which does not exist — the
    # screen answered 500 on every load. The real reader is
    # get_global_settings(), and it returns the nested shape of
    # GlobalSettings, not the flat keys this template was written against.
    settings_service = get_settings_service()
    settings = settings_service.get_global_settings()
    docker_cfg = settings.get('docker') or {}
    security_cfg = settings.get('security') or {}
    all_settings = {
        'docker_host': docker_cfg.get('host', ''),
        'registry_url': docker_cfg.get('registry', ''),
        'token_expiry_hours': security_cfg.get('jwt_expiry_hours', 24),
    }
    ...
    <div class="qa-settings-dashboard">
        ...
        <div class="qa-grid qa-grid-2 qa-gap-6">
            ...
            <div class="qa-card">
                ...
                <div class="qa-card-body">
                    <!-- There was a "JWT Secret" field here posting to
                         /api/settings/auth, a route that does not exist.
                         Even if it had, auth_service reads the signing key
                         from the environment and nowhere else: typing a
                         secret here changed nothing while looking like it
                         had. Read-only, with the truth about where the
                         values come from. -->
                    <div class="qa-form-group">
                        <label class="qa-label">Signing key</label>
                        <p class="qa-text-muted">Read from the
                        <code>JWT_SECRET_KEY</code> environment variable at
                        startup. Not settable from this screen.</p>
                    </div>
                    <div class="qa-form-group">
                        <label class="qa-label">Admin password</label>
                        <p class="qa-text-muted">Read from
                        <code>ADMIN_PASSWORD</code>. When unset, one is
                        generated per process and printed at startup.</p>
                    </div>
                    <div class="qa-form-group">
                        <label class="qa-label">Token expiry</label>
                        <p class="qa-text-muted">''' + str(all_settings.get('token_expiry_hours', 24)) + ''' hours</p>
                    </div>
                </div>
            </div>
            ...
            <div class="qa-card">
                ...
                <div class="qa-card-body">
                    <!-- Same story as the auth card: these fields posted to
                         /api/settings/webhooks, which does not exist, and
                         webhook_service reads both values from the
                         environment anyway. Showing whether they are set is
                         useful; pretending they can be typed here is not. -->
                    <div class="qa-form-group">
                        <label class="qa-label">GitHub webhook secret</label>
                        <p class="qa-text-muted"><code>GITHUB_WEBHOOK_SECRET</code>: ''' + (
                            'configured' if __import__('os').environ.get('GITHUB_WEBHOOK_SECRET')
                            else 'not set — /webhooks/github refuses every call') + '''</p>
                    </div>
                    <div class="qa-form-group">
                        <label class="qa-label">GitLab webhook token</label>
                        <p class="qa-text-muted"><code>GITLAB_WEBHOOK_TOKEN</code>: ''' + (
                            'configured' if __import__('os').environ.get('GITLAB_WEBHOOK_TOKEN')
                            else 'not set — /webhooks/gitlab refuses every call') + '''</p>
                    </div>
                </div>
            </div>
...
@app.post("/api/settings/docker", tags=["Settings"])
async def save_docker_settings(
    request: Request,
    user: User = Depends(require_auth),
):
    """Persist the Docker settings the settings screen collects.

    The form on that screen has always posted here; the route did not exist,
    so "Save Docker Settings" returned 404 into an hx-swap="none" — no error
    shown, nothing saved. This is the missing half.

    The body is parsed by hand rather than with Form(...) or request.form():
    both require python-multipart, which this install does not declare or
    have. htmx posts urlencoded, which urllib parses in three lines — and
    accepting JSON too costs nothing.
    """
    if user.role != 'admin':
    ...
    docker_host = str(fields.get("docker_host", "") or "")
    registry_url = str(fields.get("registry_url", "") or "")
    ...
    svc.set_setting("docker.host", docker_host.strip())
    svc.set_setting("docker.registry", registry_url.strip())
    return {"success": True, "docker": {"host": docker_host, "registry": registry_url}}
