...
# Rotas publicas por natureza, alem de `/api/auth/*` e `/api/health`:
#
# - `/auth/login` e onde se obtem o token; exigi-lo aqui e uma porta trancada
#   sem chave.
# - `/webhooks/*` nao podem carregar JWT — quem chama e o GitHub/GitLab. Elas
#   tem verificacao propria por assinatura HMAC, que recusa quando o segredo
...
def startup_event():
    ...
    forge_host = os.environ.get("DOCKER_HOST", "unix:///var/run/docker.sock")
    try:
...
def update_datasource(
    ...
    """Alterar um datasource.

    O schema DatasourceUpdate já existia; nenhuma rota o usava. Dava para
    criar e excluir um datasource, nunca corrigi-lo: host errado ou senha
    errada significava excluir e recriar, perdendo o histórico. Era o buraco
    mais visível do núcleo do admin.

    A senha é cifrada aqui, como no caminho de criação; enviar `password`
    vazio ou ausente mantém a que está gravada, em vez de apagá-la.
    """
    ...
    if not fields:
        # Nada a fazer não é erro, mas dizer "atualizado" sem atualizar é
        # exatamente o padrão que esta sessão passou o dia removendo.
        return datasource

    updated = crud.update_datasource(db, datasource_id, **fields)
    logger.info(
        "datasource %s alterado por %s: %s",
...
def test_datasource_connection(
    ...
    try:
        ...
        # Chave de criptografia trocada/ausente. Dizer isso, em vez de tentar
        # conectar com o texto cifrado e reportar "senha invalida".
...
def create_cloud_integration(data: Dict[str, Any], db: Session = Depends(get_db), request: Request = None):
    ...
    # The comment here said "in production, use proper encryption" and the
    # line below wrote plain JSON into a column named credentials_encrypted.
    # Cloud provider keys sat in the database in the clear, under a name that
    # said otherwise. secret_manager (Fernet) already existed; it just was
    # not used.
    try:
        from .secret_manager import encrypt_value
    except ImportError:
    ...
    credentials_json = encrypt_value(json.dumps(data.get('credentials', {})))
...
def update_cloud_integration(integration_id: int, data: Dict[str, Any], db: Session = Depends(get_db), request: Request = None):
    ...
    if 'credentials' in data and data['credentials']:
        try:
            ...
            from secret_manager import encrypt_value
        integ.credentials_encrypted = encrypt_value(json.dumps(data['credentials']))
...
def test_existing_cloud_integration(integration_id: int, db: Session = Depends(get_db)):
    ...
    try:
        credentials = (
            json.loads(decrypt_or_legacy(integ.credentials_encrypted))
            if integ.credentials_encrypted else {}
        )
