...
class QuantumParser:
    ...
    def _validate_xml(cls, content: str) -> List[str]:
        ...
        # A MESMA normalizacao que o parser do framework faz.
        #
        # Sem ela, este validador reprovava arquivos .q que o proprio Quantum
        # carrega sem reclamar: `<input required>`, `<br>`, `Forms & Actions`,
        # `condition="n < 0"`, o corpo Python de um <q:python>. O componente
        # aparecia na interface do admin marcado com "XML parse error" e o
        # autor nao tinha o que consertar — o arquivo estava certo.
        try:
        ...
        # A declaracao XML tem de sair antes de embrulhar. Um arquivo que
        # comeca com `<?xml version="1.0"?>` — dezenas dos entregues — virava
        # `<root ...><?xml ...?>`, e o erro reportado era "XML or text
        # declaration not at start of entity", que descreve o embrulho, nao o
        # arquivo.
        content = re.sub(r'^\s*<\?xml[^?]*\?>\s*', '', content)

        # E todo prefixo usado no documento precisa estar declarado no
        # embrulho. So `q:` estava, entao `qg:` (jogos), `qt:` (terminal),
        # `ui:` e `qtest:` davam "unbound prefix" — de novo um defeito do
        # embrulho, num arquivo correto. Declarar o que o documento usa
        # mantem os erros ESTRUTURAIS visiveis (tag nao fechada, token
        # invalido) e elimina os inventados.
        prefixos = sorted(set(re.findall(r'</?([A-Za-z_][\w.-]*):', content)))
        ns = ' '.join(f"xmlns:{p}='quantum-{p}'" for p in prefixos) or ''
