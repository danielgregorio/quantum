...
class QuantumParser:
    ...
    def _validate_xml(cls, content: str) -> List[str]:
        ...
        # The SAME normalisation the framework's parser does.
        #
        # Without it, this validator rejected .q files that Quantum itself
        # loads without complaint: `<input required>`, `<br>`, `Forms & Actions`,
        # `condition="n < 0"`, the Python body of a <q:python>. The component
        # showed up in the admin interface marked with "XML parse error" and the
        # author had nothing to fix — the file was right.
        try:
        ...
        # The XML declaration has to come out before wrapping. A file that
        # starts with `<?xml version="1.0"?>` — dozens of the shipped ones — became
        # `<root ...><?xml ...?>`, and the reported error was "XML or text
        # declaration not at start of entity", which describes the wrapper, not
        # the file.
        content = re.sub(r'^\s*<\?xml[^?]*\?>\s*', '', content)

        # And every prefix used in the document must be declared in the
        # wrapper. Only `q:` was, so `qg:` (games), `qt:` (terminal),
        # `ui:` and `qtest:` gave "unbound prefix" — again a defect of the
        # wrapper, in a correct file. Declaring what the document uses
        # keeps the STRUCTURAL errors visible (unclosed tag, invalid
        # token) and eliminates the invented ones.
        prefixes = sorted(set(re.findall(r'</?([A-Za-z_][\w.-]*):', content)))
        ns = ' '.join(f"xmlns:{p}='quantum-{p}'" for p in prefixes) or ''
        wrapped = f"<root {ns}>{content}</root>"
