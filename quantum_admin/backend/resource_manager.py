...
class PortManager:
    ...
    def release(self, port_or_service: int | str, host: str = "localhost") -> bool:
        ...
        allocation.released_at = datetime.now(timezone.utc).replace(tzinfo=None)
        self.db.commit()
...
class SecretManager:
    ...
    def rotate(
        ...
        secret.last_rotated = datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    async def check_health(self, service_id: int) -> Dict:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                service.last_health_check = datetime.now(timezone.utc).replace(tzinfo=None)
