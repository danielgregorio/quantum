...
async def run_health_check(
    ...
    try:
        async with httpx.AsyncClient(timeout=check.timeout_seconds) as client:
            ...
            response_time_ms = int((datetime.now(timezone.utc).replace(tzinfo=None) - start_time).total_seconds() * 1000)
            ...
            if is_healthy:
                ...
                if active_incident:
                    active_incident.resolved_at = datetime.now(timezone.utc).replace(tzinfo=None)
