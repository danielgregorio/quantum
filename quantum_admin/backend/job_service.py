...
# quantum is an installed package now (FRAMEWORK_PLAN Fase 1.1). This used to
# push <repo>/src onto sys.path and import `runtime.job_executor` — the
# pre-packaging layout. src/ became quantum/, so the import failed, the
# except swallowed it into a warning, HAS_JOB_EXECUTOR went False, and the
# admin's whole Jobs subsystem quietly reported zero jobs forever. The repo
# root is still added so the admin works from a checkout without installing.
QUANTUM_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if QUANTUM_ROOT not in sys.path:
    sys.path.insert(0, QUANTUM_ROOT)

try:
    from quantum.runtime.job_executor import (
        JobExecutor, JobQueueService, ScheduleService, ThreadService
    )
