...
Q_APPLICATION = TagInfo(
    ...
    attributes={
        ...
        "type": _attr("type", type=AttributeType.ENUM, default="html",
                     enum_values=["html", "game", "terminal", "testing", "ui", "microservices", "api", "desktop", "mobile"],
...
Q_SET = TagInfo(
    ...
    attributes={
        ...
        "type": _attr("type", type=AttributeType.ENUM, default="string",
                     enum_values=["string", "integer", "decimal", "boolean", "array", "struct", "date", "datetime", "number", "numeric", "float", "double", "int", "long", "text", "object"],
...
Q_FUNCTION = TagInfo(
    ...
    attributes={
        ...
        "returnType": _attr("returnType", type=AttributeType.ENUM, default="any",
                           enum_values=["any", "void", "string", "integer", "decimal", "boolean", "array", "struct", "query", "number", "numeric", "float", "double", "int", "long", "text", "object"],
...
Q_PARAM = TagInfo(
    ...
    attributes={
        ...
        "type": _attr("type", type=AttributeType.ENUM, default="string",
                     enum_values=["string", "integer", "decimal", "boolean", "array", "struct", "date", "datetime", "binary", "email", "url", "number", "numeric", "float", "double", "int", "long", "text", "object"],
...
Q_RETURN = TagInfo(
    ...
    attributes={
        ...
        "type": _attr("type", type=AttributeType.ENUM, default="string",
                     enum_values=["string", "integer", "decimal", "boolean", "array", "struct", "query", "json", "number", "numeric", "float", "double", "int", "long", "text", "object"],
                     description="Return type"),
...
Q_QUERY = TagInfo(
    ...
    attributes={
        "name": _attr("name", required=False, description="Query result variable name"),
...
Q_INVOKE = TagInfo(
    ...
    attributes={
        "name": _attr("name", required=False, description="Result variable name"),
...
Q_REDIRECT = TagInfo(
    ...
    attributes={
        # `required=False` e `to` como alias: o framework le AS DUAS formas
        # (parser.py aceita url= e to=), e os exemplos usam `to=` 27 vezes.
        # Exigir `url` acusava codigo correto — 27 dos 84 falsos positivos.
        "url": _attr("url", type=AttributeType.URL, required=False, description="Target URL"),
        "to": _attr("to", type=AttributeType.URL, description="Target URL (alias de url)"),
...
# Nomes de tipo aceitos pelo RUNTIME (component._PARAM_INT_TYPES e
# _PARAM_FLOAT_TYPES) que este esquema nao listava. `type="number"` e usado 36
# vezes nos proprios exemplos e o servidor o acusava como invalido: 240 dos
# 623 diagnosticos falsos vinham dai. Esquema mantido a mao diverge; a lista
# abaixo existe para deixar registrado de ONDE ela tem de ser derivada.

# Tags ANINHADAS: resolvidas pelo parser do PAI (q:tool dentro de q:agent,
# q:column dentro de q:data, ui:card-body dentro de ui:card), entao nunca
# aparecem no registry de tags de topo. Sem esta lista o servidor as
# acusava como desconhecidas em arquivos corretos.
#
# Colhidas dos proprios .q do repositorio. Para regerar:
...
def _framework_tags() -> set:
    """As tags que o parser do framework realmente registra.

    QUANTUM_TAGS e uma lista mantida A MAO, e ela divergiu: 76 entradas aqui
    contra 95 no registry. O servidor acusava "Unknown tag <q:python>",
    "<q:queue>", "<ui:card-body>" em arquivos que o framework parseia limpo —
    623 diagnosticos falsos em 124 arquivos. Falso positivo em massa e o que
    faz alguem desligar o language server.

    Consultar o registry e o unico jeito de as duas nao divergirem de novo. Se
    o framework nao estiver importavel, devolve vazio e o caminho antigo vale.
    """
    try:
        from quantum.core.parser import QuantumParser
        registro = QuantumParser().parser_registry
        nomes = set(getattr(registro, '_parsers', {}) or {})
...
def is_known_tag(tag_name: str) -> bool:
    """Se o esquema OU o framework conhecem a tag, ela nao e desconhecida."""
