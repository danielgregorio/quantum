...
def _validate_variable_usage(doc) -> List[types.Diagnostic]:
    ...
    # This read `get_symbols_by_kind(doc.symbols._by_kind)` — the whole dict
    # passed as the kind — and its body was `pass`. The dict is unhashable,
    # so the lookup raised TypeError, validate_document raised with it, and
    # the editor got NO diagnostics for ANY document, ever: not an unknown
    # tag, not a missing attribute, not malformed XML. The language server's
    # main job was dead on every file.
    #
    # The comment said parameters are implicitly available, which is right —
    # they are defined names, so they belong in the set.
    for symbol in doc.symbols.get_symbols_by_kind(SymbolKind.PARAMETER):
        defined_vars.add(symbol.name)
