...
class QuantumDocument:
    ...
    def _parse(self):
        ...
        if not self._parse_error:
            self._parse_error = self._check_well_formed()

    def _framework_accepts_document(self) -> bool:
        """O parser do framework le este documento sem erro?

        Cacheado: e chamado uma vez por elemento desconhecido e parsear o
        documento inteiro a cada um seria caro num arquivo grande.
        """
        if getattr(self, '_framework_ok', None) is None:
            self._framework_ok = False
            try:
                from quantum.core.parser import QuantumParser
                QuantumParser().parse(self.text)
                self._framework_ok = True
            except Exception:
                self._framework_ok = False
        return self._framework_ok

    def _check_well_formed(self) -> Optional[str]:
        """Report a file that will not parse, before it is run.

        _parse_elements is REGEX based — it tracks positions, which
        ElementTree does not — so an unclosed tag produced no error at all
        and the editor said nothing about a file the framework will reject
        outright. The most useful thing a language server can tell you was
        the one thing it did not.

        The framework's own normaliser runs first where it is importable, so
        the server agrees with the parser about ordinary HTML (bare `&`,
        boolean attributes, unclosed <br>) instead of inventing errors the
        ...
        try:
            ...
            pass       # analyse the raw text; better than not analysing

        # A .q file does not have to declare xmlns:q — the framework injects
        # it. Without this every single file came back "unbound prefix",
        # which is a false error on correct code and the fastest way to make
        # someone turn the language server off.
        try:
            from quantum.core.parser import QuantumParser
            text = QuantumParser()._inject_namespace(text)
        except Exception:
            return None    # cannot judge without the same preprocessing
        ...
        try:
            ...
            if 'unbound prefix' in message:
                # A fragment, not a document: the namespace injector only adds
                # xmlns to a q:component/q:application root, so a snippet like
                # `<q:set type="string" />` — what an editor sees constantly
                # while someone is typing, and what the existing test uses —
                # would be reported as broken. Worse, get_diagnostics returns
                # EARLY on a parse error, so this masked the real "missing
                # required attribute 'name'" message.
                return None
            return message
        except Exception:
            return None
    ...
    def _parse_elements(self):
        ...
        # `[^>]*?` parava no PRIMEIRO `>`, mesmo dentro de aspas. Uma linha
        # perfeitamente valida como
        #
        #     <q:if condition="a > b">
        #
        # era cortada em `condition="a `, e o servidor acusava "Missing
        # required attribute 'condition'" — em codigo correto. Medido: 643
        # diagnosticos falsos em 129 de 253 arquivos .q que o framework
        # parseia limpo. Falso positivo em massa e o que faz alguem desligar
        # o language server.
        #
        # Mesma correcao que quantum/core/html_compat._TAG ja tinha: o trecho
        # de atributos precisa entender valor entre aspas.
    ...
    def _validate_element(self, elem: ParsedElement) -> List[types.Diagnostic]:
        ...
        elif (tag.startswith('q:') or tag.startswith('ui:')) and not is_known_tag(tag):
            # Uma versao anterior suprimia isto quando o FRAMEWORK aceitava o
            # documento. Estava errado: o framework IGNORA em silencio uma tag
            # `q:` que nao conhece — `<q:naoexiste/>` vira HTMLNode e some.
            # Suprimir aqui apagava a deteccao de digitacao, que e valiosa
            # exatamente PORQUE o framework e silencioso.
            #
            # As tags filhas (q:tool, q:column, ui:card-body) sao resolvidas
            # por is_known_tag, que consulta o registry e a lista de aninhadas.
            diagnostics.append(types.Diagnostic(
                range=types.Range(
                    start=types.Position(line=elem.start_line, character=elem.start_col),
                    end=types.Position(line=elem.start_line, character=elem.end_col)
