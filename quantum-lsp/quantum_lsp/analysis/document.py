...
class QuantumDocument:
    ...
    def _framework_accepts_document(self) -> bool:
        """Does the framework's parser read this document without error?

        Cached: it is called once per unknown element, and parsing the whole
        document for each one would be expensive in a large file.
        """
    ...
    def _parse_elements(self):
        ...
        # `[^>]*?` stopped at the FIRST `>`, even inside quotes. A perfectly
        # valid line such as
        #
        #     <q:if condition="a > b">
        #
        # was cut at `condition="a `, and the server reported "Missing
        # required attribute 'condition'" — on correct code. Measured: 643
        # false diagnostics in 129 of 253 .q files that the framework
        # parses cleanly. Mass false positives are what makes someone turn
        # off the language server.
        #
        # Same fix quantum/core/html_compat._TAG already had: the attribute
        # section has to understand quoted values.
    ...
    def _validate_element(self, elem: ParsedElement) -> List[types.Diagnostic]:
        ...
        if tag_info:
            ...
            # An earlier version suppressed this when the FRAMEWORK accepted
            # the document. That was wrong: the framework silently IGNORES a
            # `q:` tag it does not know — `<q:doesnotexist/>` becomes an
            # HTMLNode and disappears. Suppressing it here erased typo
            # detection, which is valuable precisely BECAUSE the framework is
            # silent.
            #
            # Child tags (q:tool, q:column, ui:card-body) are resolved by
            # is_known_tag, which checks the registry and the nested list.
            diagnostics.append(types.Diagnostic(
                range=types.Range(
                    start=types.Position(line=elem.start_line, character=elem.start_col),
                    end=types.Position(line=elem.start_line, character=elem.end_col)
