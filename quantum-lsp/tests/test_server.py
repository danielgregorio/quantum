...
class TestSchema:
    ...
    def test_required_attributes(self):
        ...
        # q:query does NOT require 'name'. A WRITE query has no result to
        # bind to any variable — the framework's own parser says so ("A write
        # query (UPDATE/INSERT/DELETE) has no result set to bind") and does
        # not raise without the attribute. The four <q:query> without a name
        # in the repository are all UPDATE or DELETE.
        #
        # This test claimed it was required, encoding the schema's belief
        # instead of the framework's behaviour — and because of it the
        # server flagged correct code 31 times.
