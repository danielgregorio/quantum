<q:component name="TestAuthLogin">
  ...
  <q:action name="login" method="POST">
    ...
    <!-- AuthService.is_session_expired() requires session.sessionExpiry to be
         set (fail-closed: missing expiry == expired). There's no declarative
         "now + N hours" helper in the language yet, so this is hardcoded for
         the demo instead of computed. -->
    <q:set name="session.sessionExpiry" value="2027-01-01T00:00:00" />
