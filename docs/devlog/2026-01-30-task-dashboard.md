...
**Status:** Funcional no forge (localhost), pendente roteamento via sargas.cloud
...
Dashboard 100% funcional em `http://localhost/dashboard/`:
...
O dashboard funciona via IP direto (`http://localhost/dashboard/`) mas NAO via dominio (`https://quantum.sargas.cloud/dashboard/`).

**Diagnostico:**
- O custom `http.conf` gerado pelo deploy service cria server block com `server_name localhost`
...
**Solucao necessaria:**
- Configurar o proxy host do NPM para `quantum.sargas.cloud` para fazer proxy para `http://localhost:80` com `proxy_set_header Host localhost`
