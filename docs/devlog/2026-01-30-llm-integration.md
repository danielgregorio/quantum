...
Implementacao completa da tag `<q:llm>` no Quantum Framework, permitindo invocar modelos LLM (Large Language Models) diretamente de componentes `.q`. Backend utiliza Ollama rodando no forge (localhost:11434) com modelo phi3.
...
**Causa:** O deploy service no forge (`/home/user/quantum-deploy/`) tinha codigo-fonte antigo sem suporte a `LLMNode`. Os 5 arquivos core nunca tinham sido atualizados no servidor.
...
- `/home/user/quantum-deploy/src/runtime/execution_context.py` (deploy service base)
...
- Ollama: `http://localhost:11434` com modelo `phi3:latest`
- Deploy service: `/home/user/quantum-deploy/` (uvicorn)
- Container: `quantum-llm-demo` (Docker, porta 8080, proxy via nginx)
- URL publica: `http://localhost/llm-demo/`
