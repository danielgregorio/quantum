...
[One App, Many Screens](/guide/ui). The core set of elements listed there is
drawn the same in the browser, the console and the desktop window (SPEC
`UI-7`); the other elements on these reference pages work in the browser only
and are **Experimental**. Examples below written as
`<q:application type="ui">` are standalone builds: layout only (`UI-8`).
:::
...
`--target desktop` was removed in 0.16. The same pages the browser serves run in
a native window with `quantum desktop` (see [Desktop](/targets/desktop)).
