...
```bash
# Start the server
pypy3 -m quantum.cli.runner start

# Run a component
pypy3 -m quantum.cli.runner run examples/counter.q
...
```python
from quantum.runtime.pypy_compat import print_compatibility_report
...
```bash
python -c "from quantum.runtime.pypy_compat import print_compatibility_report; print_compatibility_report()"
```
...
```python
from quantum.runtime.pypy_compat import PyPyOptimizer
...
1. **Use for long-running servers**
   ...
   pypy3 -m quantum.cli.runner start --port 8000
   ```
...
```python
from quantum.runtime.pypy_compat import (
    is_pypy,
...
# Monitor with:
from quantum.runtime.pypy_compat import QuantumPyPyAdapter
