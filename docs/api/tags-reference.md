...
Defines a Quantum application of one of the non-web engines. A web app is not
a `q:application`: it is pages in `components/`, served by `quantum start`
(APP-1).
...
| `type` | enum | **required** | Application type: `game` (Laboratory), `terminal`, `ui` (Experimental) |
