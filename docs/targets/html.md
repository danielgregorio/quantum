...
::: warning Layout only
A standalone build (`<q:application type="ui">` with `--target html`) draws
**layout only**: there is no runtime in the file, so `q:set`, `q:function` or
any other command in it is an error (SPEC `UI-8`). For screens with data and
actions, write pages — see [One App, Many Screens](/guide/ui) — and serve them
with `quantum start`.
:::
