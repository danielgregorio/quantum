...
run an application in the terminal — pages, actions, sessions — use
`quantum console`: the same pages the browser shows, drawn by Textual from the
same runtime. See [One App, Many Screens](/guide/ui).
:::
...
# Output is saved as {app_id}_console.py
# Run the terminal app
python myapp_console.py
```
