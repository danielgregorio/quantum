...
`returnType` is checked on every return: the value is converted to the type
(`"7"` becomes `7` for `number`), and a value that is not of the type is an error
that names the function. `any` (the default) accepts anything; `void` means the
