...
**`Not stopping PID …`** — `quantum stop` found a `.quantum.pid` whose process
is not the server that wrote it (the server ended without cleaning up, and its
number now belongs to another program). It kills nothing and removes the stale
file; if a Quantum server is still running, stop it by hand.
