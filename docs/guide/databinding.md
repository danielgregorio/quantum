...
`in` compares the way `==` does, so a value that arrived as text still finds
its number in a list:
...
double-quoted attribute: `{status == 'active'}`.
...
| `len(x)`, `first(x)`, `last(x)` | `len(items)` | number of items |
| `get(x, key, default=null)` — an optional key or index | `get(prefs, 'theme', 'light')` | the value, or `default` when it is missing |
| `join(list, sep=', ')`, `sort(list)` | `join(sort(tags), ' / ')` | text |
| `round(n, digits=0)` — halves away from zero | `round(2.5)` | `3` |
...
| `random()`, `random(a, b)` | `random(1, 6)` | a number from 0 up to 1; an integer from `a` to `b` |
| `chance(p)`, `pick(list)` | `pick(tips)` | true with probability `p`; one element |

Units for the date functions: `s`, `n` (minutes), `h`, `d`, `w`. A
`q:function` declared in the component is called the same way: `{twice(price)}`.

Functions are called by name, never as methods: `split(title, ' ')`, not
`title.split(' ')`; `ceil(n)`, not `Math.ceil(n)`. The error for a JavaScript
habit names the function to use. `.length` works on text and lists.
