...
The login lives in the session, which is a signed cookie. To remember the user
for longer, give the session a later expiry — there is no separate cookie
scope:
...
<q:action name="login" method="POST">
  ...
  <q:if condition="form.remember">
    <q:set name="session.sessionExpiry" value="{dateAdd('d', 30)}" type="string" />
  </q:if>
</q:action>
```
