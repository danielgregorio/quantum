...
# The same evaluator the interpreter uses, so a {expression} means one thing
# whether the .q file is interpreted or compiled to Python. This replaced an
# eval(expr, {'__builtins__': {}}, context): emptying builtins removes names,
# not the object graph, so ().__class__.__mro__[1].__subclasses__() still
# reached every loaded class from a compiled template.
...
def bind(template: str, context: Dict[str, Any]) -> str:
    ...
    def replace(match):
        ...
        # {n}, {n,m} are regex quantifiers, not expressions.
        if is_regex_quantifier(expr):
            return match.group(0)
        try:
            result = _EXPRESSIONS.evaluate(expr, context)
