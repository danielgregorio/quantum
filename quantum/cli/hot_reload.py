...
class HotReloadServer:
    ...
    def stop(self):
        ...
        # Only clear the flag: main() leaves its loop within 0.1 s and the
        # `async with serve(...)` closes the server and its sockets. Stopping
        # the event loop instead abandoned main() mid-`async with`; the loop
        # was then closed under it, and when the orphaned coroutine was
        # collected later its cleanup raised "Event loop is closed".
        self._running = False
    ...
    def _run_server(self):
        ...
        try:
            import websockets
            try:
                # websockets >= 13; the legacy module warns since 14.
...
class HotReloadManager:
    ...
    def _handle_changes(self, changes: List[FileChange]):
        ...
        # The callback runs BEFORE the browsers are told: the server clears
        # its caches there, and a page reloading right after the message must
        # not be served from the old ones.
        if self.on_reload:
            try:
