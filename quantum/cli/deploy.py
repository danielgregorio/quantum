...
class QuantumDeployer:
    ...
    EXCLUDE_PATTERNS = [
        ...
        '.env.local',
        '.env.*.local',
        # `.env` sozinho so casava com o arquivo com esse nome exato:
        # `.env.production` e `.env.prod` iam para o servidor junto com o
        # resto.
        '.env.*',
        'node_modules',
        '.venv',
    ...
    # O que `.env.*` NAO deve levar junto.
    #
    # `.env.example` e o oposto de um segredo: e a documentacao de quais
    # variaveis o operador precisa definir, e e o arquivo que se QUER no
    # servidor. Um `.env.*` cru arrastava esses tambem — trocar um vazamento
    # por uma omissao silenciosa nao e conserto.
    KEEP_PATTERNS = [
        '.env.example',
        '.env.sample',
        '.env.template',
    ...
    def _is_excluded(cls, name: str) -> bool:
        """O nome casa com algum EXCLUDE_PATTERN?

        Isto era um casador escrito a mao que so entendia quatro formas:
        `*x*`, `*.ext`, `prefixo*` e igualdade exata. Um `*` no MEIO caia no
        ultimo caso, e tres patterns da lista tem um:

            .env.*.local   ->  `.env.production.local` era EMPACOTADO E
                               ENVIADO ao servidor. Segredo de producao no
                               tarball, sem aviso nenhum.
            test_*.py      ->  testes iam junto
            *_test.py      ->  idem

        Nenhum dos tres casava com nada, a nao ser com um arquivo chamado
        literalmente `.env.*.local`. fnmatch entende as tres formas — e as
        outras continuam funcionando igual.

        `fnmatchcase`, nao `fnmatch`: o segundo normaliza a caixa conforme o
        sistema, entao a mesma lista excluiria coisas diferentes no Windows e
        no Linux, e um deploy feito de um lado subiria um conjunto de
        arquivos diferente do outro.
        """
    ...
    def _create_tarball(self, app_path: Path, name: str, verbose: bool = False) -> Path:
        ...
        def should_exclude(item: Path, raiz: Path) -> bool:
            """O item, ou algum diretorio dele ATE `raiz`, e excluido?

            So ate a raiz. Antes era `[item] + list(item.parents)`, e
            `parents` sobe ate a raiz do disco: um app em
            `C:\\...\\venv\\meuapp` ou `~/projetos/tests/app` casava com um
            pattern num diretorio de FORA do app, e o tarball saia vazio —
            "deploy com sucesso" de nada.
            """
            return any(self._is_excluded(parte)
                       for parte in item.relative_to(raiz).parts)

        with tarfile.open(tarball_path, 'w:gz') as tar:
            # Add app files
            for item in app_path.rglob('*'):
                # Skip excluded items
                if should_exclude(item, app_path):
                    if verbose:
            ...
            if include_runtime and not app_has_src:
                ...
                if quantum_src and quantum_src.exists():
                    ...
                    for item in quantum_src.rglob('*'):
                        # Tambem pelos diretorios: antes so o NOME do arquivo
                        # era testado, entao `tests/` do runtime era pulado
                        # como diretorio mas todo arquivo dentro dele ia
                        # junto.
                        if should_exclude(item, quantum_src):
                            continue
                        if item.is_file():
...
class QuantumAppsManager:
    ...
    def list_apps(self, target: str = 'forge', verbose: bool = False) -> List[Dict]:
        ...
        try:
            ...
            # O alvo default e `http://forge:9000`, um host que so resolve na
            # rede de quem escreveu isto. Para todo mundo, `quantum apps`
            # falhava com um erro de DNS citando um nome nunca visto e sem
            # dizer de onde ele veio.
            raise DeployError(
                f"Nao consegui falar com o servidor de deploy em {url}.\n"
                f"  Esse e o alvo '{target}'. Se voce nao tem um servidor "
                f"Quantum Forge, este comando nao se aplica a voce.\n"
                f"  Para apontar para outro lugar: "
                f"QUANTUM_FORGE_URL=http://seu-host:porta\n"
                f"  Detalhe: {e}"
            )
