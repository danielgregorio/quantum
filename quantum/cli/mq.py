...
def handle_mq(args) -> int:
    """Handle mq command"""
    from quantum.runtime.message_broker import MessageBroker, Message
    from quantum.runtime.adapters.memory_adapter import InMemoryAdapter
    ...
    try:
        ...
        # Actions that only make sense ACROSS processes. With the default
        # in-memory broker they silently do nothing, so say so.
        if args.mq_action in ('publish', 'send', 'consume', 'subscribe',
                              'worker', 'stats'):
            _warn_if_ephemeral(broker, f"mq {args.mq_action}")
...
def _get_broker():
    ...
    if broker_type == 'rabbitmq':
        try:
            from quantum.runtime.adapters.rabbitmq_adapter import RabbitMQAdapter
        ...
        try:
            from quantum.runtime.adapters.redis_adapter import RedisAdapter
            broker = RedisAdapter()
            broker.connect({
    ...
    if broker_type == 'sqlite':
        ...
        broker = SqliteAdapter()
        broker.connect({'path': os.environ.get('QUANTUM_MQ_PATH', 'quantum_messages.db')})
        return broker

    # Default: in-memory.
    #
    # This broker lives inside THIS PROCESS and dies with it. Every `quantum
    # mq` invocation is its own process, so `mq publish` wrote into a broker
    # that was discarded a moment later and `mq consume` created a fresh empty
    # one — every message silently lost, with the CLI reporting success both
    # times. Nothing said the messages were going nowhere.
    #
    # It still exists because it is genuinely useful in-process (a .q file
    # that publishes and subscribes in one run), so this warns rather than
    # refusing; the caller decides whether the operation is cross-process.
    from quantum.runtime.adapters.memory_adapter import InMemoryAdapter
    broker = InMemoryAdapter()
    broker.connect({})
    broker._is_ephemeral_cli_broker = True
    return broker


def _warn_if_ephemeral(broker, action: str) -> None:
    """Say plainly that a cross-process mq action cannot work in memory."""
    if not getattr(broker, '_is_ephemeral_cli_broker', False):
        return
    print(
        f"[WARN] QUANTUM_MQ_BROKER is 'memory' (the default). That broker lives\n"
        f"       inside this process and is discarded when it exits, so "
        f"`{action}` cannot\n"
        f"       reach any other command. Messages published this way are lost.\n"
        f"       Set QUANTUM_MQ_BROKER=rabbitmq or =redis for anything that has to\n"
