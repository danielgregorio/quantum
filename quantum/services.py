...
# What each module registered, kept across _reset(): load() re-registers from
# it instead of re-executing an already imported module.
_by_module: Dict[str, Dict[str, Callable[..., Any]]] = {}
...
def service(name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    ...
    def register(func: Callable[..., Any]) -> Callable[..., Any]:
        with _lock:
            ...
            _registry[name] = func
            _by_module.setdefault(func.__module__, {})[name] = func
        return func
...
def load(modules: Iterable[str], project_dir: str = None) -> None:
    ...
    if modules and root not in sys.path:
    ...
    for module in modules or []:
        ...
        try:
            ...
            if ja_importado and not any(f.__module__ == module for f in _registry.values()):
                # Imported before the registry was cleared (_reset): its
                # @service decorators already ran and would not run again on a
                # plain import. Register again what they registered. Reloading
                # the module instead re-executed it in place: its classes
                # became new objects while other modules kept the old ones, so
                # `except ProjectError` stopped catching the ProjectError the
                # admin raised -- in some test orders.
                recorded = _by_module.get(module)
                if recorded:
                    for name, func in recorded.items():
                        service(name)(func)
                else:
