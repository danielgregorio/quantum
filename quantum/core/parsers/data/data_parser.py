...
class DataParser(BaseTagParser):
    ...
    def _parse_column(self, element: ET.Element) -> ColumnNode:
        ...
        # PARSE-3 / DATA-1: a column converts its field to `type`. The rules
        # below were accepted and never checked — no row was ever refused.
        ignored = [a for a in ('required', 'default', 'validate', 'pattern', 'min', 'max',
                               'minlength', 'maxlength', 'range', 'enum') if element.get(a) is not None]
        if ignored:
            from quantum.core.parser import QuantumParseError
            raise QuantumParseError(
                f'<q:column name="{name}"> {ignored[0]}= is not supported: it was accepted and never '
                f'checked. A q:column takes name and type; to keep only valid rows use '
                f'<q:transform><q:filter condition="..."/></q:transform> (DATA-3).')

        return ColumnNode(name, col_type)
