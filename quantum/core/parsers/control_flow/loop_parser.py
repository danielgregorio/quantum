"""
Loop Parser - Parse q:loop statements

Handles various loop types: range, array, list, query.
"""

from typing import List
from xml.etree import ElementTree as ET
from quantum.core.parsers.base import BaseTagParser, ParserError
from quantum.core.features.loops.src.ast_node import LoopNode
from quantum.core.ast_nodes import TextNode


class LoopParser(BaseTagParser):
    """
    Parser for q:loop statements.

    Supports:
    - Range loops (from/to)
    - Array iteration
    - List iteration (with delimiter)
    - Query result iteration
    """

    @property
    def tag_names(self) -> List[str]:
        return ['loop']

    def parse(self, element: ET.Element) -> LoopNode:
        """
        Parse q:loop statement with various types.

        Args:
            element: XML element for q:loop

        Returns:
            LoopNode AST node
        """
        # Check for query attribute (shorthand syntax)
        query_attr = self.get_attr(element, 'query')

        if query_attr:
            # <q:loop query="users"> shorthand: the query name doubles as the
            # row variable, so the body reads {users.name}.
            #
            # `var=` is honoured here when given. It used to be read only in
            # the branch below, so <q:loop query="projects" var="p"> silently
            # discarded the name and {p.name} rendered as literal text with no
            # error anywhere — found by writing a real screen against it
            # (FRAMEWORK_PLAN.md Fase 4). Omitting var= keeps the old
            # behaviour exactly.
            loop_type = 'query'
            var_name = self.get_attr(element, 'var') or query_attr
            loop_node = LoopNode(loop_type, var_name)
            loop_node.query_name = query_attr
        else:
            # Traditional syntax
            loop_type = self.get_attr(element, 'type')
            if not loop_type:
                # No explicit type: infer from attributes present instead of
                # always defaulting to 'range'. A bare <q:loop items="..."
                # var="..."> with no type= used to be misclassified as a range
                # loop and fail validation ("Range loop requires 'from'
                # attribute") — confirmed breaking examples in multiple
                # unrelated features (agents, data_import, scripting).
                loop_type = 'array' if self.get_attr(element, 'items') else 'range'
            if loop_type not in LOOP_TYPES:
                # PARSE-5: a type nothing runs is a parse error with the line. It
                # used to parse and then fail — or do nothing — when it ran.
                raise ParserError(f'<q:loop type="{loop_type}"> does not exist; type is one of: '
                                  f'{", ".join(LOOP_TYPES)}')
            var_name = self.get_attr(element, 'var')

            if not var_name:
                raise ParserError("Loop requires 'var' attribute")

            loop_node = LoopNode(loop_type, var_name)

        # Configure based on loop type
        if loop_type == 'range':
            loop_node.from_value = self.get_attr(element, 'from')
            loop_node.to_value = self.get_attr(element, 'to')
            loop_node.step_value = self.get_int_attr(element, 'step', 1)

        elif loop_type == 'array':
            loop_node.items = self.get_attr(element, 'items')
            loop_node.index_name = self.get_attr(element, 'index')

        elif loop_type == 'list':
            loop_node.items = self.get_attr(element, 'items')
            loop_node.delimiter = self.get_attr(element, 'delimiter', ',')
            loop_node.index_name = self.get_attr(element, 'index')

        elif loop_type == 'query':
            loop_node.index_name = self.get_attr(element, 'index')

        # Parse loop body - add text content before first child
        if element.text and element.text.strip():
            loop_node.add_statement(TextNode(element.text))

        # Parse loop body statements
        for child in element:
            statement = self.parse_statement(child)
            if statement:
                loop_node.add_statement(statement)
            # Add tail text after child element
            if child.tail and child.tail.strip():
                loop_node.add_statement(TextNode(child.tail))

        return loop_node


# PARSE-5: the loop types the executor and the renderer run.
LOOP_TYPES = ('range', 'array', 'list', 'query')
