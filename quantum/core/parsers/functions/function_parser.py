...
class FunctionParser(BaseTagParser):
    ...
    def parse(self, element: ET.Element) -> FunctionNode:
        ...
        # FN-4: returnType is checked on every return (see ComponentRuntime).
        from quantum.runtime.param_validation import RETURN_TYPES
        if func_node.return_type.lower() not in RETURN_TYPES:
            from quantum.core.parser import QuantumParseError
            raise QuantumParseError(
                f'<q:function name="{name}"> returnType="{func_node.return_type}" is not a type; '
                f'use one of: {", ".join(sorted(RETURN_TYPES))}')
