...
class NotJSONResponse(ValueError):
    """responseFormat="json" and the body is not JSON (INV-2)."""
...
class InvocationService:
    ...
    def _invoke_http(self, params: Dict[str, Any]) -> InvocationResult:
        ...
        for attempt in range(retry + 1):
            try:
                ...
                except NotJSONResponse as exc:
                    # A failure (INV-2), not retried: asking again returns the same body.
                    return InvocationResult(
                        success=False,
                        error={"message": str(exc), "type": "NotJSONResponse",
                               "status_code": response.status_code},
                        metadata={"status_code": response.status_code, "attempts": attempt + 1})
    ...
    def _parse_http_response(self, response: requests.Response, response_format: str) -> Any:
        ...
        if response_format == 'auto':
            ...
            # INV-2: responseFormat="json" promises JSON. A response that is
            # not JSON used to succeed, with {"error", "text"} as the value —
            # the page went on with a dict it did not expect.
            try:
                return response.json()
            except ValueError:
                excerpt = ' '.join(response.text.split())[:80]
                raise NotJSONResponse(
                    f'responseFormat="json", but the response is not JSON '
                    f'(Content-Type: {response.headers.get("Content-Type") or "none"}): {excerpt!r}') from None
