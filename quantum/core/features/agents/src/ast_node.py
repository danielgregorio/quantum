...
class AgentNode(QuantumNode):
    ...
    Attributes:
        ...
        model: LLM model identifier (default: the configured one, IA-1)
    ...
    Example:
        ...
        <q:agent name="support" model="phi3" maxIterations="5">
            ...
        </q:agent>
    ...
    """
    name: str = ""
    model: Optional[str] = None  # None: llm.model / QUANTUM_LLM_DEFAULT_MODEL
    endpoint: str = ""
    provider: str = "auto"  # ollama, openai, anthropic, auto
    api_key: str = ""  # For cloud providers
    max_iterations: int = 10
    timeout: int = 60000  # milliseconds
    on_error: str = "fail"  # IA-5: "continue" hands a failure to the page
    ...
    # Set by the team parser for agents declared inside <q:team>. Those agents
    # are driven by the team's own <q:execute>, so requiring one of their own
    # would reject every valid team component.
    in_team: bool = False
    ...
    def validate(self) -> List[str]:
        ...
        if not self.execute and not self.in_team:
