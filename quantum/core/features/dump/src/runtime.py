...
class DumpService:
    ...
    def _dump_html(
        ...
        # Check depth limit
        if current_depth >= max_depth:
            output.append('<span style="color: #999;">... (max depth reached)</span>')
            if current_depth == 0:
                output.append('</div>')
            return ''.join(output)
        ...
        if isinstance(var, bool):
            ...
            output.append(' <span style="color: #999; font-size: 0.9em;">(boolean)</span>')
            ...
            output.append(' <span style="color: #999; font-size: 0.9em;">(string)</span>')
        # Handle dictionaries/objects
