...
def parse_knowledge(element: ET.Element) -> KnowledgeNode:
    ...
    if element.get('model') is not None:
        ...
        raise QuantumParseError(f'<q:knowledge name="{name}"> model= is not supported: a knowledge base only embeds its sources (embedModel=). The model that answers is chosen on q:llm: <q:llm model="…" knowledge="{name}"> (IA-3, IA-6)')
