...
class LogNode(QuantumNode):
    ...
    # Spellings people actually write, mapped to the level they mean. `warn`
    # is what Python's own logging, JavaScript's console and most log
    # libraries call it, and two examples in this repository use it — one of
    # them the only q:websocket example there is, which therefore did not
    # parse. Rejecting a word the whole industry uses is not strictness, it
    # is a papercut.
    LEVEL_ALIASES = {
        'warn': 'warning',
        'fatal': 'critical',
    ...
    def __init__(
        ...
        level = self.LEVEL_ALIASES.get(str(level).strip().lower(), level)
