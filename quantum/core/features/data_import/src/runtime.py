...
# Shared with ComponentRuntime and HTMLRenderer, so a {field} expression in a
# q:data filter means the same thing it means anywhere else in the language.
...
class DataImportService:
    ...
    def _import_csv(self, source: str, params: Dict[str, Any]) -> DataResult:
        ...
        try:
            # Get CSV content
            content = self._get_source_content(source, params.get('headers', []), params.get('encoding'))
            ...
            # DATA-1: skip_rows drops the lines BEFORE the header (a title, a
            # note). It used to drop data rows after the header had been read.
            ...
            csv_reader = csv.DictReader(
                ...
                delimiter=delimiter,
                quotechar=quote
            ) if has_header else csv.reader(text, delimiter=delimiter, quotechar=quote)
    ...
    def _get_source_content(self, source: str, headers: List[Dict[str, str]] = None,
                            encoding: str = None) -> str:
        """Get content from file or URL, in `encoding` (DATA-1; UTF-8 by default)."""
        ...
        if source.startswith('http://') or source.startswith('https://'):
            ...
            # encoding= was parsed and never used: every file was read as UTF-8.
    ...
    def _evaluate_condition(self, condition: str, record: Dict[str, Any], context: Any) -> bool:
        ...
        `{field}` names the record's field. The braces are stripped so the
        evaluator resolves the NAME; the record's values are never spliced
        into the expression text.

        That distinction is the whole fix. This used to build a string by
        interpolating each field value into the condition — quoting strings as
        f"'{value}'" — and then call a bare eval() on it, with full builtins.
        A single quote anywhere in an imported CSV cell closed the literal and
        the rest of the cell ran as Python. The input here is an imported file,
        which is the least trusted thing the framework touches.
        """
        return bool(self._evaluate_expression(condition, record, context))

    def _evaluate_expression(self, expression: str, record: Dict[str, Any], context: Any) -> Any:
        """Evaluate a computed-field expression against one imported record.

        See _evaluate_condition for why this no longer interpolates values.
        """
        expr = _BINDING.sub(lambda m: m.group(1), expression).strip()
        return _EXPRESSIONS.try_evaluate(expr, record, default=None)
