...
def _bound(value) -> bool:
    """An attribute written as `{expression}` (UI-1): its value exists only at render."""
...
class UILayoutMixin:
    ...
    def _init_layout_attrs(self):
        ...
        # UI-2: responsive layout, declared (sm < 640 px, md < 768 px, lg < 1024 px)
        self.stack_below: Optional[str] = None   # hbox side by side, stacked below the breakpoint
        self.hide_below: Optional[str] = None
        self.hide_above: Optional[str] = None
        self.grow: Optional[str] = None          # "true": takes the room left in its row
...
class UIFormNode(QuantumNode, UILayoutMixin):
    ...
    def __init__(self):
        ...
        self.rules: Optional[str] = None  # UI-9: 'off' = no rules from the action
        self.values: Optional[str] = None  # UI-10: {row} whose fields fill the form
        self.submit: Optional[str] = None  # UI-10: text of the generated button
        self.children: List[QuantumNode] = []
...
class UIInputNode(QuantumNode, UILayoutMixin):
    ...
    def __init__(self):
        ...
        self.value: Optional[str] = None  # initial value (UI-6)
        self.search: Optional[str] = None  # UI-12: id of the region it refreshes as you type
        self.delay: Optional[int] = None   # UI-12: ms after the last key (300 with search=)
        self.placeholder: Optional[str] = None
        self.input_type: str = "text"    # text, password, email, number, file
        self.rows: Optional[int] = None    # UI-14: a multi-line field (a textarea)
        self.accept: Optional[str] = None  # UI-14: file types a file field offers
    ...
    def to_dict(self) -> Dict[str, Any]:
        d = {"type": "ui_input", "bind": self.bind,
             ...
             "validators": self.validators, "rows": self.rows, "accept": self.accept}
        d.update(self._layout_to_dict())
        return d
...
class UICheckboxNode(QuantumNode, UILayoutMixin):
    ...
    def __init__(self):
        self.bind: Optional[str] = None
        self.checked: Optional[str] = None  # "true"/"false" or {expr} (UI-6)
        self.label: Optional[str] = None
        self._init_layout_attrs()
...
class UIRadioNode(QuantumNode, UILayoutMixin):
    ...
    def __init__(self):
        self.bind: Optional[str] = None
        self.value: Optional[str] = None  # selected option (UI-6)
...
class UITableNode(QuantumNode, UILayoutMixin):
    ...
    def __init__(self):
        self.source: Optional[str] = None
        self.as_var: Optional[str] = None  # row variable for column content (UI-5)
        self.sort: bool = False               # UI-13: headers sort the query
        self.edit: Optional[str] = None       # UI-13: table whose cells are edited in place
        self.edit_datasource: Optional[str] = None
...
class UIColumnNode(QuantumNode):
    ...
    def __init__(self):
        ...
        self.edit: bool = True            # UI-13: edit="false" keeps it read-only
        # UI-5: a column with content draws it once per row (buttons, links).
        self.children: List[QuantumNode] = []
...
class UIAlertNode(QuantumNode, UILayoutMixin):
    ...
    def validate(self) -> List[str]:
        errors = self._validate_layout()
        # UI-1: `variant="{...}"` is resolved by the page when it renders;
        # only a literal can be checked here.
        if not _bound(self.variant) and self.variant not in ('info', 'success', 'warning', 'danger'):
...
class UISkeletonNode(QuantumNode, UILayoutMixin):
    ...
    def validate(self) -> List[str]:
        errors = self._validate_layout()
        if not _bound(self.variant) and self.variant not in ('text', 'circle', 'rect', 'card'):
...
class UIToastNode(QuantumNode, UILayoutMixin):
    ...
    def validate(self) -> List[str]:
        errors = self._validate_layout()
        if not _bound(self.variant) and self.variant not in self.VALID_VARIANTS:
            errors.append(f"Invalid toast variant: {self.variant}. Valid: {', '.join(self.VALID_VARIANTS)}")
        if not _bound(self.position) and self.position not in self.VALID_POSITIONS:
...
class UIPagerNode(QuantumNode, UILayoutMixin):
    """<ui:pager for="query"> — previous, page numbers and next over a paginated query (UI-11)."""

    def __init__(self):
        self.for_query: Optional[str] = None
        self.param: str = 'page'
        self.window: int = 2
        self.children: List[QuantumNode] = []
        self._init_layout_attrs()
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"type": "ui_pager", "for": self.for_query, "param": self.param}

    def validate(self) -> List[str]:
        return [] if self.for_query else ["ui:pager needs for="]


class UIStreamNode(QuantumNode, UILayoutMixin):
    """<ui:stream for="answer"> — a q:llm answer, arriving as it is written (IA-7)."""

    def __init__(self):
        self.for_llm: Optional[str] = None
        self.children: List[QuantumNode] = []
        self._init_layout_attrs()

    def to_dict(self) -> Dict[str, Any]:
        return {"type": "ui_stream", "for": self.for_llm}

    def validate(self) -> List[str]:
        return []


class UIHistoryNode(QuantumNode, UILayoutMixin):
    """<ui:history table="t" key="{row.id}" datasource="db"> — a row's change history (DB-11)."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"type": "ui_history", "table": self.table, "key": self.key}

    def validate(self) -> List[str]:
        return []


def is_ui_node(node) -> bool:
    """A ui:* element (UI-1): markup, drawn by a renderer, never executed."""
    return type(node).__module__ == __name__


# UI-7: the elements every renderer draws — browser, console and desktop — with
# the same meaning, checked by tests/apps/test_ui_parity_script.py. The other ui:*
# tags work in the browser only; the console says it cannot draw them.
CORE_TAGS = frozenset({
    'window', 'hbox', 'vbox', 'grid', 'panel', 'section', 'scrollbox', 'spacer', 'rule',
    'header', 'footer', 'card', 'card-header', 'card-body', 'card-footer', 'tabpanel', 'tab',
    'text', 'badge', 'alert', 'link', 'image', 'progress',
    'table', 'column', 'list', 'item', 'pager', 'history', 'stream',
    'form', 'formitem', 'input', 'checkbox', 'switch', 'radio', 'select', 'option', 'button',
})
