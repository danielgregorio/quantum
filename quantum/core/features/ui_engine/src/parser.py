...
from .ast_nodes import (
    ...
    UIValidatorNode, UIAnimateNode, UICardNode, UICardHeaderNode, UICardBodyNode, UICardFooterNode,
    ...
    UICalendarNode, UIDatePickerNode, UIPagerNode, UIHistoryNode, UIStreamNode,
)
...
class UIParser:
    ...
    def __init__(self, parent_parser, page_mode: bool = False):
        """
        Args:
        ...
        # UI-1: inside a served page, HTML children are content too. In a
        # <q:application type="ui"> build they were (and stay) ignored.
    ...
    UI_TAG_MAP = {
        ...
        'pager': '_parse_ui_pager',
        'history': '_parse_ui_history',
        'stream': '_parse_ui_stream',
        'list': '_parse_ui_list',
        'item': '_parse_ui_item',
    ...
    def parse_ui_element(self, local_name: str, element: ET.Element):
        ...
        if method_name is None:
            import difflib
            close = difflib.get_close_matches(local_name, self.UI_TAG_MAP, n=1)
            hint = f"; did you mean <ui:{close[0]}>?" if close else ""
            error = UIParseError(f"Unknown UI tag: <ui:{local_name}>{hint}")
            error.line = getattr(element, 'sourceline', None)
            raise error
        ...
        try:
            ...
            raise
        # DEV-2: a ui:* element knows its line and file, like a q: statement.
        if getattr(element, 'sourceline', None) and node is not None:
            try:
                node.source_line = element.sourceline
                node.source_file = getattr(self.parent, '_source_path', None)
    ...
    def _apply_layout_attrs(self, node, element: ET.Element):
        ...
        # UI-2: responsive attributes; a breakpoint is sm, md or lg.
        for attr, field in (('stack-below', 'stack_below'), ('hide-below', 'hide_below'),
                            ('hide-above', 'hide_above')):
            value = element.get(attr)
            if value is not None and value not in ('sm', 'md', 'lg'):
                raise UIParseError(f'{attr}="{value}": use sm (640 px), md (768 px) or lg (1024 px)')
            setattr(node, field, value)
        node.grow = element.get('grow')
    ...
    def _parse_child(self, child: ET.Element):
        ...
        elif ns == 'quantum' or self.page_mode:
    ...
    def _parse_children(self, element: ET.Element, node):
        ...
        UI-1: inside a page, a container's own text is content too
        (`<ui:card-header>Resumo</ui:card-header>`); it used to vanish. Nodes
        that keep their text in `content` read it themselves."""
        text_is_content = self.page_mode and not hasattr(node, 'content')
        if text_is_content and (element.text or '').strip():
            node.add_child(TextNode(element.text.strip()))
        for child in element:
            parsed = self._parse_child(child)
            if parsed:
                node.add_child(parsed)
            if text_is_content and (child.tail or '').strip():
                node.add_child(TextNode(child.tail.strip()))
    ...
    def _parse_ui_form(self, element: ET.Element) -> UIFormNode:
        ...
        node.send = element.get('with')
        # UI-9: rules="off" stops the form from taking its fields' rules from the action.
        node.rules = element.get('rules')
        # UI-10: an edit form opens with a row's values; the generated button's text.
        node.values = element.get('values')
        node.submit = element.get('submit')
        if node.rules not in (None, 'on', 'off'):
            raise UIParseError(f'<ui:form rules="{node.rules}">: use on (the default) or off')
    ...
    def _parse_ui_input(self, element: ET.Element) -> UIInputNode:
        ...
        # UI-12: search as you type — refresh the element with this id.
        node.search = element.get('search')
        delay = element.get('delay')
        if delay is not None and not delay.isdigit():
            raise UIParseError(f'<ui:input delay="{delay}">: milliseconds, e.g. delay="300"')
        node.delay = int(delay) if delay is not None else (300 if node.search else None)
        if node.search is not None and not node.bind:
            raise UIParseError('<ui:input search=...> needs bind= (the URL parameter it sets)')
        node.placeholder = element.get('placeholder')
        node.input_type = element.get('type', 'text')
        rows = element.get('rows')
        if rows is not None:
            if not rows.isdigit() or int(rows) < 2:
                raise UIParseError(f'<ui:input rows="{rows}">: the number of lines, 2 or more (UI-14)')
            node.rows = int(rows)
        node.accept = element.get('accept')
    ...
    def _parse_ui_checkbox(self, element: ET.Element) -> UICheckboxNode:
        node = UICheckboxNode()
        node.bind = element.get('bind')
        node.checked = element.get('checked')
        node.label = element.get('label')
        self._apply_layout_attrs(node, element)
        return node
    ...
    def _parse_ui_table(self, element: ET.Element) -> UITableNode:
        ...
        # UI-13: sortable headers and cells edited in place.
        node.sort = self._parse_bool(element.get('sort'))
        node.edit = element.get('edit')
        node.edit_datasource = element.get('datasource')
        if node.edit and not node.edit_datasource:
            raise UIParseError(f'<ui:table edit="{node.edit}"> needs datasource= (where the table is)')
        if (node.sort or node.edit) and not node.source:
            raise UIParseError('<ui:table sort/edit> needs source="{query}"')
        self._apply_layout_attrs(node, element)
    ...
    def _parse_ui_column(self, element: ET.Element) -> UIColumnNode:
        ...
        node.align = element.get('align')
        node.edit = element.get('edit', 'true') != 'false'
        self._parse_children(element, node)
        return node

    def _parse_ui_pager(self, element: ET.Element) -> UIPagerNode:
        node = UIPagerNode()
        node.for_query = element.get('for')
        if not node.for_query:
            raise UIParseError('<ui:pager> needs for="<the paginated q:query>"')
        node.param = element.get('param') or 'page'
        window = element.get('window')
        if window is not None and not window.isdigit():
            raise UIParseError(f'<ui:pager window="{window}">: the number of pages shown each side of the current one')
        node.window = int(window) if window is not None else 2
        self._apply_layout_attrs(node, element)
        return node

    def _parse_ui_stream(self, element: ET.Element) -> UIStreamNode:
        node = UIStreamNode()
        node.for_llm = element.get('for')
        if not node.for_llm:
            raise UIParseError('<ui:stream> needs for="<the q:llm name>"')
        self._apply_layout_attrs(node, element)
        return node

    def _parse_ui_history(self, element: ET.Element) -> UIHistoryNode:
        node = UIHistoryNode()
        node.table = element.get('table')
        node.key = element.get('key')
        node.datasource = element.get('datasource')
        missing = [a for a in ('table', 'key', 'datasource') if not element.get(a)]
        if missing:
            raise UIParseError(f'<ui:history> needs {", ".join(a + "=" for a in missing)}')
        limit = element.get('limit')
        if limit is not None and not limit.isdigit():
            raise UIParseError(f'<ui:history limit="{limit}">: a number of entries')
        node.limit = int(limit) if limit is not None else 50
        self._apply_layout_attrs(node, element)
        return node
