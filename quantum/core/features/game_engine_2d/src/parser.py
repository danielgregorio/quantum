...
class GameParser:
    ...
    GAME_TAG_MAP = {
        ...
        'tile': '_parse_game_hud_tile',
        'counter': '_parse_game_hud_counter',
        'collection': '_parse_game_hud_collection',
        'slot': '_parse_game_hud_slot',
        'option': '_parse_game_hud_option',
        'hud-behavior': '_parse_game_hud_behavior',
        'action': '_parse_game_action_dispatch',
        'transition': '_parse_game_scene_transition',
        'collision-layer': '_parse_game_collision_layer',
        'persistent': '_parse_game_persistent',
        'map-node': '_parse_game_map_node',
        'map-path': '_parse_game_map_path',
        'enemy': '_parse_game_enemy',
    }
    ...
    def _parse_game_scene(self, element: ET.Element) -> SceneNode:
        ...
        # Multi-scene attributes
        node.scene_type = element.get('type', 'level')
        node.initial = self._parse_bool(element.get('initial'))
        # Death / game over
        node.death_sequence = element.get('death-sequence')
        if element.get('death-timer') is not None:
            node.death_timer = self._parse_float(element.get('death-timer'), 3.0)
        node.game_over_jingle = element.get('game-over-jingle')
        node.game_over_alphabet = element.get('game-over-alphabet')
    ...
    def _parse_game_sprite(self, element: ET.Element) -> SpriteNode:
        ...
        node.shape = element.get('shape', 'box')  # box, circle, slope-left, slope-right
        ...
        # Collision layers (numeric bitmask or named layer)
        if element.get('collision-layer') is not None:
            raw = element.get('collision-layer')
            try:
                node.collision_layer = int(raw)
            except ValueError:
                # Named layer — store as string, resolve in codegen
                node.collision_layer = raw
        if element.get('collision-mask') is not None:
            raw = element.get('collision-mask')
            try:
                node.collision_mask = int(raw)
            except ValueError:
                # Named layers (comma-separated) — store as string, resolve in codegen
                node.collision_mask = raw
        if element.get('floor-max-angle') is not None:
            node.floor_max_angle = self._parse_float(element.get('floor-max-angle'))
        if element.get('spin-jump') is not None:
            node.spin_jump = self._parse_bool(element.get('spin-jump'))
    ...
    def _parse_game_physics(self, element: ET.Element) -> PhysicsNode:
        ...
        # Parse collision-layer children
        for child in element:
            local = self._get_local_name(child)
            if local == 'collision-layer':
                layer_def = self._parse_game_collision_layer(child)
                node.collision_layers[layer_def.name] = layer_def.layer_id
                node.add_child(layer_def)
        return node
    ...
    def _parse_game_hud(self, element: ET.Element) -> HudNode:
        ...
        node.background_height = self._parse_int(element.get('background-height')) if element.get('background-height') else None
        node.sprite_prefix = element.get('sprite-prefix')

        for child in element:
            ns = self._get_namespace(child)
            local = self._get_local_name(child)

            if ns == 'game' and local in self.GAME_TAG_MAP:
                parsed = self.parse_game_element(local, child)
                if parsed:
                    node.add_child(parsed)
            else:
                # Fallback: parse as HTML (backward compat)
                parsed = self._parse_child(child)
                if parsed:
                    node.add_child(parsed)
                elif hasattr(self.parent, '_parse_html_element'):
                    html_node = self.parent._parse_html_element(child)
                    if html_node:
                        node.add_child(html_node)
        return node

    def _parse_game_hud_tile(self, element: ET.Element) -> HudTileNode:
        node = HudTileNode()
        node.sprite = element.get('sprite', '')
        node.x = self._parse_int(element.get('x'))
        node.y = self._parse_int(element.get('y'))
        node.width = self._parse_int(element.get('width')) if element.get('width') else None
        node.height = self._parse_int(element.get('height')) if element.get('height') else None
        return node

    def _parse_game_hud_counter(self, element: ET.Element) -> HudCounterNode:
        node = HudCounterNode()
        node.strip = element.get('strip', '')
        node.x = self._parse_int(element.get('x'))
        node.y = self._parse_int(element.get('y'))
        node.digits = self._parse_int(element.get('digits'), 2)
        node.bind = element.get('bind', '')
        node.digit_width = self._parse_int(element.get('digit-width'), 8)
        node.digit_height = self._parse_int(element.get('digit-height'), 8)
        node.countdown = self._parse_bool(element.get('countdown'))
        node.hurry_at = self._parse_int(element.get('hurry-at')) if element.get('hurry-at') else None
        node.flash = element.get('flash')
        node.align = element.get('align', 'left')
        node.right_edge = self._parse_int(element.get('right-edge')) if element.get('right-edge') else None
        node.tint = element.get('tint')
        node.intelligent = self._parse_bool(element.get('intelligent'))
        node.icon = element.get('icon')
        node.symbol = element.get('symbol')
        node.extra_life_at = self._parse_int(element.get('extra-life-at')) if element.get('extra-life-at') else None
        return node

    def _parse_game_hud_collection(self, element: ET.Element) -> HudCollectionNode:
        node = HudCollectionNode()
        node.sprite = element.get('sprite', '')
        node.x = self._parse_int(element.get('x'))
        node.y = self._parse_int(element.get('y'))
        node.spacing = self._parse_int(element.get('spacing'), 8)
        node.max = self._parse_int(element.get('max'), 5)
        node.bind = element.get('bind', '')
        return node

    def _parse_game_hud_slot(self, element: ET.Element) -> HudSlotNode:
        node = HudSlotNode()
        node.x = self._parse_int(element.get('x'))
        node.y = self._parse_int(element.get('y'))
        node.bind = element.get('bind', '')
        for child in element:
            local = self._get_local_name(child)
            if local == 'option':
                option = self._parse_game_hud_option(child)
                node.add_option(option)
        return node

    def _parse_game_hud_option(self, element: ET.Element) -> HudOptionNode:
        node = HudOptionNode()
        node.value = element.get('value', '')
        node.sprite = element.get('sprite', '')
        return node

    def _parse_game_hud_behavior(self, element: ET.Element) -> HudBehaviorNode:
        node = HudBehaviorNode()
        node.target = element.get('target', '')
        node.event = element.get('event', '')
        node.value = self._parse_int(element.get('value')) if element.get('value') else None
        for child in element:
            local = self._get_local_name(child)
            if local == 'action':
                action = self._parse_game_hud_action(child)
                node.add_action(action)
        return node

    def _parse_game_hud_action(self, element: ET.Element) -> HudActionNode:
        node = HudActionNode()
        node.action_type = element.get('type', '')
        node.event = element.get('event')
        node.rate = self._parse_float(element.get('rate')) if element.get('rate') else None
        return node
    ...
    def _parse_game_use(self, element: ET.Element) -> UseNode:
        ...
        # A behavior often needs more than one collision handler — a patrol
        # that turns at a wall AND at a ledge. The attribute form allows one,
        # and a platformer example tried to write two by repeating
        # on-collision=, which is a duplicate attribute and killed the parse
        # of the whole file. Repeating the <qg:use> instead would instantiate
        # the behavior twice, which is not the same thing.
        #
        # So: extra handlers as children.
        #
        ...
        for child in element:
            ...
            if tag != 'on-collision':
                continue
            handler = child.get('handler') or child.get('on-collision')
            if not handler:
                continue
            collision_tag = (child.get('tag') or child.get('collision-tag')
                             or child.get('with-tag'))
            if node.on_collision is None:
                node.on_collision = handler
                node.collision_tag = collision_tag
            else:
                node.extra_collisions.append(
                    {'handler': handler, 'tag': collision_tag})
    ...
    def _parse_game_prefab(self, element: ET.Element) -> PrefabNode:
        ...
        # --- Classification ---
        node.entity_type = element.get('type')

        # --- Combat ---
        if element.get('health') is not None:
            node.health = self._parse_int(element.get('health'), 1)
        node.defeated_by = element.get('defeated-by')
        node.on_defeat = element.get('on-defeat')
        if element.get('contact-damage') is not None:
            node.contact_damage = self._parse_int(element.get('contact-damage'), 1)
        if element.get('knockback') is not None:
            node.knockback = self._parse_float(element.get('knockback'), 0.0)
        if element.get('i-frames') is not None:
            node.i_frames = self._parse_int(element.get('i-frames'), 0)
        if element.get('stomp-bounce') is not None:
            node.stomp_bounce = self._parse_float(element.get('stomp-bounce'), 0.0)

        # --- Movement ---
        node.movement = element.get('movement')
        if element.get('move-speed') is not None:
            node.move_speed = self._parse_float(element.get('move-speed'), 1.0)
        if element.get('turn-on-edge') is not None:
            node.turn_on_edge = self._parse_bool(element.get('turn-on-edge'))
        node.facing = element.get('facing')
        if element.get('turn-cooldown') is not None:
            node.turn_cooldown = self._parse_float(element.get('turn-cooldown'), 0.0)

        # --- Reward ---
        node.reward = element.get('reward')
        node.reward_kill = element.get('reward-kill')

        # --- Respawn ---
        if element.get('respawns') is not None:
            node.respawns = self._parse_bool(element.get('respawns'))
        if element.get('respawn-delay') is not None:
            node.respawn_delay = self._parse_float(element.get('respawn-delay'), 0.0)

        # --- Item ---
        node.effect = element.get('effect')
        if element.get('duration') is not None:
            node.duration = self._parse_float(element.get('duration'), 0.0)
        if element.get('auto-move') is not None:
            node.auto_move = self._parse_bool(element.get('auto-move'))
        if element.get('collectible') is not None:
            node.collectible = self._parse_bool(element.get('collectible'))
        if element.get('carryable') is not None:
            node.carryable = self._parse_bool(element.get('carryable'))

        # --- Block ---
        node.content = element.get('content')
        if element.get('hits') is not None:
            node.hits = self._parse_int(element.get('hits'), 1)
        if element.get('breakable') is not None:
            node.breakable = self._parse_bool(element.get('breakable'))
        if element.get('invisible') is not None:
            node.invisible = self._parse_bool(element.get('invisible'))
        if element.get('checkpoint') is not None:
            node.checkpoint = self._parse_bool(element.get('checkpoint'))
        if element.get('flying') is not None:
            node.flying = self._parse_bool(element.get('flying'))
    ...
    def _parse_game_event(self, element: ET.Element) -> EventNode:
        ...
        Supports both handler-reference style and inline children:
            <qg:event name="press-start" handler="onStart" />
        ...
        # Parse inline children (e.g., <qg:transition> inside <qg:event>)
        for child in element:
            parsed = self._parse_child(child)
            if parsed:
    ...
    def _parse_game_scene_transition(self, element: ET.Element) -> SceneTransitionNode:
        """Parse <qg:transition> - Scene transition.

        Example:
            <qg:transition to="world-map" effect="fade" duration="0.5" />
        """
        to = element.get('to', '')
        node = SceneTransitionNode(to)
        node.effect = element.get('effect', 'fade')
        node.duration = self._parse_float(element.get('duration'), 0.5)

        # Parse optional data attributes (data-* pattern)
        data = {}
        for attr_name, attr_val in element.attrib.items():
            if attr_name.startswith('data-'):
                data[attr_name[5:]] = attr_val
        if data:
            node.data = data

        return node

    def _parse_game_persistent(self, element: ET.Element) -> PersistentNode:
        """Parse <qg:persistent> - Persistent state block.
    ...
    def _parse_game_map_node(self, element: ET.Element) -> MapNodeDef:
        """Parse <qg:map-node> - World map node.

        Example:
            <qg:map-node id="green-island-1" x="80" y="180" scene="green-island-1" />
        """
        node_id = element.get('id', '')
        node = MapNodeDef(node_id)
        node.x = self._parse_float(element.get('x'))
        node.y = self._parse_float(element.get('y'))
        node.scene = element.get('scene')
        node.locked = self._parse_bool(element.get('locked'))
        node.icon = element.get('icon')
        return node

    def _parse_game_map_path(self, element: ET.Element) -> MapPathNode:
        """Parse <qg:map-path> - World map path between nodes.

        Example:
            <qg:map-path from="start" to="green-island-1" />
            <qg:map-path from="green-island-1" to="green-island-2" unlock="green-island-1-cleared" />
        """
        from_node = element.get('from', '')
        to_node = element.get('to', '')
        node = MapPathNode(from_node, to_node)
        node.unlock = element.get('unlock')
        node.direction = element.get('direction')
        return node
    ...
    def _parse_game_collision_layer(self, element: ET.Element) -> CollisionLayerDef:
        """Parse <qg:collision-layer> - Named collision layer definition.

        Example:
            <qg:collision-layer id="1" name="world" />
            <qg:collision-layer id="2" name="player" />
        """
        layer_id = self._parse_int(element.get('id'), 1)
        name = element.get('name', '')
        return CollisionLayerDef(layer_id, name)

    # ------------------------------------------------------------------
    # Enemy system
    # ------------------------------------------------------------------

    def _parse_game_enemy(self, element: ET.Element) -> EnemyNode:
        """Parse <qg:enemy> - Enemy behavior definition.

        Children are parsed contextually: <qg:ai> and <qg:defeat>.
        """
        prefab = element.get('prefab', '')
        node = EnemyNode(prefab)

        for child in element:
            local = self._get_local_name(child)
            if local == 'ai':
                node.ai = self._parse_enemy_ai(child)
            elif local == 'defeat':
                node.defeat = self._parse_enemy_defeat(child)

        return node

    def _parse_enemy_ai(self, element: ET.Element) -> EnemyAiNode:
        """Parse <qg:ai> - Enemy AI behavior."""
        node = EnemyAiNode()
        node.ai_type = element.get('type', 'patrol')
        node.speed = self._parse_float(element.get('speed'), 30.0)
        node.turn_cooldown = self._parse_float(element.get('turn-cooldown')) if element.get('turn-cooldown') else None
        node.turn_on_edge = self._parse_bool(element.get('turn-on-edge'))
        node.facing = element.get('facing', 'left')
        return node

    def _parse_enemy_defeat(self, element: ET.Element) -> EnemyDefeatNode:
        """Parse <qg:defeat> - How enemy can be defeated.

        Children: <qg:on-hit>, <qg:on-kill>.
        """
        node = EnemyDefeatNode()
        node.by = element.get('by', 'stomp')
        node.health = self._parse_int(element.get('health'), 1)
        node.bounce = self._parse_float(element.get('bounce')) if element.get('bounce') else None

        for child in element:
            local = self._get_local_name(child)
            if local == 'on-hit':
                node.on_hit = self._parse_enemy_on_hit(child)
            elif local == 'on-kill':
                node.on_kill = self._parse_enemy_on_kill(child)

        return node

    def _parse_enemy_on_hit(self, element: ET.Element) -> EnemyOnHitNode:
        """Parse <qg:on-hit> - Reward/effect on non-lethal hit."""
        node = EnemyOnHitNode()
        node.score = self._parse_int(element.get('score'))
        node.effect = element.get('effect')
        node.speed_multiply = self._parse_float(element.get('speed-multiply')) if element.get('speed-multiply') else None
        return node

    def _parse_enemy_on_kill(self, element: ET.Element) -> EnemyOnKillNode:
        """Parse <qg:on-kill> - Reward/effect on kill."""
        node = EnemyOnKillNode()
        node.score = self._parse_int(element.get('score'))
        node.effect = element.get('effect')
        node.drop = element.get('drop')
        return node

    # ------------------------------------------------------------------
    # Event action dispatch (qg:action can be HUD or Event context)
    # ------------------------------------------------------------------

    def _parse_game_action_dispatch(self, element: ET.Element):
        """Dispatch <qg:action> based on context attributes.

        If element has 'type' in EventActionNode.VALID_ACTION_TYPES -> EventActionNode
        Otherwise fall back to HudActionNode (legacy behavior).
        """
        action_type = element.get('type', '')
        if action_type in EventActionNode.VALID_ACTION_TYPES:
            return self._parse_game_event_action(element)
        return self._parse_game_hud_action(element)

    def _parse_game_event_action(self, element: ET.Element) -> EventActionNode:
        """Parse <qg:action> inside <qg:event> - Inline event action.
        ...
        """
        node = EventActionNode()
        node.action_type = element.get('type', '')
        node.target = element.get('target')
        node.value = element.get('value')
        if element.get('amount') is not None:
            node.amount = self._parse_int(element.get('amount'))
        return node
