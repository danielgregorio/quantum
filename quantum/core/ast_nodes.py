...
from .features.conditionals.src.ast_node import IfNode  # noqa: F401 -- re-exported: quantum/compiler imports it from here
...
class ApplicationNode(QuantumNode):
    ...
    def __init__(self, app_id: str, app_type: str):
        ...
        # q:set and q:function written directly in <q:application>.
        #
        # There was nowhere to keep them, and the per-engine parsers only
        # looked at children of their OWN namespace (qt:, qg:, ui:), so a
        #
        #     <q:application type="terminal">
        #       <q:function name="save"> ... </q:function>
        #
        # was dropped without a word. The terminal generator only found
        # functions declared INSIDE a <qt:screen>, and the on-click that
        # pointed to the global function fell into the "unknown action"
        # branch, becoming a `self.notify("Action: save")` — the button
        # announced the action instead of running it.
        self.functions: List[QuantumNode] = []   # FunctionNode list
        self.state_vars: List[QuantumNode] = []  # SetNode list
        ...
        self.enemies: List[QuantumNode] = []     # EnemyNode list
        self.persistent: List[QuantumNode] = []  # PersistentNode children (SetNodes)
...
class JobNode(QuantumNode):
    ...
    def validate(self) -> List[str]:
        ...
        if self.priority < 0 or self.priority > 10:
            errors.append("Job priority must be between 0 and 10")
        for param in self.params:
            errors.extend(param.validate())
...
# Import from new location (Option C migration)
from .features.loops.src.ast_node import LoopNode  # noqa: F401 -- re-exported: quantum/compiler imports it from here

# MIGRATED: FunctionNode and RestConfig moved to feature-based structure
# Import from new location (Option C migration)
from .features.functions.src.ast_node import FunctionNode
...
# Import from new location (Option C migration)
from .features.state_management.src.ast_node import SetNode  # noqa: F401 -- re-exported: quantum/compiler imports it from here
...
class QueryNode(QuantumNode):
    ...
    def __init__(self, name: str, datasource: str, sql: str, params: List['QueryParamNode'] = None):
        ...
        self.page = None
        self.page_expr = None  # DB-9: page as an expression (default {query.page})
        self.on_error = 'fail'  # IA-5: 'continue' hands a failure to <name>_result
        self.sortable = False  # UI-13: ORDER BY the URL's ?sort=/&dir=
        self.page_size = 20
        self.timeout = None
        self.maxrows = None
    ...
    def validate(self) -> List[str]:
        errors = []
        # name is optional: a write query (UPDATE/INSERT/DELETE) has no result
        # set to bind. Reads without a name simply cannot be referenced.
...
# q:invoke's nodes live in the invocation feature (the parser builds them from
# there); re-exported so every node can be imported from this module. The
# q:data nodes (DataNode and its children) are defined in this module, and the
# data_import feature re-exports them: one class per node.
from .features.invocation.src.ast_node import InvokeNode, InvokeHeaderNode  # noqa: F401 -- re-export
...
class ImportNode(QuantumNode):
    ...
    Game Engine Imports (classic Componentization):
      <q:import behavior="PlayerBehavior" from="./behaviors" />
      <q:import prefab="ShellWalkerGreen" from="./prefabs" />
...
class ActionNode(QuantumNode):
    ...
    def __init__(
        ...
        self.params: List[QuantumParam] = []
        self.body: List[QuantumNode] = []
        # UI-10 (M17): params from a table's schema — see quantum/runtime/table_params.py
        self.table: Optional[str] = None
        self.table_datasource: Optional[str] = None
        self.table_columns: Optional[List[str]] = None
...
class LLMNode(QuantumNode):
    ...
    def __init__(self, name: str):
        ...
        self.on_error = 'fail'          # IA-5: 'continue' hands a failure to <name>_result
        self.knowledge = None           # IA-6: answer from this knowledge base, citing sources
        self.top = 4                    # IA-6: how many sources to retrieve
        self.min_relevance = 0.0        # IA-9: chunks below this relevance are not retrieved
        self.stream = False             # IA-7: stream the answer to <ui:stream> (web requests)
        self.temperature = None         # 0.0-2.0
        self.max_tokens = None          # Token limit
        self.response_format = None     # "text" or "json"
        self.cache = False
        self.ttl = None                 # Cache TTL in seconds (None = no expiry)
        self.provider = None            # ollama, openai, anthropic, auto
        self.api_key = None             # For cloud providers
        self.timeout = 30
...
class DataNode(QuantumNode):
    ...
    def validate(self) -> List[str]:
        ...
        for data_field in self.fields:
            errors.extend(data_field.validate())
