...
class ApplicationNode(QuantumNode):
    ...
    def __init__(self, app_id: str, app_type: str):
        ...
        # q:set e q:function escritos direto no <q:application>.
        #
        # Nao havia onde guarda-los, e os parsers por engine so olhavam os
        # filhos do PROPRIO namespace (qt:, qg:, ui:), entao um
        #
        #     <q:application type="terminal">
        #       <q:function name="salvar"> ... </q:function>
        #
        # era descartado sem uma palavra. O gerador do terminal so
        # encontrava funcoes declaradas DENTRO de um <qt:screen>, e o
        # on-click que apontava para a funcao global caia no ramo "acao
        # desconhecida", virando um `self.notify("Action: salvar")` — o
        # botao anunciava a acao em vez de executa-la.
