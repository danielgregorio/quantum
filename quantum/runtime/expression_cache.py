...
class ExpressionCache:
    ...
    def evaluate(self, expr: str, context: Dict[str, Any]) -> Any:
        ...
        start_time = time.perf_counter() if self._enable_stats else 0
        hits_before = self._compile_cached.cache_info().hits if self._enable_stats else 0
        ...
        try:
            ...
            if self._enable_stats:
                elapsed = (time.perf_counter() - start_time) * 1000
                # The lru_cache hit counter advances when the compiled code was
                # served from cache; mirror that into our stats as a hit/miss.
                was_hit = self._compile_cached.cache_info().hits > hits_before
                with self._lock:
