...
class MessageQueueService:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # The message being processed, PER THREAD.
        #
        # It was a single instance attribute, written by the callback of EVERY
        # subscription — and each subscription runs on its own polling thread.
        # With two active subscriptions, the `ack()` called inside handler A
        # confirmed the message handler B had just claimed: A's went back to
        # the queue and B's vanished without having been processed.
        # The slower the handler, the bigger the window.
        self._local = threading.local()

    def _get_broker_type(self) -> str:
        """Get broker type from config or environment.

        The default is `sqlite`, not `memory`. The in-memory broker lives inside
        ONE interpreter: a publish in this process is invisible to any other,
        and everything queued dies with it — under gunicorn, one private queue
        per worker. That is silent data loss as a default.

        sqlite is the only point of the spectrum that is durable AND
        serverless. It adds no dependency (it is in the stdlib) and no
        process; the file sits next to `quantum_jobs.db`, which the job queue
        already uses the same way.

        `MESSAGE_BROKER_TYPE=memory` goes back to the previous behaviour —
        legitimate for tests and for a single-process app that wants no file
        at all.
        """
    ...
    def _current_message(self) -> Optional[Message]:
        """This thread's message. Kept as a property because the name is
        used by tests and by code that reads the service's state."""
        return getattr(self._local, 'current_message', None)

    def ack(self) -> None:
        """Acknowledge the current message (for manual ack mode)."""
        message = self._current_message
        if message and self._broker:
    ...
    def nack(self, requeue: bool = True) -> None:
        ...
        message = self._current_message
        if message and self._broker:
            self._broker.nack(message, requeue)
