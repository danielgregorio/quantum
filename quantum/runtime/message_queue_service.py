...
from quantum.runtime.message_broker import Message, QueueInfo, MessageBrokerError
from quantum.runtime.adapters import get_adapter, MemoryAdapter

logger = logging.getLogger('quantum.messaging')
...
class MessageQueueService:
    ...
    Configuration (environment variables):
        MESSAGE_BROKER_TYPE: 'memory', 'sqlite', 'redis' or 'rabbitmq'
            (default: memory — in ONE process only; nothing published
            crosses a process boundary or survives a restart. 'sqlite'
            is the durable option that needs no server.)
        QUANTUM_MQ_PATH: sqlite database file (default: quantum_messages.db)
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # A mensagem em processamento, POR THREAD.
        #
        # Era um atributo de instancia unico, escrito pelo callback de TODA
        # assinatura — e cada assinatura roda na propria thread de polling.
        # Com duas assinaturas ativas, o `ack()` chamado dentro do handler A
        # confirmava a mensagem que o handler B tinha acabado de reivindicar:
        # a de A voltava para a fila e a de B sumia sem ter sido processada.
        # Quanto mais lento o handler, maior a janela.
        self._local = threading.local()

    def _get_broker_type(self) -> str:
        """Get broker type from config or environment.

        O padrao e `sqlite`, nao `memory`. O broker em memoria vive dentro de
        UM interpretador: uma publicacao neste processo e invisivel para
        qualquer outro, e tudo enfileirado morre com ele — sob gunicorn, uma
        fila privada por worker. Isso e perda silenciosa de dados como
        default.

        sqlite e o unico ponto do espectro que e duravel E sem servidor. Nao
        adiciona dependencia (esta na stdlib) e nao adiciona processo; o
        arquivo fica ao lado de `quantum_jobs.db`, que a fila de jobs ja usa
        do mesmo jeito.

        `MESSAGE_BROKER_TYPE=memory` volta ao anterior — legitimo para teste e
        para um app de processo unico que nao quer arquivo nenhum.
        """
        return self._config.get(
            'broker_type',
            os.getenv('MESSAGE_BROKER_TYPE', 'sqlite')
        )
    ...
    def _get_broker_config(self) -> Dict[str, Any]:
        ...
        if broker_type == 'sqlite':
            return {
                'path': os.getenv('QUANTUM_MQ_PATH', 'quantum_messages.db'),
            }
    ...
    def connect(self) -> None:
        ...
        if broker_type == 'memory':
            # The in-memory broker lives inside ONE interpreter. A publish in
            # this process is invisible to any other, and everything queued is
            # gone when it exits — under gunicorn that means one private queue
            # per worker. It is the right default for a single-process app and
            # a silent data loss anywhere else, so say which one this is.
            logger.warning(
                "q:message is using the in-memory broker: messages do not "
                "cross processes and do not survive a restart. Set "
                "MESSAGE_BROKER_TYPE=sqlite (with QUANTUM_MQ_PATH) for a "
                "durable one, or redis/rabbitmq."
            )
    ...
    def subscribe(
        ...
        def message_callback(msg: Message):
            ...
            self._local.current_message = msg
            ...
            try:
                ...
                self._local.current_message = None
    ...
    @property
    def _current_message(self) -> Optional[Message]:
        """A mensagem desta thread. Mantido como propriedade porque o nome
        e usado por testes e por codigo que le o estado do servico."""
        return getattr(self._local, 'current_message', None)

    def ack(self) -> None:
        """Acknowledge the current message (for manual ack mode)."""
        mensagem = self._current_message
        if mensagem and self._broker:
            self._broker.ack(mensagem)
    ...
    def nack(self, requeue: bool = True) -> None:
        ...
        mensagem = self._current_message
        if mensagem and self._broker:
            self._broker.nack(mensagem, requeue)
