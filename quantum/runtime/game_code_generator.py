...
from quantum.runtime.game_templates import (
    ...
    emit_tween_system, emit_particle_system,
...
class GameCodeGenerator:
    ...
    def _emit_enemy_semantics(self, js: JsBuilder, info: dict, sem: dict):
        ...
        if movement == 'patrol':
            js.line(f"game.setPatrolAI({sid_str}, {js_number(effective_speed)});")
        elif movement == 'chase':
    ...
    def _emit_block_semantics(self, js: JsBuilder, info: dict, sem: dict):
        ...
        if breakable:
            js.line("game.destroySprite(self.id);")
            js.line("game.emit('block-broken', { self: self.id, other: other.id, id: self.id });")
        else:
    ...
    def _emit_goal_semantics(self, js: JsBuilder, info: dict, sem: dict):
        ...
        js.line(f"_sprites[{sid_str}].collisionHandlers.push((self, other) => {{")
        js.indent()
        js.line("game.emit('level-complete', { self: self.id, other: other.id });")
        js.dedent()
        js.line("});")
        js.blank()
    ...
    def _emit_input_update_loop(self, js: JsBuilder):
        for info in self._sprites:
            if info.get('controls'):
                ...
                # classic-style physics parameters
                ...
                if ctrl in ('wasd', 'arrows'):
                    ...
                    # classic-style jump physics
                    js.comment("classic-style jump: coyote time, variable height, asymmetric gravity")
