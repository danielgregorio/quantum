...
# ==========================================================================
# 16-bit console hardware constants
# ==========================================================================
# Source: console dev documentation and frame analysis
...
RETRO_TILE = 16
RETRO_FPS = 60
RETRO_WINDOW_SCALE = 3  # 3x upscale → 768×672 window

# ==========================================================================
# classic Physics Constants (for Godot: px/s and px/s² at 60fps)
# ==========================================================================
# Derived from classic platformer frame analysis + frame analysis:
...
CLASSIC_WALK_SPEED = 90.0       # px/s (1.5 px/frame)
CLASSIC_JUMP_VELOCITY = -280.0  # px/s (initial upward, negative = up)
CLASSIC_GRAVITY = 750.0         # px/s² (project default)
CLASSIC_GRAVITY_RISING = 600.0  # px/s² (holding jump, lighter feel)
CLASSIC_GRAVITY_FALLING = 1050.0  # px/s² (falling, snappy)
CLASSIC_MAX_FALL = 240.0        # px/s (4 px/frame terminal velocity)
CLASSIC_JUMP_HOLD_BOOST = 50.0  # px/s² (extra lift while holding jump)
CLASSIC_COYOTE_FRAMES = 6       # frames of grace after leaving edge

# Validation: jump height = v² / (2 * g_rise) = 280² / (2*600) = 65px ≈ 4 tiles ✓
# With hold boost over 0.4s rise: ~80px ≈ 5 tiles ✓ (classic standard)

# ==========================================================================
# Legacy scale helpers (kept for backward compat with tests)
# ==========================================================================

QUANTUM_SCALE = 100  # Legacy — NOT used for positions or retro console physics
...
def build_project_godot(config: Dict[str, Any]) -> str:
    ...
    lines.append('')
    vp_width = config.get('viewport_width', RETRO_WIDTH)
    vp_height = config.get('viewport_height', RETRO_HEIGHT)
    ...
    scale = config.get('window_scale', RETRO_WINDOW_SCALE)
...
## Player Controller - classic-style platformer physics
...
\t\t\t}})
\t\t\tbreak
\t\telif tag == "item_block" and normal.y > 0.5:
\t\t\tif collider.has_method("hit"):
\t\t\t\tcollider.hit()
\t\t\tQuantumEventBus.emit_event("item_block-hit", {{
\t\t\t\t"other": collider,
\t\t\t\t"normal_y": normal.y
\t\t\t}})
...
\t# classic spritesheets face LEFT by default, flip when facing right
...
func _ready():
\t_player_sprite = $Player if has_node("Player") else null
\tif map_data.has("start_node"):
\t\tcurrent_node = map_data["start_node"]
\tif _player_sprite and map_data.has("nodes") and map_data["nodes"].has(current_node):
\t\tvar node_info = map_data["nodes"][current_node]
\t\t_player_sprite.position = Vector2(node_info["x"], node_info["y"])
...
\tif _player_sprite:
\t\tvar tween = create_tween()
\t\ttween.tween_property(_player_sprite, "position", target_pos, 0.2)
\t\tawait tween.finished
...
GAME_OVER_SCREEN_GD = '''\
extends CanvasLayer
## Game Over Screen - classic-style slide-in animation
