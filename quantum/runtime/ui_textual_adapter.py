...
from quantum.core.features.ui_engine.src.ast_nodes import (
    ...
    UIListNode, UIItemNode, UIImageNode,
...
from quantum.runtime.terminal_templates import PyBuilder, py_string
...
def _q_stepper_apply(self, sid: str) -> None:
    ...
    for position in range(len(steps)):
        # The same rule as the html adapter: the explicit mark wins; without it,
        # the step is completed if it is behind the current one.
...
def _q_truthy(self, value) -> bool:
    """The same notion of true that the html target uses in a binding."""
...
def _q_set_state(self, name: str, value) -> None:
    """The single point where state bound by bind= is written.

    The widgets wrote straight into self._q_state, so nothing could REACT to
    a change — and that is why a ui:toast's `show=` could only become a
    comment. With a single point, whoever needs to observe, observes.
    """
    self._q_state[name] = value
    react = getattr(self, '_q_toast_on_state', None)
...
class UITextualAdapter:
    ...
    def _layout_css_rules(self, node, widget_id: str) -> Optional[str]:
        ...
        if hasattr(node, 'border') and node.border:
            ...
            rules.append("border: solid $primary")
    ...
    def _render_scrollbox(self, node: UIScrollBoxNode, py: PyBuilder):
        ...
        if attrs_str:
            ...
            py.line('with ScrollableContainer():')
        py.indent()
        self._render_children(node.children, py)
        py.dedent()
    ...
    def _render_radio(self, node: UIRadioNode, py: PyBuilder):
        ...
        attrs = self._get_widget_attrs(node, widget_id)
        self._format_widget_attrs(attrs)
    ...
    def _render_select(self, node: UISelectNode, py: PyBuilder):
        ...
        self._get_widget_attrs(node, widget_id)
    ...
    def _render_stepper(self, node: UIStepperNode, py: PyBuilder):
        ...
        # The SAME rule as the html adapter. Here there was a third version —
        # `index < current or _as_bool(step.completed, False)` — that ignored an
        # explicit `completed="false"`, because _as_bool collapses "not set"
        # and "set to false".
        completed = set(completed_step_indices(steps, current))
        ...
        for index, step in enumerate(steps):
            # The html adapter uses `step.icon if step.icon else str(i + 1)` in the
            # indicator. Here the icon was silently dropped — neither in the
            # indicator nor in the panel. A documented attribute that did
            # nothing, with no warning at all.
            mark = step.icon if step.icon else f'{index + 1}'
            label = mark
        ...
        # `completed` is tri-state — not set, "true", "false" — and the explicit
        # mark does not change when the user navigates, while the positional one
        # does. Keeping a single boolean per step would lose the difference, which is
        # exactly what happened: _q_stepper_apply did
        # `set_class(position < index, "q-step-done")` and erased, after the
        # mount, what compose() had drawn from `completed`.
        # The attribute had no effect at all.
        explicit = {}
        for i, child in enumerate(steps):
            if getattr(child, 'completed', None) is not None:
                explicit[i] = self._as_bool(child.completed, False)
    ...
    def _build_methods(self) -> str:
        ...
        if self._toasts:
            ...
            if not shown:
                py.line('return')
            # `show=` names a state variable, and the binding became a
            # COMMENT: `self.q_toast_1()  # show="hasError"`. The toast
            # fired unconditionally — a <ui:toast show="hasError">
            # appeared on every launch of the app, error or not. The
            # html adapter treats it as "hidden until the variable becomes
            # true"; the same contract holds here.
            conditional = []
            for toast in shown:
                if toast['show']:
                    conditional.append(toast)
            ...
            # And when the variable changes AFTER the mount, the toast shows —
            # otherwise "hidden until it becomes true" would become "never".
            py.line('def _q_toast_on_state(self, name: str) -> None:')
            py.indent()
            py.line('"""Fire the toasts whose show= has just become true."""')
            if not conditional:
                py.line('return')
            else:
                py.line('if not self._q_truthy(self._q_state.get(name)):')
                py.line('    return')
                py.line('shown = getattr(self, "_q_toasts_shown", None)')
                py.line('if shown is None:')
                py.line('    shown = self._q_toasts_shown = set()')
                for toast in conditional:
                    name_literal = py_string(toast['show'])
                    method_literal = py_string(toast['method'])
                    py.line(f'if name == {name_literal} and {method_literal} not in shown:')
                    py.line(f'    shown.add({method_literal})')
                    py.line(f'    self.{toast["method"]}()')
            py.dedent()
            py.blank()
