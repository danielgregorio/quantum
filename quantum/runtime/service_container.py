...
if TYPE_CHECKING:
    ...
    from quantum.runtime.websocket_service import WebSocketService
...
def load_project_config(config_path: str = _DEFAULT_CONFIG_FILE) -> Dict[str, Any]:
    ...
    _config_cache.clear()      # one entry is enough; there is only one file
...
class ServiceContainer:
    ...
    def llm(self) -> 'LLMService':
        ...
        if 'llm' not in self._services:
            ...
            self._services['llm'] = LLMService(
                ...
                default_model=self._configured_llm_model(),
    ...
    def _configured_llm_model(self):
        """The model an AI tag uses when it declares none (IA-1): QUANTUM_LLM_DEFAULT_MODEL,
        then llm.model (llm.default_model, the older spelling), else None."""
        llm_cfg = self._config.get('llm', {}) or {}
        return (os.environ.get('QUANTUM_LLM_DEFAULT_MODEL') or llm_cfg.get('model')
                or llm_cfg.get('default_model') or None)

    def llm_model(self, declared, tag: str) -> str:
        """The model for an AI tag: the one it declares, else the configured one.

        There used to be a literal: `node.model or 'phi3'` in q:llm and a 'phi3'
        default in the q:agent parser, so QUANTUM_LLM_DEFAULT_MODEL — read by
        the services — never reached a tag, and a program without model= asked
        a server for a model it may not have.
        """
        model = declared or self._configured_llm_model()
        if not model:
            raise ValueError(f'{tag} names no model and none is configured: declare model= on the tag, '
                             f'or set llm.model in quantum.config.yaml or the '
                             f'QUANTUM_LLM_DEFAULT_MODEL environment variable')
        return model
    ...
    def python_scripting_enabled(self) -> bool:
        ...
        **Off by default.** It used to be on, which made it the framework's
        last fail-open setting: running a .q you did not write was enough to
        run its Python. A default has to be safe for whoever has not read the
        documentation — and whoever uses q:python has read it, because they
        had to write the tag.

        Turn it on with `security.python_scripting: true` in quantum.config.yaml,
        or QUANTUM_PYTHON_SCRIPTING=1 in the environment.
        """
    ...
    def email(self) -> 'EmailService':
        ...
        if 'email' not in self._services:
            from quantum.runtime.email_service import EmailService
            self._services['email'] = EmailService(self._config)
    ...
    def job_executor(self) -> 'JobExecutor':
        ...
        if 'job_executor' not in self._services:
            # The process's executor, not this container's: a container lives
            # for one request, and its worker threads used to outlive it.
            from quantum.runtime.job_executor import shared_job_executor
            self._services['job_executor'] = shared_job_executor(
                self._config.get('job_db_path', 'quantum_jobs.db'),
                self._config.get('max_thread_workers', 10))
