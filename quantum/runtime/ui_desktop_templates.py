...
{state_init}
    ...
    def set_state(self, args):
        """Two-way binding from JS inputs.

        Chamava-se `__set_state`. Dois underscores dentro de uma classe
        acionam o name mangling do Python: o metodo passa a existir como
        `_QuantumAPI__set_state`, e o pywebview so expoe nomes publicos. O JS
        gerado chamava `__quantumCall('__set_state', ...)`, que caia no
        `console.warn('function not found')` — em silencio, do lado do
        usuario. Ou seja: TODO input com `bind=` no alvo desktop nunca
