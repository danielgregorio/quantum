...
from quantum.runtime.dev_panel import note_query


def _register_sqlite_date_adapters() -> None:
    """Store dates the way sqlite3's default adapters did (ISO text).

    Python 3.12 deprecated those defaults; registering the same conversion
    keeps what is written identical and silences the warning at its source.
    """
    import datetime
    import sqlite3
...
_register_sqlite_date_adapters()
...
class DatabaseService:
    ...
    def get_datasource_config(self, datasource_name: str) -> Dict[str, Any]:
        ...
        if datasource_name in self.local_datasources:
            ...
            return {
                ...
                'history': bool(local_cfg.get('history')),        # M22 (DB-11)
            }

        # DB-7: a datasource is declared in quantum.config.yaml, or it does not
        # exist. This used to fall back to an HTTP call to the old admin API
        # on localhost:8000 — a network request hidden in every typo of a
        # datasource name, with a 5 second timeout before the error.
        declared_names = ', '.join(sorted(self.local_datasources)) or 'none'
        raise DatabaseConnectionError(
            f"Datasource '{datasource_name}' is not declared in quantum.config.yaml "
    ...
    def _create_sqlite_connection(self, config: Dict[str, Any]):
        ...
        if not db_path:
            raise DatabaseConnectionError(
                f"Datasource '{config.get('name')}' (sqlite) has no database: path")
        if db_path == ':memory:':
            conn = sqlite3.connect(db_path, timeout=30.0)
        else:
            # DB-7: open an EXISTING file. sqlite3.connect creates a missing
            # one, so a wrong path (or a key it does not read, like
            # sqlite_path:) gave an empty database and "no such table".
            db_file = Path(db_path)
            if not db_file.is_file():
                raise DatabaseConnectionError(
                    f"Datasource '{config.get('name')}': SQLite database {db_path!r} does not "
                    f"exist ({db_file.resolve()}). Relative paths are relative to the folder "
                    f"the server runs in. Create it with `quantum migrate up`, or fix "
                    f"datasources.{config.get('name')}.database.")
            conn = sqlite3.connect(f"file:{db_file.resolve().as_posix()}?mode=rw", uri=True, timeout=30.0)
    ...
    def execute_query(self, datasource_name: str, sql: str, params: Dict[str, Any] = None) -> QueryResult:
        ...
        try:
            ...
            # M22 (DB-11): with history on, an action's write records the rows it touches.
            capture = None
            if config.get('history') and db_type == 'sqlite':
                from quantum.runtime import history
                capture = history.begin(conn, sql, params,
                                        lambda q, p: self._prepare_query(q, p, db_type))
            ...
            if cursor.description:
                ...
                if capture is not None:
                    # Before the commit: the write and its history go together.
                    from quantum.runtime import history
                    history.finish(conn, capture, getattr(cursor, 'lastrowid', None))
            ...
            # M12: the /_dev panel shows each query of the request (no-op off debug).
            note_query(datasource_name, sql, params, execution_time, record_count)
            ...
            if not in_transaction:
                try:
                    ...
                    pass
            note_query(datasource_name, sql, params, (time.time() - start_time) * 1000, None, error=str(e))
