...
class SqliteAdapter(MessageBroker):
    ...
    # How long before a claimed message is considered abandoned. Without
    # this, a consumer that dies between claiming and confirming traps the
    # message in 'inflight' FOREVER: it does not go back to the queue, does
    # not go to the DLQ and does not show up in the count. It vanishes.
    VISIBILITY_TIMEOUT = 300.0     # seconds

    def _reclaim_abandoned(self, conn, destination: str, kind: str) -> None:
        """Put back on the queue whatever stayed claimed for too long."""
    ...
    def _poll_loop(self, sub_id: str, destination: str, kind: str,
        ...
        while self._running and sub_id in self._subs:
            ...
            try:
                handler(msg)
                # `done` only when the handler decided NOTHING. This marked
                # done unconditionally, so a handler that called
                # `nack(msg, requeue=True)` — "put it back on the queue" — saw
                # the message DESTROYED right after: the nack set it to
                # 'ready' and the next line set it to 'done'. Data loss in
                # the broker that is the default.
    ...
    def _finish_if_untouched(self, message_id: str) -> None:
        """Confirm the message only if the handler has not moved it.

        `WHERE state = 'inflight'` is the guard: if the handler called ack (done)
        or nack (ready, or moved to the DLQ), the state has already changed and
    ...
    def declare_queue(self, name: str, durable: bool = True,
        ...
        The parameter was called `dlq`, and the base class calls it
        `dead_letter_queue`. MessageQueueService passes the base's name, so
        NO queue could be declared by the service: TypeError. And since this
        adapter became the default broker, the DLQ I had just wired up was
        unreachable through the normal path.

        `dlq=` is still accepted because the tests and the internal code use it.
        `exclusive` and `auto_delete` are accepted and IGNORED: the service
        always passes them, and there is no exclusive connection or ephemeral
        queue in a file broker. Raising TypeError for them prevented declaring
        any queue; pretending to implement them would be worse.
        """
