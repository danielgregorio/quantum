...
# The package path first, as the other adapters do. Only the bare
# `message_broker` import (for running this file from its own folder) was
# here, so the module never imported from the installed package: asking for
# the redis broker said "requires 'redis' package" even with redis installed.
try:
    from quantum.runtime.message_broker import (
        MessageBroker, Message, QueueInfo, MessageHandler,
        ConnectionError, PublishError,
        QueueError
    )
except ImportError:
