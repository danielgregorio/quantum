...
class JobQueueService:
    ...
    def __init__(self, db_path: str = "quantum_jobs.db"):
        ...
        self._running = False
        # Set to stop the workers; they wait on it instead of sleeping, so a
        # stop does not have to outlast a poll interval.
    ...
    def start_worker(self, queue: str = 'default', poll_interval: float = 1.0):
        ...
        def worker_loop():
            ...
            while not stop.is_set():
                try:
                    ...
                    if job:
                        # False means the job went back to the queue (no
                        # handler here). Wait anyway, or the loop spins on
                        # the same row at full speed.
                        if self._process_job(job) is False:
        ...
        worker = threading.Thread(target=worker_loop, daemon=True, name=f'quantum-job-worker-{queue}')
        worker.start()
        self._workers[queue] = worker
        logger.info(f"Started worker thread for queue '{queue}'")

    def stop_workers(self, timeout: float = 10.0):
        """Stop the worker threads and wait for them (a job in progress finishes first).

        It only lowered a flag: a worker asleep between polls, or in the middle
        of a job, went on after the stop — and logged into a closed stream once
        the program (or the test) that started it had ended.
        """
        self._running = False
        self._stop.set()
        ...
        if any(w.is_alive() for w in workers):
            logger.warning("A job worker did not stop within %ss", timeout)
    ...
    def _process_job(self, job: JobInfo):
        ...
        if not handler:
            # Return it to the queue rather than failing it.
            #
            # Handlers are registered by EXECUTING the .q file that declares
            # <q:job>. A standalone `quantum jobs worker start` process has an
            # empty handler table, so this used to mark every pending job
            # 'failed' on the first poll — permanently destroying queued work
            # that another worker (or the same one, after loading the app)
            # could have run. A worker that cannot do a job has not learned
            # that the job is bad.
            logger.warning(
                "No handler registered for job '%s' (id=%s) in this process; "
                "leaving it pending. Run the .q that declares <q:job name=\"%s\">, "
                "or start the worker from that application.",
    ...
    def _release_job(self, job_id: int):
        """Put a job the worker locked back into 'pending'.

        _fetch_next_job marks a job 'running' before handing it over. If this
        process cannot run it, the row must go back to 'pending' or it is
        stranded in 'running' forever.
        """
        conn = self._get_connection()
        try:
            conn.execute(
                "UPDATE quantum_jobs SET status = 'pending', started_at = NULL "
...
# ============================================
# ONE EXECUTOR PER PROCESS (per job database)
# ============================================
#
# The web server builds a runtime — and a service container — per request
# (ACT-10). Each container used to build its own JobExecutor, so every request
# to a page with q:job started one more queue worker, and one with q:schedule
# one more APScheduler thread running the same schedule again; nothing ever
# stopped them. The executor is the process's, like the queue it serves.

_shared_executors: Dict[Tuple[str, int], 'JobExecutor'] = {}
_shared_lock = threading.Lock()


def shared_job_executor(job_db_path: str = "quantum_jobs.db", max_thread_workers: int = 10) -> 'JobExecutor':
    """The process's JobExecutor for this job database (a new one if it was shut down)."""
    key = (str(Path(job_db_path).resolve()) if job_db_path != ':memory:' else job_db_path, max_thread_workers)
    with _shared_lock:
        executor = _shared_executors.get(key)
        if executor is None or executor.closed:
            executor = JobExecutor(max_thread_workers=max_thread_workers, job_db_path=job_db_path)
            _shared_executors[key] = executor
        return executor


def shutdown_job_executors() -> None:
    """Stop every shared executor's threads, waiting for them; runs at exit."""
    with _shared_lock:
...
atexit.register(shutdown_job_executors)
...
class JobExecutor:
    ...
    def shutdown(self, wait: bool = True):
        """Stop the scheduler, the thread pool and the queue workers, waiting for them."""
        self.closed = True
