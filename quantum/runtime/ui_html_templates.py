...
TOAST_JS = r"""
// RAW string on purpose: it was a NON-raw triple-quoted string, so Python
// collapsed the \' and the JS came out as `dismiss('' + id + '')` — a syntax
// error that took down THIS WHOLE MODULE in the browser. The result was
// __quantumToast / __quantumCalendar undefined: no toast closed and
// no calendar navigated, on any page, ever.
<script>
...
window.__quantumCalendar = (function() {
    ...
    function init(id, options) {
        ...
        // The initial `value=` applies to all THREE modes.
        //
        // rangeStart, rangeEnd and multiple were always null/[], and `selected`
        // got `parseDate(options.value)` with the WHOLE string - which in
        // range/multiple is "2026-01-05,2026-01-10" and becomes Invalid Date. The
        // server rendered the right selection and, on load, this init
        // erased it: the calendar showed for an instant with the dates
        // marked and then went empty.
        ...
        // And the view opens on the month of the first chosen date, not on the
        // current month: with value="2020-03-15" the calendar opened on today and
        // the selection stayed invisible until the user navigated there.
        ...
        function parseDate(str) {
            ...
            // `new Date("2026-01-05")` is read as MIDNIGHT UTC, but
            // formatDate and all the rest of this module use the LOCAL
            // getters. West of Greenwich that goes back a day: someone in
            // UTC-3 asked for 05/01 and the calendar marked 04/01, while the
            // server (date.fromisoformat, no time zone) marked 05/01. The
            // two sides disagreed by a day, all year long.
            //
            // An ISO date without a time is a calendar date, not an
            // instant: built from its components, in the local time zone.
