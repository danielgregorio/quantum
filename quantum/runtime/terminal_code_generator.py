...
class TerminalCodeGenerator:
    ...
    def generate(self, screens: List[ScreenNode], title: str = "Quantum TUI",
        ...
        `functions` e `state_vars` sao os <q:function>/<q:set> escritos no
        topo do <q:application>. So os declarados DENTRO de um <qt:screen>
        chegavam ate aqui, entao um `on-click` apontando para uma funcao
        global caia no ramo "acao desconhecida" e virava um
        `self.notify("Action: salvar")`: o botao anunciava a acao em vez de
        executa-la.
        """
    ...
    def _emit_action_methods(self, py: PyBuilder):
        ...
        for action in sorted(all_actions):
            # `on-click="salvar()"` e a mesma acao que `on-click="salvar"`.
            #
            # Antes o texto CRU virava nome de metodo: `py_id("salvar()")`
            # devolvia `salvar()` e o gerador escrevia
            # `def action_salvar()(self):` — SyntaxError, o app inteiro
            # deixava de importar por causa de um botao. E a comparacao
            # `action in user_func_names` nunca casava com parenteses, entao
            # nem a q:function certa era chamada.
            nome, args = split_call(action)
            if nome in builtin_actions:
                continue
            if nome in emitted:
                continue
            emitted.add(nome)
            ...
            if nome in user_func_names:
                ...
                py.method_def(f'action_{safe_action}', 'self')
                py.docstring(f'Action: {action}')
                py.line(f'self.notify({py_string(f"Action: {nome}")})')
                py.end_block()
