...
from quantum.core.features.terminal_engine.src.ast_nodes import (
    ScreenNode, PanelNode, LayoutNode, TableNode, TerminalInputNode, ButtonNode, MenuNode, TextNode_Terminal, ProgressNode, TreeNode, TabsNode, LogNode_Terminal, HeaderNode_Terminal, FooterNode, StatusNode,
    KeybindingNode, TimerNode_Terminal, ServiceNode, CssNode,
    RawCodeNode_Terminal,
)
from quantum.core.ast_nodes import QuantumNode, QueryNode
...
from quantum.runtime.terminal_templates import (
    APP_TEMPLATE, PyBuilder, py_string, py_id, py_bool, split_call,
)
...
class TerminalCodeGenerator:
    ...
    def generate(self, screens: List[ScreenNode], title: str = "Quantum TUI",
                 ...
                 services: List = None, functions: List = None,
                 state_vars: List = None) -> str:
        """Generate full Python app from ScreenNodes.

        `functions` and `state_vars` are the <q:function>/<q:set> written at the
        top of the <q:application>. Only the ones declared INSIDE a <qt:screen>
        made it here, so an `on-click` pointing to a global function
        fell into the "unknown action" branch and became a
        `self.notify("Action: save")`: the button announced the action instead of
        running it.
        """
        for func in (functions or []):
            if func not in self._functions:
                self._functions.append(func)
        for var in (state_vars or []):
            self._state_vars.append({
                'name': var.name,
                'value': var.value,
                'type': getattr(var, 'type', 'string'),
            })
    ...
    def _build_app_class(self, py: PyBuilder, screens: List[ScreenNode], title: str):
        ...
        if self._timers:
            ...
            for timer in self._timers:
                ...
                py.line(f'self.set_interval({interval}, '
                        f'self.action_{py_id(split_call(action)[0])})')
            py.end_block()
            ...
            for btn_id, action in button_handlers:
                if btn_id:
                    ...
                    if first:
                        ...
                        py.elif_block(cond)
                    py.line(f'self.action_{py_id(split_call(action)[0])}()')
            if not first:
                py.end_block()
            py.end_block()
    ...
    def _emit_action_methods(self, py: PyBuilder):
        ...
        for action in sorted(all_actions):
            # `on-click="save()"` is the same action as `on-click="save"`.
            #
            # Before, the RAW text became the method name: `py_id("save()")`
            # returned `save()` and the generator wrote
            # `def action_save()(self):` — SyntaxError, the whole app
            # stopped importing because of one button. And the comparison
            # `action in user_func_names` never matched with parentheses, so
            # not even the right q:function was called.
            name, args = split_call(action)
