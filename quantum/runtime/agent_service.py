...
from quantum.runtime.llm_providers import (
    MultiProviderLLMService, LLMProviderError
)
...
class ToolCall:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "tool": self.tool,
            "args": self.args,
            # IA-4: the call as a page shows it — low_stock(below=5)
            "call": f"{self.tool}({', '.join(f'{k}={v}' for k, v in (self.args or {}).items())})",
            "result": self.result,
            "error": self.error,
...
IMPORTANT: Always respond with valid JSON in the format shown above. Nothing else.'''
    ...
    def llm_service(self):
        ...
        if self._llm_service is None:
            from quantum.runtime.llm_service import get_llm_service
        ...
        try:
            ...
            while iteration < max_iterations:
                ...
                llm_response = self._call_llm(
                    ...
                    provider=provider,
                    api_key=api_key,
                    # the model gets what is left of the agent's timeout, not a fixed 60s
                    timeout=max(1, int((timeout_ms - elapsed_ms) / 1000)),
                )
