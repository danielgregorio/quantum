...
def setup_logging(config: Dict[str, Any]) -> logging.Logger:
    ...
    from quantum.runtime.web_config import DEFAULT_PATHS
    log_dir = paths_config.get('logs', DEFAULT_PATHS['logs'])

    # Get or create the quantum root logger and clear existing handlers
    logger = logging.getLogger('quantum')
    logger.setLevel(level)
    # Close what is replaced: handlers.clear() alone left each previous
    # FileHandler's file open, one leak per reconfiguration.
    for old in list(logger.handlers):
        logger.removeHandler(old)
        old.close()

    datefmt = '%Y-%m-%d %H:%M:%S'
    # CFG-3: logging.format was documented here and never read.
    plain_fmt = log_config.get('format') or '%(asctime)s %(levelname)-8s %(name)s - %(message)s'
    ...
    if enable_console:
        ...
        colored_fmt = log_config.get('format') or '%(asctime)s %(levelname)s %(name)s - %(message)s'
