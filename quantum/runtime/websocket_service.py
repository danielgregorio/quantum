...
class WebSocketService:
    ...
    def set_connection_state(
        ...
        # The handler runs AFTER releasing the lock (see _dispatch_event).
        pending_event = None
        with self._lock:
            if connection_id in self._connections:
                ...
                if state == WebSocketState.OPEN and old_state != WebSocketState.OPEN:
                    ...
                    pending_event = ("close", {"error": error})

                logger.debug(f"Connection {connection_id} state: {old_state.name} -> {state.name}")

        if pending_event is not None:
            self._dispatch_event(connection_id, pending_event[0], pending_event[1])
    ...
    def _dispatch_event(
        ...
        The handlers run OUTSIDE `self._lock`.

        They used to be called from inside it — `receive_message` wrapped its
        whole body in a `with self._lock` and only then dispatched. The lock
        belongs to the service, not the connection, so a slow on-message of
        ONE connection stopped everything: no other send_message, no other
        receive_message, no state change of any other connection moved while
        the handler had not returned. A ten-second <q:query> inside an
        on-message froze the whole websocket service for ten seconds.
        (Isolating the handler workers per connection in the transport did
        not solve this alone: they fought over the same lock.)

        Here the lock only protects READING the handler list.
        """
        with self._lock:
            ...
            if not conn:
                return
            conn_name = conn.name
            # Copy: a handler may register another, and the live list would
            # change under the iteration.
            handlers = list(self._handlers.get(conn_name, {}).get(event, []))

        for handler in handlers:
            try:
                handler(data)
            except Exception as e:
                logger.error(f"Error in WebSocket handler {conn_name}:{event}: {e}")
    ...
    def receive_message(
        ...
        # Outside the lock: the handler is app code and may take as long as
        # it likes without stopping the other connections.
    ...
    def _send_to_connection(
        ...
        Exists because `broadcast` called `send_message(conn.name, ...)` inside
        a loop over the connections with that name: send_message already walks
        ALL the connections of the name, so N connections got N frames each —
        N*N in total — and the returned count was N. With 50 clients in a
        broadcast that was 2500 frames and each client saw the same message 50
        times. The outer loop needs a send to ONE connection.
        """
    ...
    def broadcast(
        ...
        for conn in connections:
            # ONE connection per iteration. Calling send_message(conn.name)
            # here resent to the whole group on every iteration.
            if self._send_to_connection(conn, data, msg_type):
                sent += 1
