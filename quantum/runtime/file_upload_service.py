...
class FileUploadService:
    ...
    def __init__(self, root: str = None):
        ...
        `root` confines every path this service touches. Defaults to
        ./uploads under the current working directory.
        """
        self.max_file_size = 10 * self.MB  # Default 10MB
        self.allowed_extensions = None  # None = allow all
        self.root = Path(root or 'uploads').resolve()

    def resolve_within_root(self, filepath: str) -> Path:
        """Resolve `filepath` and refuse anything outside the root.

        q:file's path can come straight from databinding — a form field, a
        query parameter. delete_file used to do `Path(filepath).unlink()` with
        no containment at all, so

            <q:file action="delete" file="{fileName}" />

        with fileName = "uploads/../important-database.db" deleted a file
        outside the upload directory and reported
        {'success': True, 'message': 'File deleted successfully'}.
        Reproduced before fixing.

        Same rule as the component router's _is_within_components: reject the
        traversal token outright, then resolve and confirm containment.
        """
        raw = str(filepath or '')
        if not raw:
            raise FileUploadError("empty file path")
        if '..' in raw.replace('\\', '/').split('/'):
            raise FileUploadError(
                f"path {raw!r} escapes the upload directory"
            )
        candidate = Path(raw)
        ...
        if target != self.root and self.root not in target.parents:
            raise FileUploadError(
                f"path {raw!r} resolves outside the upload directory "
                f"({self.root})"
            )
        return target
    ...
    def upload_file(
        ...
        #
        # `destination` is databound, so it can carry a form value. It used to
        # be Path(destination).mkdir(parents=True) with no containment, which
        # let an upload create and write directories anywhere on disk. Every
        # destination= in this repo already lives under ./uploads/, so
        # confining to the root breaks no real usage.
        dest_path = self.resolve_within_root(destination)
    ...
    def delete_file(self, filepath: str) -> Dict[str, Any]:
        ...
        """
        try:
            file_path = self.resolve_within_root(filepath)
