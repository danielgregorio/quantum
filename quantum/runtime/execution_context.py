...
# EXPR-11: a scope variable by name — `session.userId` — and nothing more.
_SCOPE_NAME = re.compile(r'(session|application|request|cookie)\.\w+')
...
class ExecutionContext:
    ...
    def get_variable(self, name: str) -> Any:
        ...
        """
        # Phase F: Parse scope prefix from variable name.
        # EXPR-11: only a scope NAME — `session.n` — is read here. Anything
        # else goes on and, not being a variable, reaches the expression
        # evaluator. `session.n + 1` used to be read as the session key
        # "n + 1", which did not exist, so the page rendered '' in HTML while
        # q:set evaluated the same expression.
        if '.' in name and _SCOPE_NAME.fullmatch(name):
            prefix, var_name = name.split('.', 1)
            if prefix in ['session', 'application', 'request', 'cookie']:
                if prefix == 'session':
    ...
    def get_all_variables(self) -> Dict[str, Any]:
        ...
        # EXPR-11: cookies live in the session under __cookie_<name>; the
        # evaluator reads them as cookie.<name>, as get_variable does.
        for key, value in self.session_vars.items():
            if isinstance(key, str) and key.startswith('__cookie_'):
                all_vars[f'cookie.{key[len("__cookie_"):]}'] = value
