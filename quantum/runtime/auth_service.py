...
class AuthService:
    ...
    def is_session_expired(session_data: Dict[str, Any]) -> bool:
        ...
        try:
            # SET-5: `<q:set name="session.sessionExpiry" value="{dateAdd('h', 8)}"/>`
            # stores a date, not text; the session cookie brings it back
            # timezone-aware. Text (ISO) is read as before.
            expiry_time = expiry_str if isinstance(expiry_str, datetime) else datetime.fromisoformat(str(expiry_str))
