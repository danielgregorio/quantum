...
class QueryValidator:
    ...
    def sanitize_sql(sql: str) -> str:
        ...
        # A regex denylist over SQL text used to live here, matching
        # ';.*DROP TABLE', ';.*DELETE FROM', ';.*INSERT INTO' and
        # 'UNION.*SELECT'. It was removed because it was worse than nothing:
        #
        #   BLOCKED  "SELECT name FROM a UNION SELECT email FROM b"   (legitimate)
        #   ALLOWED  "SELECT 1 ... UNION\nSELECT password FROM users --"
        #   ALLOWED  "SELECT 1;\nDROP TABLE users"
        #
        # `.` does not match a newline, so one line break walked past all four
        # patterns — while a perfectly ordinary UNION query was rejected with
        # "SQL contains suspicious pattern". It broke real queries and stopped
        # nothing, which is the shape of defence SECURITY.md already argues
        # against.
        #
        # It was also guarding a threat this architecture prevents by
        # construction, verified by execution: node.sql is NEVER databound
        # (the executor passes it through untouched), q:param values are bound
        # by the driver, and page/page_size are ints read from the template at
        # parse time. A '{form.name}' written into the SQL body reaches the
        # database as literal text and matches nothing.
        #
        # Which is the real problem worth catching, and what this now does.
        # That silent empty result is the mistake people actually make, and it
        # used to give them no signal at all.
        placeholder = re.search(r'\{[^}\n]+\}', sql)
        if placeholder:
            raise QueryValidationError(
                f"SQL contains the databinding placeholder {placeholder.group(0)}. "
                f"Values are not interpolated into SQL — that is what makes "
                f"injection impossible here. Use a bound parameter instead:\n"
                f"    WHERE col = :name\n"
                f"    <q:param name=\"name\" value=\"{placeholder.group(0)}\" />"
            )
