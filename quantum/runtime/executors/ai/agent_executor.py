    ...
    def execute(self, node: AgentNode, exec_context) -> Any:
        ...
        try:
            ...
            agent_result = self.services.agent.execute(
                ...
                model=self.services.llm_model(
                    self.apply_databinding(node.model, context) if node.model else None,
                    f'<q:agent name="{node.name}">'),
                endpoint=endpoint,
        ...
        # IA-5: an agent that did not finish used to leave success=false and an
        # empty answer on a page that went on as if nothing happened.
        if not result.get('success') and getattr(node, 'on_error', 'fail') != 'continue':
            message = (result.get('error') or {}).get('message') or 'the agent did not finish'
            raise ExecutorError(f'Agent "{node.name}" failed: {message}. Add onerror="continue" to '
                                f'handle it on the page ({node.name}_result.success is false).')
        return result
...
def coerce_tool_args(params, tool_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # IA-4: an argument the model left out takes its q:param's default, as in
    # a q:function call (FN-1). default= was parsed and never applied.
    for param in params or []:
        if param.name not in coerced and getattr(param, 'default', None) is not None:
            coerced[param.name] = coerce_tool_args([param], {param.name: param.default})[param.name]
