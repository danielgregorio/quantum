...
class BaseExecutor(ABC):
    ...
    def evaluate_condition(self, condition: str, context: Dict[str, Any] = None, guard: bool = False) -> bool:
        """
        ...
        if context is None:
            context = self.get_all_variables()
        if guard:
            return self._runtime._evaluate_condition(condition, context, guard=True)
    ...
    def require_python_scripting(self, tag: str):
        ...
        if not self.services.python_scripting_enabled:
            # The message has to say HOW to turn it on. The default became
            # off, so this is the first thing anyone using q:python runs into —
            # and an error that only says "disabled" sends the person digging
            # through the code for which key it is.
            raise ExecutorError(
