...
class BaseLLMProvider(ABC):
    ...
    def stream_chat(self, messages: List[Dict[str, str]], model: Optional[str] = None,
                    **kwargs) -> Iterator[str]:
        """IA-7: the answer in pieces. A provider without streaming sends it whole, once."""
...
class OllamaProvider(BaseLLMProvider):
    ...
    def chat(
        ...
        **kwargs
        ...
        timeout = kwargs.pop('timeout', None) or self.timeout   # per request: q:llm timeout=
        ...
        try:
            ...
            raise LLMProviderError(f"Ollama request timed out after {timeout}s")
    ...
    def stream_chat(self, messages: List[Dict[str, str]], model: Optional[str] = None,
                    ...
                    **kwargs) -> Iterator[str]:
        """IA-7: Ollama streams /api/chat as JSON lines; each line carries a piece."""
        timeout = kwargs.pop('timeout', None) or self.timeout   # per request: q:llm timeout=
        ...
        if options:
            payload["options"] = options
        try:
            with requests.post(f"{self.base_url}/api/chat", json=payload, timeout=timeout,
                               stream=True) as resp:
                resp.raise_for_status()
    ...
    def chat(
        ...
        **kwargs
        ...
        try:
            ...
            raise LLMProviderError(f"Request timed out after {timeout}s")
        except requests.HTTPError as e:
...
class MultiProviderLLMService:
    ...
    def stream_chat(self, messages: List[Dict[str, str]], model: Optional[str] = None,
                    provider: Optional[str] = None, endpoint: Optional[str] = None,
                    api_key: Optional[str] = None, **kwargs) -> Iterator[str]:
        """IA-7: the answer in pieces, from any provider."""
        llm = self._get_provider(provider, endpoint, api_key)
        yield from llm.stream_chat(messages=messages, model=model, **kwargs)
