...
def emit_tilemap_collision(js: JsBuilder):
    """Emit AABB-based tilemap collision resolution.

    This replaces per-tile physics bodies with a direct grid lookup,
    which avoids edge-catching between adjacent tiles and naturally
    supports walking up stair-step terrain.
    """
    js.comment("Tilemap Collision System")
    js.const('_collisionGrids', '[]')
    js.blank()

    # The core resolution function
    js.raw("""function _resolveTilemapCollisions() {
  for (const cg of _collisionGrids) {
    const grid = cg.grid, tw = cg.tw, th = cg.th;
    for (const [id, info] of Object.entries(_sprites)) {
      if (!info.body || info.body.isStatic) continue;
      const body = info.body;
      const bw = body.bounds.max.x - body.bounds.min.x;
      const bh = body.bounds.max.y - body.bounds.min.y;
      const hw = bw / 2, hh = bh / 2;
      let bx = body.position.x, by = body.position.y;

      // Vertical resolution first (landing/ceiling)
      const vy = body.velocity.y;
      const minCol = Math.floor((bx - hw + 2) / tw);
      const maxCol = Math.floor((bx + hw - 2) / tw);

      if (vy >= 0) {
        // Falling or on ground — check below
        const footRow = Math.floor((by + hh) / th);
        if (footRow >= 0 && footRow < grid.length) {
          for (let c = minCol; c <= maxCol; c++) {
            if (c >= 0 && c < (grid[footRow] || []).length && grid[footRow][c]) {
              const tileTop = footRow * th;
              if (by + hh > tileTop && by + hh < tileTop + th) {
                by = tileTop - hh;
        ...
        // Rising — check above
        const headRow = Math.floor((by - hh) / th);
        if (headRow >= 0 && headRow < grid.length) {
          for (let c = minCol; c <= maxCol; c++) {
            if (c >= 0 && c < (grid[headRow] || []).length && grid[headRow][c]) {
              const tileBottom = (headRow + 1) * th;
              if (by - hh < tileBottom && by - hh > tileBottom - th) {
                by = tileBottom + hh;
                Matter.Body.setPosition(body, { x: bx, y: by });
                Matter.Body.setVelocity(body, { x: body.velocity.x, y: 0 });
                break;
      ...
      // Horizontal resolution (walls) with step-up support
      const vx = body.velocity.x;
      if (Math.abs(vx) > 0.1) {
        // Check the column the body is moving into
        const frontX = vx > 0 ? bx + hw : bx - hw;
        const frontCol = Math.floor(frontX / tw);
        const topRow = Math.floor((by - hh + 2) / th);
        const botRow = Math.floor((by + hh - 2) / th);

        let blocked = false;
        for (let r = topRow; r <= botRow; r++) {
          if (r >= 0 && r < grid.length && frontCol >= 0 && frontCol < (grid[r] || []).length && grid[r][frontCol]) {
            blocked = true;
            break;
          }
        }
        ...
        if (blocked) {
          // Check for step-up: is the tile just at foot level?
          // If only the bottom row is blocked and the row above is clear, step up
          const stepRow = botRow;
          const aboveRow = stepRow - 1;
          const onlyFootBlocked = (
            stepRow >= 0 && stepRow < grid.length && frontCol >= 0 &&
            grid[stepRow] && grid[stepRow][frontCol] &&
            (aboveRow < 0 || !grid[aboveRow] || !grid[aboveRow][frontCol])
          );

          if (onlyFootBlocked) {
            // Step up: move body to top of the blocking tile
            const stepTop = stepRow * th;
            const newY = stepTop - hh;
            if (by - newY <= th) { // Only step up one tile height max
              Matter.Body.setPosition(body, { x: bx, y: newY });
              by = newY;
            }
          } else {
            // Full wall — block horizontal movement
            if (vx > 0) {
              Matter.Body.setPosition(body, { x: frontCol * tw - hw, y: by });
            } else {
              Matter.Body.setPosition(body, { x: (frontCol + 1) * tw + hw, y: by });
            }
...
def emit_game_api(js: JsBuilder):
    ...
    js.comment('Update physics body size for big player')
    ...
    # game.setPatrolAI(patrollerId, speed)
    # Makes the patroller walk with given speed and change direction at walls
    js.line("game.setPatrolAI = function(patrollerId, speed) {")
    js.indent()
    js.const('info', '_sprites[patrollerId]')
    js.if_block('!info || !info.body')
    js.ret()
    js.block_close()
    js.blank()
    ...
    js.assign('info._patrolSpeed', 'speed')
    js.assign('info._patrolDir', 'speed < 0 ? -1 : 1')
    js.blank()

    js.comment('Add update behavior for patroller AI')
    ...
    js.line('const gInfo = _sprites[patrollerId];')
    js.line('if (!gInfo || !gInfo.body) return;')
    js.blank()
    js.comment('Apply constant horizontal velocity')
    js.line('const currentSpeed = Math.abs(gInfo._patrolSpeed) * gInfo._patrolDir;')
    ...
    js.indent()
    js.line('gInfo.sprite.scale.x = gInfo._patrolDir > 0 ? -1 : 1;')
    js.dedent()
    js.line('}')
    js.dedent()
    js.line('}')
    ...
    js.comment('Listen for collisions to change direction (lateral only)')
    js.if_block('!info.collisionHandlers')
    js.assign('info.collisionHandlers', '[]')
    js.block_close()
    js.line('info.collisionHandlers.push(function(self, other) {')
    js.indent()
    js.line('const gInfo = _sprites[patrollerId];')
    js.line('if (!gInfo || !gInfo.body || !other.body) return;')
    js.blank()
    js.comment('Only react to terrain/walls, not players or other enemies')
    js.line("const isWall = other.tag === 'terrain' || other.tag === 'tilemap-collision' || other.tag === 'wall' || other.tag === 'item_block';")
    js.line('if (!isWall) return;')
    js.blank()
    js.comment('Use body bounds to detect lateral vs vertical contact')
    js.line('const gb = gInfo.body.bounds;')
    js.line('const ob = other.body.bounds;')
    js.line('const overlapX = Math.min(gb.max.x, ob.max.x) - Math.max(gb.min.x, ob.min.x);')
    js.line('const overlapY = Math.min(gb.max.y, ob.max.y) - Math.max(gb.min.y, ob.min.y);')
    js.comment('Lateral collision: small horizontal overlap, larger vertical overlap')
    js.line('if (overlapX > 0 && overlapY > 0 && overlapX < overlapY) {')
    js.indent()
    js.comment('Cooldown to prevent rapid flipping')
    js.line('const now = Date.now();')
    js.line('if (!gInfo._lastFlipTime || now - gInfo._lastFlipTime > 200) {')
    js.indent()
    js.line('gInfo._patrolDir *= -1;')
    js.line('gInfo._lastFlipTime = now;')
    js.dedent()
    js.line('}')
    js.dedent()
    js.line('}')
    ...
    js.line("app.stage.addChild(overlay);")
    js.line("app.stage.sortChildren();")
    js.line("let frame = 0;")
    js.line("const centerX = app.screen.width / 2;")
    js.line("const centerY = app.screen.height / 2;")
    ...
    js.line("overlay.rect(0, 0, app.screen.width, app.screen.height);")
    ...
    js.line("} else {")
    js.indent()
    js.line("app.stage.removeChild(overlay);")
    ...
    js.const('deathSrc', "deathSpriteSrc || 'assets/classic/sprites/hero_death.png'")
    ...
    js.comment('Remove Hero from physics (so he can do death animation)')
    js.if_block('info.body')
    js.line('Matter.Composite.remove(mWorld, info.body);')
    js.line('delete _bodyToSprite[info.body.id];')
    ...
    js.comment('Death animation - Hero jumps up then falls (slower, wait for sound)')
