...
from quantum.core.features.game_engine_2d.src.ast_nodes import (
    ...
    SpawnNode, HudNode, TweenNode, TilemapNode, BehaviorNode, UseNode, PrefabNode, InstanceNode, GroupNode,
    RawCodeNode,
    ClickableNode, EventNode, OnCollisionNode, HudTileNode, HudCounterNode, HudCollectionNode,
    HudSlotNode, HudBehaviorNode, SceneTransitionNode, PersistentNode, MapNodeDef, MapPathNode,
    EventActionNode, EnemyNode,
)
...
from quantum.runtime.godot_templates import (
    ...
    q2g_gravity, q2g_speed, gd_string, gd_bool, gd_vector2, gd_vector2i, gd_color,
    ...
    SCENE_MANAGER_GD, GAME_OVER_SCREEN_GD, TILEMAP_LOADER_GD,
    # retro console constants
    RETRO_WIDTH, RETRO_HEIGHT, CLASSIC_GRAVITY, CLASSIC_GRAVITY_RISING, CLASSIC_GRAVITY_FALLING,
    CLASSIC_WALK_SPEED, CLASSIC_JUMP_VELOCITY, CLASSIC_MAX_FALL,
    CLASSIC_JUMP_HOLD_BOOST,
)
...
class GodotCodeGenerator:
    ...
    def __init__(self, source_dir: str = None):
        ...
        self._viewport_width: int = RETRO_WIDTH
        self._viewport_height: int = RETRO_HEIGHT
    ...
    def generate(self, scene: SceneNode, behaviors: List[BehaviorNode] = None,
        ...
        self._viewport_width = scene.viewport_width or RETRO_WIDTH
        self._viewport_height = scene.viewport_height or RETRO_HEIGHT
        ...
        # 9. Game over screen (if death_sequence="classic")
    ...
    def _write_multi_project_godot(self, out: Path, initial_scene: SceneNode,
        ...
        config = {
            ...
            'viewport_width': initial_scene.viewport_width or RETRO_WIDTH,
            'viewport_height': initial_scene.viewport_height or RETRO_HEIGHT,
            'gravity': gravity_y,
    ...
    def _build_multi_scene_script(self, scene: SceneNode) -> str:
        ...
        if self._huds:
            ...
            gd.line('$HUD.update_label("bonus_coins", SceneManager.get_state("bonus_coins", 0))')
            ...
            # --- Generic event with actions/transitions ---
                ...
                for action in actions:
                    ...
                    if atype == 'score':
                        ...
                        if self._huds and target in ('coins', 'lives', 'score', 'bonus_coins', 'time_left'):
        ...
        if player_id:
            ...
            if self._huds:
                gd.line('$HUD.stop_timer()')
            # Play death sound
            for sid in sound_triggers.get('player-died', []):
                gd.line(f'_play_sound("{sid}")')
            # Stop BGM
            for bgm_id in bgm_sounds:
                gd.line(f'_stop_sound("{bgm_id}")')
    ...
    def _build_map_scene_script(self, scene: SceneNode) -> str:
        ...
        gd.line('var _player_sprite: Sprite2D')
        ...
        for sprite in self._sprites:
            ...
            gd.line(f'QuantumBridge.register_sprite("{sid}", {_gd_node_ref(sid)})')
        gd.blank()
        gd.line('_player_sprite = $Player if has_node("Player") else null')
        gd.line('if _player_sprite and map_nodes.has(current_node):')
        gd.indent()
        gd.line('var node_info = map_nodes[current_node]')
        gd.line('_player_sprite.position = Vector2(node_info["x"], node_info["y"])')
        gd.dedent()
        ...
        gd.line('if _player_sprite:')
        gd.indent()
        gd.line('var tween = create_tween()')
        gd.line('tween.tween_property(_player_sprite, "position", target_pos, 0.2)')
    ...
    def _write_project_godot(self, out: Path, scene: SceneNode, project_name: str):
        ...
        gravity_y = CLASSIC_GRAVITY
        if self._physics:
            # Use retro gravity as default, ignore Quantum abstract gravity
            pass
        ...
        config = {
            ...
            'viewport_width': scene.viewport_width or RETRO_WIDTH,
            'viewport_height': scene.viewport_height or RETRO_HEIGHT,
            'gravity': gravity_y,
            'input_map': input_map,
            'autoloads': {
                'QuantumBridge': 'scripts/quantum_bridge.gd',
    ...
    def _add_sprite_to_tscn(self, tscn: TscnBuilder, sprite: Dict, parent: str):
        ...
        if sensor:
            ...
            if has_patrol or sprite.get('tag') in ('enemy',):
                tscn.add_node_with_script(
                    sid, node_type, script_path='scripts/patrol_ai.gd',
                    parent=parent, properties=props
                )
            else:
                tscn.add_node(sid, node_type, parent=parent, properties=props)
    ...
    def _write_prefab_scenes(self, prefabs_dir: Path, scripts_dir: Path):
        ...
        for name, prefab in self._prefabs.items():
            ...
            if body_type == 'dynamic':
                ...
                # Detect item_block by tag even without explicit content/hits attrs
                is_item_block = prefab_tag == 'item_block'
                ...
                if enemy_def is not None:
                    # Sensor/hazard enemy (e.g., biting plant with emerge AI)
                    script_name = f'prefab_{name}.gd'
                    tscn.add_node_with_script(
                        name, 'Node2D',
                        script_path=f'scripts/{script_name}',
    ...
    def _write_charge_enemy_script(self, scripts_dir: Path, name: str,
                                    prefab: PrefabNode, enemy: EnemyNode,
                                    anims: list = None):
        """Generate GDScript for charge AI enemy (Bullet Charger - horizontal flight, no gravity)."""
        gd = GdScriptBuilder()
        gd.extends('CharacterBody2D')
        gd.blank()
        ...
        gd.line('QuantumEventBus.emit_event("charger-launch", {"other": self})')
        gd.block_close()
        gd.block_close()
        gd.block_close()
        gd.blank()
    ...
    def _write_emerge_enemy_script(self, scripts_dir: Path, name: str,
                                    prefab: PrefabNode, enemy: EnemyNode,
                                    anims: list = None):
        """Generate GDScript for emerge AI enemy (Biting Plant - timer-based emerge/retract)."""
        gd = GdScriptBuilder()
        gd.extends('Node2D')
        gd.blank()
    ...
    def _write_block_script(self, scripts_dir: Path, name: str,
                            prefab: PrefabNode, anims: list = None):
        """Generate GDScript for a item block (content + hits countdown)."""
        gd = GdScriptBuilder()
        ...
        gd.line('QuantumEventBus.emit_event("block-hit", {"content": _content, "other": self})')
    ...
    def _write_collectible_script(self, scripts_dir: Path, name: str,
        ...
        # Bonus coin shimmer effect (palette shimmer instead of fake 3D spin)
        is_bonus_coin = (prefab_tag == 'bonus_coin')
        if is_bonus_coin:
        ...
        # _process: animation + bonus coin spin
        if has_anim or is_bonus_coin:
            gd.func('_process', 'delta: float')
            if is_bonus_coin:
    ...
    def _write_checkpoint_script(self, scripts_dir: Path, name: str,
        ...
        # _on_body_entered: activate checkpoint with classic-style feedback
        gd.func('_on_body_entered', 'body: Node')
    ...
    def _write_player_controller(self, scripts_dir: Path):
        ...
        # Use sprite-level physics attrs if specified, fallback to classic constants
        speed = float(player.get('speed') or 0) or CLASSIC_WALK_SPEED
        # Convert abstract speed to Godot px/s: if small value (< 10), scale by 60
        if speed < 10:
            speed = speed * 60.0
        jump_vel = CLASSIC_JUMP_VELOCITY
        ...
        if jump_force and float(jump_force) > 0:
            ...
            if jf < 50:
                ...
                jump_vel = -jf
        gravity_up = float(player.get('gravity_up') or 0) or CLASSIC_GRAVITY_RISING
        gravity_down = float(player.get('gravity_down') or 0) or CLASSIC_GRAVITY_FALLING
        jump_hold_boost = float(player.get('jump_hold_boost') or 0) or CLASSIC_JUMP_HOLD_BOOST
        max_fall = float(player.get('max_fall_speed') or 0) or CLASSIC_MAX_FALL
    ...
    def _write_hud_script(self, scripts_dir: Path):
        ...
        label_ready = '\n'.join(
            '\tpass  # Labels auto-connected via @onready'
    ...
    def _write_tile_hud_script(self, scripts_dir: Path):
        ...
        gd.comment('HUD Manager - Tile-based retro-style overlay')
        ...
        gd.raw('\t\tvar digit = s[i].to_int()')
        gd.raw('\t\tvar src_rect = Rect2(digit * dw, 0, dw, dh)')
        gd.raw('\t\tvar dst_rect = Rect2(x + i * dw, y, dw, dh)')
        ...
        if has_right_counter:
            ...
            gd.raw('\t\tvar digit = s[i].to_int()')
            gd.raw('\t\tvar src_rect = Rect2(digit * dw, 0, dw, dh)')
            gd.raw('\t\tvar dst_rect = Rect2(start_x + i * dw, y, dw, dh)')
            gd.raw(f'\t\t_canvas.draw_texture_rect_region({strip_var}, dst_rect, src_rect, tint)')
            gd.func_close()
            gd.blank()
    ...
    def _write_game_over_screen(self, scripts_dir: Path, scene: SceneNode):
        """Write game_over_screen.gd when death_sequence='classic'."""
        death_sequence = getattr(scene, 'death_sequence', None)
        if death_sequence != 'classic':
            return

        alphabet = getattr(scene, 'game_over_alphabet', None) or 'assets/classic/sprites/hud_alphabet.png'
        jingle = getattr(scene, 'game_over_jingle', None) or 'assets/classic/sounds/classic_game_over.wav'
        vp_width = getattr(scene, 'viewport_width', None) or RETRO_WIDTH
    ...
    def _copy_assets(self, out: Path):
        ...
        The _assets set contains paths like 'assets/classic/sprites/walker_walk.png'.
        """
        search_dirs = []
        if self._source_dir:
    ...
    def _build_scene_script(self, scene: SceneNode) -> str:
        ...
        # Auto-declare 'lives' for classic death sequence (default 3)
        if has_death and death_sequence == 'classic':
        ...
        if has_checkpoint_prefab and has_death and death_sequence == 'classic':
            gd.blank()
        ...
        if has_stomp_enemies:
            ...
            if stomp_enemies:
                ...
                gd.if_block('enemy.get_meta("walker_squished", false)')
                ...
                gd.else_block()
                # First stomp: squish
                gd.line('enemy.set_meta("walker_squished", true)')
                gd.line('var fh = enemy.frame_h if "frame_h" in enemy else 32.0')
                gd.if_block('enemy.has_node("Sprite2D")')
                gd.line('enemy.get_node("Sprite2D").scale.y = 0.5')
                gd.line('enemy.get_node("Sprite2D").position.y += fh * 0.25')
