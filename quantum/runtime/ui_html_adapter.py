...
Inside a served page, quantum/runtime/ui_page_renderer.py reuses this drawing
with the page's runtime (UI-1); the desktop is that page in a window (UI-4).
"""
...
from quantum.runtime.ui_html_templates import (
    ...
    VALIDATION_JS, VALIDATION_CSS, RESPONSIVE_CSS,
...
def completed_step_indices(steps, current: int) -> List[int]:
    """Which steps of a ui:stepper count as completed.

    A single authority, on purpose. This rule used to live only here, and the
    desktop adapter's bridge had ITS own version — `list(range(current)) if linear
    else []`. The two disagreed in two ways:

    - `linear` has nothing to do with it. In a non-linear stepper the bridge
      emitted an empty list and ERASED, at boot, the marks this file had
      drawn correctly. The user saw the progress vanish.
    - `completed` is tri-state: not set (None), "true" and "false". The bridge
      collapsed None and "false" into the same value, so a step explicitly
      marked `completed="false"` before the current one got the mark
      back.

    The rule: the explicit attribute wins; without the attribute, a step is
    completed if it is behind the current one.
    """
    indices: List[int] = []
    for i, step in enumerate(steps):
        explicit = getattr(step, 'completed', None)
        if explicit is not None:
            # The same allow-list as UIHtmlAdapter._as_bool. A DENY-list
            # would treat `completed="maybe"` as completed, the opposite
            # of what this file has always done.
            if explicit is True or (
                    not isinstance(explicit, bool)
                    and str(explicit).strip().lower() in ('true', '1', 'yes', 'on')):
                indices.append(i)
        elif i < current:
            indices.append(i)
    return indices
...
def _is_checked(value) -> bool:
    """UI-6: checked="true" (or an expression that resolves to a true value)."""
    return str(value).strip().lower() in ('true', '1', 'on', 'yes', 'checked') if value is not None else False


def _option_attrs(value: str, chosen) -> dict:
    attrs = {'value': value}
    if chosen is not None and value == str(chosen).strip():
        attrs['selected'] = True
    return attrs
...
class UIHtmlAdapter:
    ...
    def _emit_runtime(self, source: str) -> str:
        ...
        This used to fix the JS on the OUTPUT with a regex, because TOAST_JS and
        CALENDAR_JS lived in non-raw triple-quoted strings: Python ate the
        backslashes that escaped quotes inside nested JS literals, and the
        page got `dismiss('' + id + '')` — a syntax error that took down
        the whole module in the browser.

        Fixing the output left the cause standing: the next runtime written the
        same way would break again, silently. The strings became raw in the
        source, so there is nothing left to repair here.
        """
        return source
    ...
    def _layout_attrs(self, node) -> dict:
        ...
        # UI-2: responsive classes
        responsive = []
        if getattr(node, 'stack_below', None):
            responsive.append(f'q-stack-{node.stack_below}')
        if getattr(node, 'hide_below', None):
            responsive.append(f'q-hide-{node.hide_below}')
        if getattr(node, 'hide_above', None):
            responsive.append(f'q-hide-above-{node.hide_above}')
        if str(getattr(node, 'grow', '') or '').lower() == 'true':
            responsive.append('q-grow')
        if responsive:
            attrs['class'] = ' '.join(filter(None, [attrs.get('class')] + responsive))
    ...
    def _render_grid(self, node: UIGridNode, b: HtmlBuilder):
        cols = (node.columns or '1').strip()
        base = {'style': 'display: grid'}
        if ':' in cols or ' ' in cols:
            # UI-2: "1 sm:2 lg:3" — 1 column, 2 from sm, 3 from lg (widest first wins)
            classes = []
            for part in cols.split():
                band, _, n = part.rpartition(':')
                if not n.isdigit() or int(n) not in range(1, 7) or band not in ('', 'sm', 'md', 'lg'):
                    raise ValueError(f'<ui:grid columns="{cols}">: each part is N or sm:N / md:N / lg:N, N from 1 to 6')
                classes.append(f'q-{band}-cols-{n}' if band else f'q-cols-{n}')
            base['class'] = ' '.join(classes)
        else:
            try:
                n = int(cols)
                base['style'] = f"display: grid; grid-template-columns: repeat({n}, 1fr)"
            except ValueError:
                base['style'] = f"display: grid; grid-template-columns: {cols}"
    ...
    def _render_input(self, node: UIInputNode, b: HtmlBuilder):
        ...
        if getattr(node, 'value', None) not in (None, ''):
            input_attrs['value'] = node.value
        if node.placeholder:
            input_attrs['placeholder'] = node.placeholder
        if node.bind:
            input_attrs['name'] = node.bind
        if getattr(node, 'accept', None) and node.input_type == 'file':
            input_attrs['accept'] = node.accept
        ...
        if has_validation:
            self._has_validation = True
            input_id = getattr(node, 'ui_id', None) or node.bind or f"input-{id(node)}"
    ...
    @staticmethod
    def _input_tag(node, attrs: dict, b: HtmlBuilder):
        """<input>, or a <textarea> for ui:input rows= (UI-14); its value is the content."""
        if not getattr(node, 'rows', None):
            b.open_tag('input', attrs, self_closing=True)
            return
        attrs = {k: v for k, v in attrs.items() if k not in ('type', 'value', 'pattern', 'min', 'max')}
        attrs['rows'] = str(node.rows)
        value = getattr(node, 'value', None) or ''
        attr_text = ' '.join(k if v is True else f'{k}="{v}"' for k, v in attrs.items()
                             if v is not None and v is not False)
        b.line(f'<textarea {attr_text}>{value}</textarea>')
    ...
    def _render_checkbox(self, node: UICheckboxNode, b: HtmlBuilder):
        ...
        if _is_checked(getattr(node, 'checked', None)):
            cb_attrs['checked'] = True
    ...
    def _render_radio(self, node: UIRadioNode, b: HtmlBuilder):
        ...
        if node.options:
            for opt in node.options.split(','):
                ...
                if getattr(node, 'value', None) is not None and opt == str(node.value).strip():
    ...
    def _render_select(self, node: UISelectNode, b: HtmlBuilder):
        ...
        if node.options:
            for opt in node.options.split(','):
                opt = opt.strip()
                b.open_tag('option', _option_attrs(opt, getattr(node, 'value', None)))
                b.indent()
                b.text(opt)
                b.dedent()
                b.close_tag('option')
        ...
        for child in getattr(node, 'children', []):
            if isinstance(child, UIOptionNode):
                b.open_tag('option', _option_attrs(child.value or '', getattr(node, 'value', None)))
                b.indent()
    ...
    def _render_card(self, node: UICardNode, b: HtmlBuilder):
        ...
        if node.image:
            ...
            b.close_tag('div')
        # UI-7: the title shows even next to a ui:card-header (it used to vanish).
        if node.title:
